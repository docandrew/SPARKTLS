...
package body RFLX.TLS_Handshake.Key_Share_HRR
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.TLS_Handshake.Key_Share_HRR.Valid_Next (Ctx, Fld);
   ...
   with
     ...
     Post =>
       ...
       and Invalid (Ctx, F_Selected_Group)
   is
   begin
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Key_Share_HRR.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_HRR.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Key_Share_HRR.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Key_Share_HRR.Valid_Next (Ctx, Fld)
       and then Fld in F_Selected_Group
       and then RFLX.TLS_Handshake.Key_Share_HRR.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Key_Share_HRR.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Key_Share_HRR.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Key_Share_HRR.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Key_Share_HRR.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Selected_Group (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Supported_Groups_Enum) is
   begin
      Set_Scalar (Ctx, F_Selected_Group, RFLX.Tls_Parameters.To_Base_Integer (Val));
   end Set_Selected_Group;

   procedure Set_Selected_Group (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Supported_Groups)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_HRR.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and Get_Selected_Group (Ctx) = Val
   ...
   begin
      Struct.Selected_Group := Get_Selected_Group (Ctx);
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Selected_Group (Ctx, Struct.Selected_Group);
   end To_Context;
