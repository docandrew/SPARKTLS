...
package body RFLX.TLS_Handshake.Cookie
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Cookie.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.Cookie.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Cookie then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   function Get_Cookie (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Cookie).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Cookie).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Cookie;

   procedure Get_Cookie (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Cookie).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Cookie).Last);
   begin
   ...
   procedure Generic_Get_Cookie (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Cookie).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Cookie).Last);
   begin
      Process_Cookie (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Cookie;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Cookie.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Cookie.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Cookie.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Cookie.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.Cookie.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Cookie.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Cookie.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Cookie.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Cookie.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Cookie.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Cookie.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Cookie.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Cookie_Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Length;

   procedure Initialize_Cookie_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_Cookie) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Cookie)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Cookie);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Cookie) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      ...
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Cookie) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Cookie_Private;

   procedure Initialize_Cookie (Ctx : in out Context) is
   begin
      Initialize_Cookie_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Cookie)));
   end Initialize_Cookie;

   procedure Set_Cookie (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Cookie));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Cookie_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Cookie)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Cookie)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Cookie))) = Data);
   end Set_Cookie;

   procedure Generic_Set_Cookie (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Cookie));
   begin
      if Length > 0 then
         Process_Cookie (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.TLS_Handshake.Cookie.Valid_Length (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie, Length));
      Initialize_Cookie_Private (Ctx, Length);
   end Generic_Set_Cookie;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Length := Get_Length (Ctx);
      Struct.Cookie := (others => 0);
      Get_Cookie (Ctx, Struct.Cookie (Struct.Cookie'First .. Struct.Cookie'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_Cookie)) + 1) - 2));
   end To_Structure;
   ...
   begin
      ...
      Set_Cookie (Ctx, Struct.Cookie (Struct.Cookie'First .. Struct.Cookie'First + RFLX_Types.Index (RFLX_Types.To_Length (8 * RFLX_Types.Bit_Length (Struct.Length)) + 1) - 2));
   end To_Context;
