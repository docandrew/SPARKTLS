...
package body RFLX.TLS_Handshake.Key_Share_SH
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_SH.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Key_Share_SH.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_SH.Valid_Next (Ctx, Fld),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_SH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_SH.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Share_SH.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.Key_Share_SH.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_SH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_SH.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Share_SH.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Key_Share_SH.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Key_Share_SH.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.Key_Share_SH.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_SH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_SH.Valid_Next (Ctx, Fld)
       and then Fld in F_Group | F_Length
       and then RFLX.TLS_Handshake.Key_Share_SH.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Key_Share_SH.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Key_Share_SH.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Key_Share_SH.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Key_Share_SH.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Share_SH.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Key_Share_SH.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_SH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_SH.Valid_Next (Ctx, RFLX.TLS_Handshake.Key_Share_SH.F_Group)
       and then RFLX.Tls_Parameters.Valid_TLS_Supported_Groups (Val)
       and then RFLX.TLS_Handshake.Key_Share_SH.Available_Space (Ctx, RFLX.TLS_Handshake.Key_Share_SH.F_Group) >= RFLX.TLS_Handshake.Key_Share_SH.Field_Size (Ctx, RFLX.TLS_Handshake.Key_Share_SH.F_Group)
       and then RFLX.TLS_Handshake.Key_Share_SH.Field_Condition (Ctx, RFLX.TLS_Handshake.Key_Share_SH.F_Group),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_SH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_SH.Valid_Next (Ctx, RFLX.TLS_Handshake.Key_Share_SH.F_Key_Exchange)
       and then RFLX.TLS_Handshake.Key_Share_SH.Valid_Length (Ctx, RFLX.TLS_Handshake.Key_Share_SH.F_Key_Exchange, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.Key_Share_SH.Available_Space (Ctx, RFLX.TLS_Handshake.Key_Share_SH.F_Key_Exchange)) >= Length,
     Post =>
       Has_Buffer (Ctx)
   ...
   begin
      ...
      pragma Assert (RFLX.TLS_Handshake.Key_Share_SH.Valid_Length (Ctx, RFLX.TLS_Handshake.Key_Share_SH.F_Key_Exchange, Length));
...
end RFLX.TLS_Handshake.Key_Share_SH;
