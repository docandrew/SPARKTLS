...
package RFLX.TLS_Handshake.Key_Update
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Request_Update, F_Final);

   subtype Field is Virtual_Field range F_Request_Update .. F_Request_Update;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Key_Update.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Update.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Update.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Update.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Key_Update.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Update.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Key_Update.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Key_Update.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Update.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Update.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Key_Update.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Update.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Update.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Update.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Update.Valid_Next (Ctx, Fld);
   ...
   function Get_Request_Update (Ctx : Context) return RFLX.TLS_Handshake.Key_Update_Request
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Update.Valid (Ctx, RFLX.TLS_Handshake.Key_Update.F_Request_Update);
   ...
   procedure Set_Request_Update (Ctx : in out Context; Val : RFLX.TLS_Handshake.Key_Update_Request)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Update.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Update.Valid_Next (Ctx, RFLX.TLS_Handshake.Key_Update.F_Request_Update)
       and then RFLX.TLS_Handshake.Valid_Key_Update_Request (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Key_Update.Available_Space (Ctx, RFLX.TLS_Handshake.Key_Update.F_Request_Update) >= RFLX.TLS_Handshake.Key_Update.Field_Size (Ctx, RFLX.TLS_Handshake.Key_Update.F_Request_Update)
       and then RFLX.TLS_Handshake.Key_Update.Field_Condition (Ctx, RFLX.TLS_Handshake.Key_Update.F_Request_Update),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Request_Update)
       and Get_Request_Update (Ctx) = Val
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Request_Update))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Request_Update) = Valid_Next (Ctx, F_Request_Update)'Old
       and Field_First (Ctx, F_Request_Update) = Field_First (Ctx, F_Request_Update)'Old;
   ...
   type Structure is
      record
         Request_Update : RFLX.TLS_Handshake.Key_Update_Request;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Update.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Update.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.Key_Update.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Request_Update (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Request_Update)) then True))
   with
     Pre =>
   ...
   function Field_First_Request_Update (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Request_Update);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Request_Update =>
             Field_First_Request_Update (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
      and then ((if
                    Well_Formed (Cursors (F_Request_Update))
                 then
                    (Cursors (F_Request_Update).Last - Cursors (F_Request_Update).First + 1 = 8
                     and then Cursors (F_Request_Update).First = First))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Request_Update)
      and then RFLX.TLS_Handshake.Key_Update.Field_First (Ctx, RFLX.TLS_Handshake.Key_Update.F_Request_Update) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Request_Update) = Ctx.Last - Ctx.First + 1
      and then Invalid (Ctx, F_Request_Update));
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Request_Update =>
             RFLX.TLS_Handshake.Valid_Key_Update_Request (Val));
   ...
   function Get_Request_Update (Ctx : Context) return RFLX.TLS_Handshake.Key_Update_Request is
     (To_Actual (Ctx.Cursors (F_Request_Update).Value));
   ...
   function Field_Size_Request_Update (Struct : Structure) return RFLX_Types.Bit_Length is
     (8);

end RFLX.TLS_Handshake.Key_Update;
