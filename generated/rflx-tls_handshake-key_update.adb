...
package body RFLX.TLS_Handshake.Key_Update
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.TLS_Handshake.Key_Update.Valid_Next (Ctx, Fld);
   ...
   with
     ...
     Post =>
       ...
       and Invalid (Ctx, F_Request_Update)
   is
   begin
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Key_Update.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Update.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Update.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Update.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Key_Update.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Key_Update.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Key_Update.Valid_Next (Ctx, Fld)
       and then Fld in F_Request_Update
       and then RFLX.TLS_Handshake.Key_Update.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Key_Update.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Key_Update.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Key_Update.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Key_Update.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Update.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Key_Update.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Request_Update (Ctx : in out Context; Val : RFLX.TLS_Handshake.Key_Update_Request) is
   begin
      Set_Scalar (Ctx, F_Request_Update, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Request_Update;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Request_Update := Get_Request_Update (Ctx);
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Request_Update (Ctx, Struct.Request_Update);
   end To_Context;
