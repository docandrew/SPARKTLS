...
package body RFLX.TLS_Alert.Alert
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Level =>
             Invalid (Ctx.Cursors (F_Description)),
          when F_Description =>
             True);
   ...
   with
     Pre =>
       ...
       and RFLX.TLS_Alert.Alert.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Alert.Alert.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Description then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Alert.Alert.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Alert.Alert.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Alert.Alert.Valid_Next (Ctx, Fld)
       and then Fld in F_Level | F_Description
       and then RFLX.TLS_Alert.Alert.Valid_Value (Fld, Val)
       and then RFLX.TLS_Alert.Alert.Valid_Size (Ctx, Fld, RFLX.TLS_Alert.Alert.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Alert.Alert.Available_Space (Ctx, Fld) >= RFLX.TLS_Alert.Alert.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Alert.Alert.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Alert.Alert.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Level (Ctx : in out Context; Val : RFLX.TLS_Alert.Alert_Level) is
   begin
      Set_Scalar (Ctx, F_Level, RFLX.TLS_Alert.To_Base_Integer (Val));
   end Set_Level;

   procedure Set_Description (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Alerts_Enum) is
   begin
      Set_Scalar (Ctx, F_Description, RFLX.Tls_Parameters.To_Base_Integer (Val));
   end Set_Description;

   procedure Set_Description (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Alerts)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Alert.Alert.Has_Buffer (Ctx)
       and then RFLX.TLS_Alert.Alert.Valid_Next (Ctx, RFLX.TLS_Alert.Alert.F_Description)
       and then RFLX.Tls_Parameters.Valid_TLS_Alerts (Val)
   ...
   begin
      Struct.Level := Get_Level (Ctx);
      Struct.Description := Get_Description (Ctx);
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Level (Ctx, Struct.Level);
      Set_Description (Ctx, Struct.Description);
   end To_Context;
