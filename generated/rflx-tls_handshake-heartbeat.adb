...
package body RFLX.TLS_Handshake.Heartbeat
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.TLS_Handshake.Heartbeat.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Heartbeat.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Heartbeat.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Heartbeat.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Heartbeat.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Heartbeat.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Heartbeat.Available_Space (Ctx, Fld)
     ...
     Post =>
       ...
       and then (Ctx.Cursors (Fld).Value = Val
                 and then (if Fld in F_Mode and then Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, Fld)))
   is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Heartbeat.Valid_Next (Ctx, Fld)
       and then Fld in F_Mode
       and then RFLX.TLS_Handshake.Heartbeat.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Heartbeat.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Heartbeat.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Heartbeat.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Heartbeat.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Heartbeat.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Heartbeat.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Mode (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Heartbeat_Modes_Enum) is
   begin
      Set_Scalar (Ctx, F_Mode, RFLX.Tls_Parameters.To_Base_Integer (Val));
   end Set_Mode;

   procedure Set_Mode (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Heartbeat_Modes)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Heartbeat.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Heartbeat.Valid_Next (Ctx, RFLX.TLS_Handshake.Heartbeat.F_Mode)
       and then RFLX.Tls_Parameters.Valid_TLS_Heartbeat_Modes (Val)
