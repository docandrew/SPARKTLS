...
package body RFLX.TLS_Handshake.Finished
with
  SPARK_Mode
is
   ...
   function Get_Verify_Data (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Verify_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Verify_Data).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Verify_Data;

   procedure Get_Verify_Data (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Verify_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Verify_Data).Last);
   begin
   ...
   procedure Generic_Get_Verify_Data (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Verify_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Verify_Data).Last);
   begin
      Process_Verify_Data (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Verify_Data;
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Finished.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Finished.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Finished.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.Finished.Composite_Field (Fld)
                 then
   ...
   procedure Set_Verify_Data_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Verify_Data, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Verify_Data_Empty;

   procedure Initialize_Verify_Data_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
     ...
     Post =>
       ...
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Verify_Data)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Verify_Data);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Verify_Data) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Verify_Data);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Verify_Data) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Verify_Data_Private;

   procedure Initialize_Verify_Data (Ctx : in out Context; Length : RFLX_Types.Length) is
   begin
      Initialize_Verify_Data_Private (Ctx, Length);
   end Initialize_Verify_Data;

   procedure Set_Verify_Data (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Verify_Data));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Verify_Data_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Verify_Data)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Verify_Data)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Verify_Data))) = Data);
   end Set_Verify_Data;

   procedure Generic_Set_Verify_Data (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Verify_Data));
   begin
      if Length > 0 then
         Process_Verify_Data (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.TLS_Handshake.Finished.Valid_Length (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data, Length));
      Initialize_Verify_Data_Private (Ctx, Length);
   end Generic_Set_Verify_Data;
