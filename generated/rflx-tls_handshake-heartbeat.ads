...
package RFLX.TLS_Handshake.Heartbeat
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Mode, F_Final);

   subtype Field is Virtual_Field range F_Mode .. F_Mode;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Heartbeat.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Heartbeat.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Heartbeat.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Heartbeat.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Heartbeat.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Heartbeat.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Heartbeat.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Heartbeat.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Heartbeat.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Heartbeat.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Heartbeat.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Heartbeat.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Heartbeat.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Heartbeat.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Heartbeat.Valid_Next (Ctx, Fld);
   ...
   function Get_Mode (Ctx : Context) return RFLX.Tls_Parameters.TLS_Heartbeat_Modes
   with
     Pre =>
       RFLX.TLS_Handshake.Heartbeat.Valid (Ctx, RFLX.TLS_Handshake.Heartbeat.F_Mode);
   ...
   procedure Set_Mode (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Heartbeat_Modes_Enum)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Heartbeat.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Heartbeat.Valid_Next (Ctx, RFLX.TLS_Handshake.Heartbeat.F_Mode)
       and then RFLX.Tls_Parameters.Valid_TLS_Heartbeat_Modes (RFLX.Tls_Parameters.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Heartbeat.Available_Space (Ctx, RFLX.TLS_Handshake.Heartbeat.F_Mode) >= RFLX.TLS_Handshake.Heartbeat.Field_Size (Ctx, RFLX.TLS_Handshake.Heartbeat.F_Mode)
       and then RFLX.TLS_Handshake.Heartbeat.Field_Condition (Ctx, RFLX.TLS_Handshake.Heartbeat.F_Mode),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Mode)
       and Get_Mode (Ctx) = (True, Val)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Mode))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Mode) = Valid_Next (Ctx, F_Mode)'Old
       and Field_First (Ctx, F_Mode) = Field_First (Ctx, F_Mode)'Old;
   ...
   type Structure is
      record
         Mode : RFLX.Tls_Parameters.TLS_Heartbeat_Modes;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Heartbeat.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Heartbeat.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.Heartbeat.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Mode (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Mode)) then True))
   with
     Pre =>
   ...
   function Field_First_Mode (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Mode);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Mode =>
             Field_First_Mode (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
      and then ((if
                    Well_Formed (Cursors (F_Mode))
                 then
                    (Cursors (F_Mode).Last - Cursors (F_Mode).First + 1 = 8
                     and then Cursors (F_Mode).First = First))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Mode)
      and then RFLX.TLS_Handshake.Heartbeat.Field_First (Ctx, RFLX.TLS_Handshake.Heartbeat.F_Mode) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Mode) = Ctx.Last - Ctx.First + 1
      and then Invalid (Ctx, F_Mode));
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Mode =>
             RFLX.Tls_Parameters.Valid_TLS_Heartbeat_Modes (Val));
   ...
   function Get_Mode (Ctx : Context) return RFLX.Tls_Parameters.TLS_Heartbeat_Modes is
     (To_Actual (Ctx.Cursors (F_Mode).Value));
   ...
   function Valid_Structure (Struct : Structure) return Boolean is
     (RFLX.Tls_Parameters.Valid_TLS_Heartbeat_Modes (Struct.Mode));
   ...
   function Field_Size_Mode (Struct : Structure) return RFLX_Types.Bit_Length is
     (8);

end RFLX.TLS_Handshake.Heartbeat;
