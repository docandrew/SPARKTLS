...
package body RFLX.TLS_Handshake.NST_Extension
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.NST_Extension.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.NST_Extension.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.NST_Extension.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.NST_Extension.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.NST_Extension.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.NST_Extension.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.NST_Extension.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.NST_Extension.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.NST_Extension.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   begin
      ...
      pragma Assert (RFLX.TLS_Handshake.NST_Extension.Valid_Length (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data, Length));
