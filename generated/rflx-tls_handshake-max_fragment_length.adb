...
package body RFLX.TLS_Handshake.Max_Fragment_Length
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.TLS_Handshake.Max_Fragment_Length.Valid_Next (Ctx, Fld);
   ...
   with
     ...
     Post =>
       ...
       and Invalid (Ctx, F_Max_Fragment_Length)
   is
   begin
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Max_Fragment_Length.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Max_Fragment_Length.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Max_Fragment_Length.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Max_Fragment_Length.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Max_Fragment_Length.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Max_Fragment_Length.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Max_Fragment_Length.Valid_Next (Ctx, Fld)
       and then Fld in F_Max_Fragment_Length
       and then RFLX.TLS_Handshake.Max_Fragment_Length.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Max_Fragment_Length.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Max_Fragment_Length.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Max_Fragment_Length.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Max_Fragment_Length.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Max_Fragment_Length.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Max_Fragment_Length.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Max_Fragment_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Max_Fragment_Length_Value) is
   begin
      Set_Scalar (Ctx, F_Max_Fragment_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Max_Fragment_Length;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Max_Fragment_Length := Get_Max_Fragment_Length (Ctx);
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Max_Fragment_Length (Ctx, Struct.Max_Fragment_Length);
   end To_Context;
