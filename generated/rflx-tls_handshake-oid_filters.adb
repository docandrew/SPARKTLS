...
package body RFLX.TLS_Handshake.OID_Filters
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.OID_Filters.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.OID_Filters.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Filters then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.OID_Filters.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.OID_Filters.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.OID_Filters.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.OID_Filters.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.OID_Filters.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.OID_Filters.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.OID_Filters.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.OID_Filters.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.OID_Filters.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.OID_Filters.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.OID_Filters.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.OID_Filters.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.OID_Filters.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Filters_Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Length;

   procedure Set_Filters_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Filters, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Filters_Empty;

   procedure Set_Filters (Ctx : in out Context; Seq_Ctx : RFLX.TLS_Handshake.Filters.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (RFLX.TLS_Handshake.Filters.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Filters, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      RFLX.TLS_Handshake.Filters.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Filters;

   procedure Initialize_Filters_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.OID_Filters.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.OID_Filters.Valid_Next (Ctx, RFLX.TLS_Handshake.OID_Filters.F_Filters)
       and then RFLX.TLS_Handshake.OID_Filters.Valid_Length (Ctx, RFLX.TLS_Handshake.OID_Filters.F_Filters, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.OID_Filters.Available_Space (Ctx, RFLX.TLS_Handshake.OID_Filters.F_Filters)) >= Length,
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Filters)
       and then Field_Size (Ctx, F_Filters) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Filters)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Filters);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Filters) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Filters);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Filters) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Filters_Private;

   procedure Initialize_Filters (Ctx : in out Context) is
   begin
      Initialize_Filters_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Filters)));
   end Initialize_Filters;

   procedure Switch_To_Filters (Ctx : in out Context; Seq_Ctx : out RFLX.TLS_Handshake.Filters.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Filters);
      Last : constant RFLX_Types.Bit_Index := Field_Last (Ctx, F_Filters);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      if Invalid (Ctx, F_Filters) then
         Reset_Dependent_Fields (Ctx, F_Filters);
         pragma Warnings (Off, "attribute Update is an obsolescent feature");
      ...
      RFLX.TLS_Handshake.Filters.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Filters;

   procedure Update_Filters (Ctx : in out Context; Seq_Ctx : in out RFLX.TLS_Handshake.Filters.Context) is
      Valid_Sequence : constant Boolean := RFLX.TLS_Handshake.OID_Filters.Complete_Filters (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      RFLX.TLS_Handshake.Filters.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
      if Valid_Sequence then
         Ctx.Cursors (F_Filters) := (State => S_Valid, First => Ctx.Cursors (F_Filters).First, Last => Ctx.Cursors (F_Filters).Last, Value => Ctx.Cursors (F_Filters).Value);
      else
         Reset_Dependent_Fields (Ctx, F_Filters);
         Ctx.Cursors (F_Filters) := (others => <>);
      end if;
   end Update_Filters;
