...
package RFLX.TLS_Handshake.Cookie
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Length, F_Cookie, F_Final);

   subtype Field is Virtual_Field range F_Length .. F_Cookie;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Cookie.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Cookie.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Cookie.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Cookie.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Cookie.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Cookie.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Cookie.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Cookie.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Cookie.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Cookie =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Cookie.Valid_Next (Ctx, Fld);
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Cookie_Length
   with
     Pre =>
       RFLX.TLS_Handshake.Cookie.Valid (Ctx, RFLX.TLS_Handshake.Cookie.F_Length);

   pragma Warnings (On, "precondition is always False");

   function Get_Cookie (Ctx : Context) return RFLX_Types.Bytes
   with
     Ghost,
     Pre =>
       RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Cookie.Well_Formed (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie)
       and then RFLX.TLS_Handshake.Cookie.Valid_Next (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie),
     Post =>
       Get_Cookie'Result'Length = RFLX_Types.To_Length (Field_Size (Ctx, F_Cookie));

   procedure Get_Cookie (Ctx : Context; Data : out RFLX_Types.Bytes)
   with
     Pre =>
       RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Cookie.Well_Formed (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie)
       and then RFLX.TLS_Handshake.Cookie.Valid_Next (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie)
       and then Data'Length = RFLX_Types.To_Length (RFLX.TLS_Handshake.Cookie.Field_Size (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie)),
     Post =>
       Equal (Ctx, F_Cookie, Data);

   generic
      with procedure Process_Cookie (Cookie : RFLX_Types.Bytes);
   procedure Generic_Get_Cookie (Ctx : Context)
   with
     Pre =>
       RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Cookie.Present (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie);
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Cookie_Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Cookie.Valid_Next (Ctx, RFLX.TLS_Handshake.Cookie.F_Length)
       and then RFLX.TLS_Handshake.Valid_Cookie_Length (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Cookie.Available_Space (Ctx, RFLX.TLS_Handshake.Cookie.F_Length) >= RFLX.TLS_Handshake.Cookie.Field_Size (Ctx, RFLX.TLS_Handshake.Cookie.F_Length)
       and then RFLX.TLS_Handshake.Cookie.Field_Condition (Ctx, RFLX.TLS_Handshake.Cookie.F_Length),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Initialize_Cookie (Ctx : in out Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Cookie.Valid_Next (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie)
       and then RFLX.TLS_Handshake.Cookie.Available_Space (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie) >= RFLX.TLS_Handshake.Cookie.Field_Size (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie),
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Cookie)
       and then (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Cookie))
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
       and then Valid_Next (Ctx, F_Cookie) = Valid_Next (Ctx, F_Cookie)'Old
       and then Get_Length (Ctx) = Get_Length (Ctx)'Old
       and then Field_First (Ctx, F_Cookie) = Field_First (Ctx, F_Cookie)'Old;

   procedure Set_Cookie (Ctx : in out Context; Data : RFLX_Types.Bytes)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Cookie.Valid_Next (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie)
       and then RFLX.TLS_Handshake.Cookie.Available_Space (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie) >= RFLX.TLS_Handshake.Cookie.Field_Size (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie)
       and then RFLX.TLS_Handshake.Cookie.Valid_Length (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie, Data'Length)
       and then RFLX.TLS_Handshake.Cookie.Available_Space (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.TLS_Handshake.Cookie.Field_Condition (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Cookie)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Cookie))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       ...
       and Field_First (Ctx, F_Cookie) = Field_First (Ctx, F_Cookie)'Old
       and Equal (Ctx, F_Cookie, Data);

   generic
      with procedure Process_Cookie (Cookie : out RFLX_Types.Bytes);
      with function Process_Data_Pre (Length : RFLX_Types.Length) return Boolean;
   procedure Generic_Set_Cookie (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Cookie.Valid_Next (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie)
       and then RFLX.TLS_Handshake.Cookie.Available_Space (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie) >= RFLX.TLS_Handshake.Cookie.Field_Size (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie)
       and then RFLX.TLS_Handshake.Cookie.Valid_Length (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.Cookie.Available_Space (Ctx, RFLX.TLS_Handshake.Cookie.F_Cookie)) >= Length
   ...
   type Structure is
      record
         Length : RFLX.TLS_Handshake.Cookie_Length;
         Cookie : RFLX_Types.Bytes (RFLX_Types.Index'First .. RFLX_Types.Index'First + 65534);
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Cookie.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Cookie.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.Cookie.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Cookie (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Length)) then True)
      and then (if Well_Formed (Cursors (F_Cookie)) then Valid (Cursors (F_Length))))
   with
     Pre =>
   ...
   function Field_First_Cookie (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First + 16)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Cookie);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          ...
          when F_Cookie =>
             Field_First_Cookie (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then ((if
                ...
                and then (if
                             Well_Formed (Cursors (F_Cookie))
                          then
                             (Cursors (F_Cookie).Last - Cursors (F_Cookie).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Length).Value) * 8
                              and then Cursors (F_Cookie).First = Cursors (F_Length).Last + 1))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.TLS_Handshake.Cookie.Field_First (Ctx, RFLX.TLS_Handshake.Cookie.F_Length) rem RFLX_Types.Byte'Size = 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Length =>
             RFLX.TLS_Handshake.Valid_Cookie_Length (Val),
          when F_Cookie =>
             True);

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Length | F_Cookie =>
             True);
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Cookie_Length is
   ...
   function Field_Size_Cookie (Struct : Structure) return RFLX_Types.Bit_Length is
     (RFLX_Types.Bit_Length (Struct.Length) * 8);

end RFLX.TLS_Handshake.Cookie;
