...
package body RFLX.TLS_Handshake.CH_Extension_DTLS
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Tag =>
             Invalid (Ctx.Cursors (F_Data_Length)),
          when F_Data_Length =>
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.CH_Extension_DTLS.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.CH_Extension_DTLS.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.CH_Extension_DTLS.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.CH_Extension_DTLS.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.CH_Extension_DTLS.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.CH_Extension_DTLS.Valid_Next (Ctx, Fld)
       and then Fld in F_Tag | F_Data_Length
       and then RFLX.TLS_Handshake.CH_Extension_DTLS.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.CH_Extension_DTLS.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.CH_Extension_DTLS.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.CH_Extension_DTLS.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.CH_Extension_DTLS.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.CH_Extension_DTLS.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.CH_Extension_DTLS.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Tag (Ctx : in out Context; Val : RFLX.Tls_Extensiontype_Values.TLS_ExtensionType_Values_Enum) is
   begin
      Set_Scalar (Ctx, F_Tag, RFLX.Tls_Extensiontype_Values.To_Base_Integer (Val));
   end Set_Tag;

   procedure Set_Data_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Data_Length) is
   begin
      Set_Scalar (Ctx, F_Data_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Data_Length;

   procedure Set_Tag (Ctx : in out Context; Val : RFLX.Tls_Extensiontype_Values.TLS_ExtensionType_Values)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.CH_Extension_DTLS.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.CH_Extension_DTLS.Valid_Next (Ctx, RFLX.TLS_Handshake.CH_Extension_DTLS.F_Tag)
       and then RFLX.Tls_Extensiontype_Values.Valid_TLS_ExtensionType_Values (Val)
   ...
   begin
      ...
      pragma Assert (RFLX.TLS_Handshake.CH_Extension_DTLS.Valid_Length (Ctx, RFLX.TLS_Handshake.CH_Extension_DTLS.F_Data, Length));
      Initialize_Data_Private (Ctx, Length);
   end Generic_Set_Data;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Tag := Get_Tag (Ctx);
      Struct.Data_Length := Get_Data_Length (Ctx);
   ...
   begin
      Reset (Ctx);
      Set_Tag (Ctx, Struct.Tag);
      Set_Data_Length (Ctx, Struct.Data_Length);
      Set_Data (Ctx, Struct.Data (Struct.Data'First .. Struct.Data'First + RFLX_Types.Index (RFLX_Types.To_Length (8 * RFLX_Types.Bit_Length (Struct.Data_Length)) + 1) - 2));
   end To_Context;
