...
package RFLX.TLS_Handshake.Early_Data_Indication
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Max_Early_Data_Size, F_Final);

   subtype Field is Virtual_Field range F_Max_Early_Data_Size .. F_Max_Early_Data_Size;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Early_Data_Indication.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Early_Data_Indication.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Early_Data_Indication.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Early_Data_Indication.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Early_Data_Indication.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Early_Data_Indication.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Early_Data_Indication.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Early_Data_Indication.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Early_Data_Indication.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Early_Data_Indication.Valid_Next (Ctx, Fld);
   ...
   function Get_Max_Early_Data_Size (Ctx : Context) return RFLX.TLS_Handshake.Max_Early_Data_Size
   with
     Pre =>
       RFLX.TLS_Handshake.Early_Data_Indication.Valid (Ctx, RFLX.TLS_Handshake.Early_Data_Indication.F_Max_Early_Data_Size);
   ...
   procedure Set_Max_Early_Data_Size (Ctx : in out Context; Val : RFLX.TLS_Handshake.Max_Early_Data_Size)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Early_Data_Indication.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Valid_Next (Ctx, RFLX.TLS_Handshake.Early_Data_Indication.F_Max_Early_Data_Size)
       and then RFLX.TLS_Handshake.Valid_Max_Early_Data_Size (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Early_Data_Indication.Available_Space (Ctx, RFLX.TLS_Handshake.Early_Data_Indication.F_Max_Early_Data_Size) >= RFLX.TLS_Handshake.Early_Data_Indication.Field_Size (Ctx, RFLX.TLS_Handshake.Early_Data_Indication.F_Max_Early_Data_Size)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Field_Condition (Ctx, RFLX.TLS_Handshake.Early_Data_Indication.F_Max_Early_Data_Size),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Max_Early_Data_Size)
       and Get_Max_Early_Data_Size (Ctx) = Val
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Max_Early_Data_Size))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Max_Early_Data_Size) = Valid_Next (Ctx, F_Max_Early_Data_Size)'Old
       and Field_First (Ctx, F_Max_Early_Data_Size) = Field_First (Ctx, F_Max_Early_Data_Size)'Old;
   ...
   type Structure is
      record
         Max_Early_Data_Size : RFLX.TLS_Handshake.Max_Early_Data_Size;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Early_Data_Indication.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Max_Early_Data_Size (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Max_Early_Data_Size)) then True))
   with
     Pre =>
   ...
   function Field_First_Max_Early_Data_Size (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Max_Early_Data_Size);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Max_Early_Data_Size =>
             Field_First_Max_Early_Data_Size (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
      and then ((if
                    Well_Formed (Cursors (F_Max_Early_Data_Size))
                 then
                    (Cursors (F_Max_Early_Data_Size).Last - Cursors (F_Max_Early_Data_Size).First + 1 = 32
                     and then Cursors (F_Max_Early_Data_Size).First = First))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Max_Early_Data_Size)
      and then RFLX.TLS_Handshake.Early_Data_Indication.Field_First (Ctx, RFLX.TLS_Handshake.Early_Data_Indication.F_Max_Early_Data_Size) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Max_Early_Data_Size) = Ctx.Last - Ctx.First + 1
      and then Invalid (Ctx, F_Max_Early_Data_Size));
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Max_Early_Data_Size =>
             RFLX.TLS_Handshake.Valid_Max_Early_Data_Size (Val));
   ...
   function Well_Formed_Message (Ctx : Context) return Boolean is
     (Valid (Ctx, F_Max_Early_Data_Size));
   ...
   function Get_Max_Early_Data_Size (Ctx : Context) return RFLX.TLS_Handshake.Max_Early_Data_Size is
     (To_Actual (Ctx.Cursors (F_Max_Early_Data_Size).Value));
   ...
   function Field_Size_Max_Early_Data_Size (Struct : Structure) return RFLX_Types.Bit_Length is
     (32);

end RFLX.TLS_Handshake.Early_Data_Indication;
