...
package body RFLX.TLS_Handshake.Certificate_Entry
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Cert_Data_Length =>
             Invalid (Ctx.Cursors (F_Cert_Data)),
          when F_Cert_Data =>
   ...
   function Composite_Field (Fld : Field) return Boolean is
     (Fld in F_Cert_Data | F_Extensions);
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Certificate_Entry.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.Certificate_Entry.Composite_Field (Fld)
   is
   ...
   function Get_Cert_Data (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Cert_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Cert_Data).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Cert_Data;

   procedure Get_Cert_Data (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Cert_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Cert_Data).Last);
   begin
   ...
   procedure Generic_Get_Cert_Data (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Cert_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Cert_Data).Last);
   begin
      Process_Cert_Data (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Cert_Data;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Certificate_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Certificate_Entry.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Certificate_Entry.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Certificate_Entry.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Certificate_Entry.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.Certificate_Entry.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Certificate_Entry.Valid_Next (Ctx, Fld)
       and then Fld in F_Cert_Data_Length | F_Extensions_Length
       and then RFLX.TLS_Handshake.Certificate_Entry.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Certificate_Entry.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Certificate_Entry.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Certificate_Entry.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Certificate_Entry.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Certificate_Entry.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Certificate_Entry.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Cert_Data_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Cert_Data_Length) is
   begin
      Set_Scalar (Ctx, F_Cert_Data_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Cert_Data_Length;

   procedure Set_Extensions_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Certificate_Extensions_Length) is
   begin
   ...
   procedure Set_Extensions (Ctx : in out Context; Seq_Ctx : RFLX.TLS_Handshake.CT_Extensions.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (RFLX.TLS_Handshake.CT_Extensions.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Extensions, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      RFLX.TLS_Handshake.CT_Extensions.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Extensions;

   procedure Initialize_Cert_Data_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Certificate_Entry.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_Cert_Data) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Cert_Data)
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Cert_Data);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Cert_Data) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Cert_Data);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Cert_Data) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Cert_Data_Private;

   procedure Initialize_Cert_Data (Ctx : in out Context) is
   begin
      Initialize_Cert_Data_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Cert_Data)));
   end Initialize_Cert_Data;
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Certificate_Entry.Valid_Length (Ctx, RFLX.TLS_Handshake.Certificate_Entry.F_Extensions, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.Certificate_Entry.Available_Space (Ctx, RFLX.TLS_Handshake.Certificate_Entry.F_Extensions)) >= Length,
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Set_Cert_Data (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Cert_Data));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Cert_Data_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Cert_Data)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Cert_Data)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Cert_Data))) = Data);
   end Set_Cert_Data;

   procedure Generic_Set_Cert_Data (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Cert_Data));
   begin
      if Length > 0 then
         Process_Cert_Data (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.TLS_Handshake.Certificate_Entry.Valid_Length (Ctx, RFLX.TLS_Handshake.Certificate_Entry.F_Cert_Data, Length));
      Initialize_Cert_Data_Private (Ctx, Length);
   end Generic_Set_Cert_Data;

   procedure Switch_To_Extensions (Ctx : in out Context; Seq_Ctx : out RFLX.TLS_Handshake.CT_Extensions.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Extensions);
   ...
   begin
      ...
      RFLX.TLS_Handshake.CT_Extensions.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Extensions;

   procedure Update_Extensions (Ctx : in out Context; Seq_Ctx : in out RFLX.TLS_Handshake.CT_Extensions.Context) is
      Valid_Sequence : constant Boolean := RFLX.TLS_Handshake.Certificate_Entry.Complete_Extensions (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      RFLX.TLS_Handshake.CT_Extensions.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
