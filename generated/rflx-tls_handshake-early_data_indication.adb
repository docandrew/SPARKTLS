...
package body RFLX.TLS_Handshake.Early_Data_Indication
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.TLS_Handshake.Early_Data_Indication.Valid_Next (Ctx, Fld);
   ...
   with
     ...
     Post =>
       ...
       and Invalid (Ctx, F_Max_Early_Data_Size)
   is
   begin
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Early_Data_Indication.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Early_Data_Indication.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Early_Data_Indication.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Early_Data_Indication.Valid_Next (Ctx, Fld)
       and then Fld in F_Max_Early_Data_Size
       and then RFLX.TLS_Handshake.Early_Data_Indication.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Early_Data_Indication.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Early_Data_Indication.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Early_Data_Indication.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Early_Data_Indication.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Early_Data_Indication.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Max_Early_Data_Size (Ctx : in out Context; Val : RFLX.TLS_Handshake.Max_Early_Data_Size) is
   begin
      Set_Scalar (Ctx, F_Max_Early_Data_Size, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Max_Early_Data_Size;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Max_Early_Data_Size := Get_Max_Early_Data_Size (Ctx);
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Max_Early_Data_Size (Ctx, Struct.Max_Early_Data_Size);
   end To_Context;
