...
is
   ...
   with
     ...
     Pre =>
       ...
       and then RFLX.TLS_Handshake.HRR_Extension_TLS.Field_Condition (Ctx, RFLX.TLS_Handshake.HRR_Extension_TLS.F_Tag),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Tag)
       ...
       and Valid_Next (Ctx, F_Data_Length)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   with
     ...
     Pre =>
       ...
       and then RFLX.TLS_Handshake.HRR_Extension_TLS.Field_Condition (Ctx, RFLX.TLS_Handshake.HRR_Extension_TLS.F_Data_Length),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.HRR_Extension_TLS.Field_Condition (Ctx, RFLX.TLS_Handshake.HRR_Extension_TLS.F_Data)
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Tag)) then True)
      and then (if Well_Formed (Cursors (F_Data_Length)) then Valid (Cursors (F_Tag)))
   ...
   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Tag | F_Data_Length | F_Data =>
             True);
