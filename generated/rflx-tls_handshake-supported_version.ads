...
package RFLX.TLS_Handshake.Supported_Version
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Version, F_Final);

   subtype Field is Virtual_Field range F_Version .. F_Version;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Supported_Version.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Supported_Version.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Supported_Version.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Supported_Version.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Supported_Version.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Supported_Version.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Supported_Version.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Supported_Version.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Supported_Version.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Supported_Version.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Supported_Version.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Supported_Version.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Supported_Version.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Supported_Version.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Supported_Version.Valid_Next (Ctx, Fld);
   ...
   function Get_Version (Ctx : Context) return RFLX.TLS_Common.Protocol_Version
   with
     Pre =>
       RFLX.TLS_Handshake.Supported_Version.Valid (Ctx, RFLX.TLS_Handshake.Supported_Version.F_Version);
   ...
   procedure Set_Version (Ctx : in out Context; Val : RFLX.TLS_Common.Protocol_Version)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Supported_Version.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Supported_Version.Valid_Next (Ctx, RFLX.TLS_Handshake.Supported_Version.F_Version)
       and then RFLX.TLS_Common.Valid_Protocol_Version (RFLX.TLS_Common.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Supported_Version.Available_Space (Ctx, RFLX.TLS_Handshake.Supported_Version.F_Version) >= RFLX.TLS_Handshake.Supported_Version.Field_Size (Ctx, RFLX.TLS_Handshake.Supported_Version.F_Version)
       and then RFLX.TLS_Handshake.Supported_Version.Field_Condition (Ctx, RFLX.TLS_Handshake.Supported_Version.F_Version),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Version)
       and Get_Version (Ctx) = Val
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Version))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   type Structure is
      record
         Version : RFLX.TLS_Common.Protocol_Version;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Supported_Version.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Supported_Version.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.Supported_Version.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then ((if
                 ...
                 then
                    (Cursors (F_Version).Last - Cursors (F_Version).First + 1 = 16
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.TLS_Handshake.Supported_Version.Field_First (Ctx, RFLX.TLS_Handshake.Supported_Version.F_Version) rem RFLX_Types.Byte'Size = 1
   ...
   function Get_Version (Ctx : Context) return RFLX.TLS_Common.Protocol_Version is
...
end RFLX.TLS_Handshake.Supported_Version;
