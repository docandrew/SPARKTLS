...
package body RFLX.TLS_Handshake.Filter
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_OID_Length =>
             Invalid (Ctx.Cursors (F_OID)),
          when F_OID =>
             Invalid (Ctx.Cursors (F_Values_Length)),
          when F_Values_Length =>
             Invalid (Ctx.Cursors (F_Values)),
          when F_Values =>
             True);
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Filter.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.Filter.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Values then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   function Get_OID (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_OID).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_OID).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_OID;

   function Get_Values (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Values).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Values).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Values;

   procedure Get_OID (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_OID).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_OID).Last);
   begin
   ...
   procedure Get_Values (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Values).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Values).Last);
   begin
      Data := (others => RFLX_Types.Byte'First);
   ...
   procedure Generic_Get_OID (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_OID).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_OID).Last);
   begin
      Process_OID (Ctx.Buffer.all (First .. Last));
   end Generic_Get_OID;

   procedure Generic_Get_Values (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Values).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Values).Last);
   begin
      Process_Values (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Values;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Filter.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Filter.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Filter.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Filter.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Filter.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.Filter.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Filter.Valid_Next (Ctx, Fld)
       and then Fld in F_OID_Length | F_Values_Length
       and then RFLX.TLS_Handshake.Filter.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Filter.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Filter.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Filter.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Filter.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Filter.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Filter.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_OID_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.OID_Length) is
   begin
      Set_Scalar (Ctx, F_OID_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_OID_Length;

   procedure Set_Values_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Values_Length) is
   begin
      Set_Scalar (Ctx, F_Values_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Values_Length;
   ...
   procedure Initialize_OID_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Filter.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_OID) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_OID)
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_OID);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_OID) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_OID);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_OID) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_OID_Private;

   procedure Initialize_OID (Ctx : in out Context) is
   begin
      Initialize_OID_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_OID)));
   end Initialize_OID;
   ...
   procedure Set_OID (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_OID));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_OID_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_OID)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_OID)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_OID))) = Data);
   end Set_OID;

   procedure Set_Values (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Values));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Values_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Values)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Values)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Values))) = Data);
   end Set_Values;

   procedure Generic_Set_OID (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_OID));
   begin
      if Length > 0 then
         Process_OID (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.TLS_Handshake.Filter.Valid_Length (Ctx, RFLX.TLS_Handshake.Filter.F_OID, Length));
      Initialize_OID_Private (Ctx, Length);
   end Generic_Set_OID;

   procedure Generic_Set_Values (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Values));
   begin
      if Length > 0 then
         Process_Values (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.TLS_Handshake.Filter.Valid_Length (Ctx, RFLX.TLS_Handshake.Filter.F_Values, Length));
      Initialize_Values_Private (Ctx, Length);
   end Generic_Set_Values;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.OID_Length := Get_OID_Length (Ctx);
      Struct.OID := (others => 0);
      Get_OID (Ctx, Struct.OID (Struct.OID'First .. Struct.OID'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_OID)) + 1) - 2));
      Struct.Values_Length := Get_Values_Length (Ctx);
      Struct.Values := (others => 0);
      Get_Values (Ctx, Struct.Values (Struct.Values'First .. Struct.Values'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_Values)) + 1) - 2));
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_OID_Length (Ctx, Struct.OID_Length);
      Set_OID (Ctx, Struct.OID (Struct.OID'First .. Struct.OID'First + RFLX_Types.Index (RFLX_Types.To_Length (8 * RFLX_Types.Bit_Length (Struct.OID_Length)) + 1) - 2));
      Set_Values_Length (Ctx, Struct.Values_Length);
      Set_Values (Ctx, Struct.Values (Struct.Values'First .. Struct.Values'First + RFLX_Types.Index (RFLX_Types.To_Length (8 * RFLX_Types.Bit_Length (Struct.Values_Length)) + 1) - 2));
   end To_Context;
