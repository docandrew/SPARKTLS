...
package RFLX.TLS_Handshake.Signature_Algorithms
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Signature_Algorithms.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Signature_Algorithms.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Signature_Algorithms.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Signature_Algorithms.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Algorithms =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Signature_Algorithms.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Signature_Algorithms.Valid (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Length);
   ...
   with
     ...
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Valid_Next (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Length)
       and then RFLX.TLS_Handshake.Valid_Signature_Algorithms_Length (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Signature_Algorithms.Available_Space (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Length) >= RFLX.TLS_Handshake.Signature_Algorithms.Field_Size (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Length)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Field_Condition (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Length),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Valid_Next (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Available_Space (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms) >= RFLX.TLS_Handshake.Signature_Algorithms.Field_Size (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Field_Condition (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Valid_Length (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms, RFLX.TLS_Handshake.Signature_Schemes.Byte_Size (Seq_Ctx))
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Valid_Next (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Field_Size (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms) > 0
       and then RFLX.TLS_Handshake.Signature_Algorithms.Field_First (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms) rem RFLX_Types.Byte'Size = 1
       and then RFLX.TLS_Handshake.Signature_Algorithms.Available_Space (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms) >= RFLX.TLS_Handshake.Signature_Algorithms.Field_Size (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Field_Condition (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms),
     Post =>
       not RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Signature_Algorithms.Valid_Next (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms);

   procedure Update_Algorithms (Ctx : in out Context; Seq_Ctx : in out RFLX.TLS_Handshake.Signature_Schemes.Context)
   with
     Pre =>
       RFLX.TLS_Handshake.Signature_Algorithms.Present (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms)
       and then not RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Signature_Schemes.Has_Buffer (Seq_Ctx)
     ...
     Post =>
       (if
           RFLX.TLS_Handshake.Signature_Algorithms.Complete_Algorithms (Ctx, Seq_Ctx)
        then
...
private
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.TLS_Handshake.Signature_Algorithms.Field_First (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Length) rem RFLX_Types.Byte'Size = 1
...
end RFLX.TLS_Handshake.Signature_Algorithms;
