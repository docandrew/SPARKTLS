...
package RFLX.TLS_Handshake.Key_Share_CH
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Length, F_Shares, F_Final);

   subtype Field is Virtual_Field range F_Length .. F_Shares;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_CH.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_CH.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Key_Share_CH.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Key_Share_CH.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_CH.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Key_Share_CH.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_CH.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Share_CH.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_CH.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Shares =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Key_Share_CH.Valid_Next (Ctx, Fld);
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Key_Shares_Length
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_CH.Valid (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Length);
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Key_Shares_Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_CH.Valid_Next (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Length)
       and then RFLX.TLS_Handshake.Valid_Key_Shares_Length (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Key_Share_CH.Available_Space (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Length) >= RFLX.TLS_Handshake.Key_Share_CH.Field_Size (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Length)
       and then RFLX.TLS_Handshake.Key_Share_CH.Field_Condition (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Length),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Length)
       and Get_Length (Ctx) = Val
       and Invalid (Ctx, F_Shares)
       and Valid_Next (Ctx, F_Shares)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   procedure Set_Shares_Empty (Ctx : in out Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_CH.Valid_Next (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares)
       and then RFLX.TLS_Handshake.Key_Share_CH.Available_Space (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares) >= RFLX.TLS_Handshake.Key_Share_CH.Field_Size (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares)
       and then RFLX.TLS_Handshake.Key_Share_CH.Field_Condition (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares)
       and then RFLX.TLS_Handshake.Key_Share_CH.Field_Size (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares) = 0,
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Shares)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Shares))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Shares) = Valid_Next (Ctx, F_Shares)'Old
       and Get_Length (Ctx) = Get_Length (Ctx)'Old
       and Field_First (Ctx, F_Shares) = Field_First (Ctx, F_Shares)'Old;

   procedure Set_Shares (Ctx : in out Context; Seq_Ctx : RFLX.TLS_Handshake.Key_Share_Entries.Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
       ...
       and then RFLX.TLS_Handshake.Key_Share_CH.Available_Space (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares) >= RFLX.TLS_Handshake.Key_Share_CH.Field_Size (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares)
       and then RFLX.TLS_Handshake.Key_Share_CH.Field_Condition (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares)
       and then RFLX.TLS_Handshake.Key_Share_CH.Valid_Length (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares, RFLX.TLS_Handshake.Key_Share_Entries.Byte_Size (Seq_Ctx))
       and then RFLX.TLS_Handshake.Key_Share_Entries.Has_Buffer (Seq_Ctx)
       and then RFLX.TLS_Handshake.Key_Share_Entries.Valid (Seq_Ctx),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Shares)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Shares))
       ...
       and Get_Length (Ctx) = Get_Length (Ctx)'Old
       and Field_First (Ctx, F_Shares) = Field_First (Ctx, F_Shares)'Old
       and (if Field_Size (Ctx, F_Shares) > 0 then Present (Ctx, F_Shares));

   procedure Initialize_Shares (Ctx : in out Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
     ...
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Shares)
       and then (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Shares))
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
       and then Valid_Next (Ctx, F_Shares) = Valid_Next (Ctx, F_Shares)'Old
       and then Get_Length (Ctx) = Get_Length (Ctx)'Old
       and then Field_First (Ctx, F_Shares) = Field_First (Ctx, F_Shares)'Old;

   procedure Switch_To_Shares (Ctx : in out Context; Seq_Ctx : out RFLX.TLS_Handshake.Key_Share_Entries.Context)
   with
     Pre =>
       not Ctx'Constrained
       ...
       and then RFLX.TLS_Handshake.Key_Share_CH.Valid_Next (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares)
       and then RFLX.TLS_Handshake.Key_Share_CH.Field_Size (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares) > 0
       and then RFLX.TLS_Handshake.Key_Share_CH.Field_First (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares) rem RFLX_Types.Byte'Size = 1
       and then RFLX.TLS_Handshake.Key_Share_CH.Available_Space (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares) >= RFLX.TLS_Handshake.Key_Share_CH.Field_Size (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares)
       and then RFLX.TLS_Handshake.Key_Share_CH.Field_Condition (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares),
     Post =>
       not RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Key_Share_Entries.Has_Buffer (Seq_Ctx)
       and Ctx.Buffer_First = Seq_Ctx.Buffer_First
       and Ctx.Buffer_Last = Seq_Ctx.Buffer_Last
       and Seq_Ctx.First = Field_First (Ctx, F_Shares)
       and Seq_Ctx.Last = Field_Last (Ctx, F_Shares)
       and RFLX.TLS_Handshake.Key_Share_Entries.Valid (Seq_Ctx)
       and RFLX.TLS_Handshake.Key_Share_Entries.Sequence_Last (Seq_Ctx) = Seq_Ctx.First - 1
       and Present (Ctx, F_Shares)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Field_Last (Ctx, F_Shares) = Field_Last (Ctx, F_Shares)'Old
     ...
     Contract_Cases =>
       (Well_Formed (Ctx, F_Shares) =>
           True,
        others =>
           True);

   function Complete_Shares (Ctx : Context; Seq_Ctx : RFLX.TLS_Handshake.Key_Share_Entries.Context) return Boolean
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_CH.Valid_Next (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares);

   procedure Update_Shares (Ctx : in out Context; Seq_Ctx : in out RFLX.TLS_Handshake.Key_Share_Entries.Context)
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_CH.Present (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares)
       and then not RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_Entries.Has_Buffer (Seq_Ctx)
       and then Ctx.Buffer_First = Seq_Ctx.Buffer_First
       and then Ctx.Buffer_Last = Seq_Ctx.Buffer_Last
       and then Seq_Ctx.First = Field_First (Ctx, F_Shares)
       and then Seq_Ctx.Last = Field_Last (Ctx, F_Shares),
     Post =>
       (if
           RFLX.TLS_Handshake.Key_Share_CH.Complete_Shares (Ctx, Seq_Ctx)
        then
           Present (Ctx, F_Shares)
        else
           Invalid (Ctx, F_Shares))
       and Has_Buffer (Ctx)
       and not RFLX.TLS_Handshake.Key_Share_Entries.Has_Buffer (Seq_Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       ...
       and RFLX.TLS_Handshake.Key_Share_Entries.Valid (Seq_Ctx) = RFLX.TLS_Handshake.Key_Share_Entries.Valid (Seq_Ctx)'Old
       and RFLX.TLS_Handshake.Key_Share_Entries.Size (Seq_Ctx) = RFLX.TLS_Handshake.Key_Share_Entries.Size (Seq_Ctx)'Old
       and Field_First (Ctx, F_Shares) = Field_First (Ctx, F_Shares)'Old
       and Field_Size (Ctx, F_Shares) = Field_Size (Ctx, F_Shares)'Old
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Length)) then True)
      and then (if Well_Formed (Cursors (F_Shares)) then Valid (Cursors (F_Length))))
   with
     Pre =>
   ...
   function Field_First_Shares (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First + 16)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Shares);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          ...
          when F_Shares =>
             Field_First_Shares (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then ((if
                ...
                and then (if
                             Well_Formed (Cursors (F_Shares))
                          then
                             (Cursors (F_Shares).Last - Cursors (F_Shares).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Length).Value) * 8
                              and then Cursors (F_Shares).First = Cursors (F_Length).Last + 1))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.TLS_Handshake.Key_Share_CH.Field_First (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Length) rem RFLX_Types.Byte'Size = 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Length =>
             RFLX.TLS_Handshake.Valid_Key_Shares_Length (Val),
          when F_Shares =>
             True);

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Length | F_Shares =>
             True);
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Key_Shares_Length is
   ...
   function Complete_Shares (Ctx : Context; Seq_Ctx : RFLX.TLS_Handshake.Key_Share_Entries.Context) return Boolean is
     (RFLX.TLS_Handshake.Key_Share_Entries.Valid (Seq_Ctx)
      and RFLX.TLS_Handshake.Key_Share_Entries.Size (Seq_Ctx) = Field_Size (Ctx, F_Shares));
...
end RFLX.TLS_Handshake.Key_Share_CH;
