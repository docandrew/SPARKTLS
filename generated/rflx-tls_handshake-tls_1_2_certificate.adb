...
package body RFLX.TLS_Handshake.TLS_1_2_Certificate
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.TLS_1_2_Certificate.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.TLS_1_2_Certificate.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.TLS_1_2_Certificate.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.TLS_1_2_Certificate.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.TLS_1_2_Certificate.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.TLS_1_2_Certificate.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.TLS_1_2_Certificate.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.TLS_1_2_Certificate.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.TLS_1_2_Certificate.Valid_Next (Ctx, Fld)
       and then Fld in F_Certificate_List_Length
       and then RFLX.TLS_Handshake.TLS_1_2_Certificate.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.TLS_1_2_Certificate.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.TLS_1_2_Certificate.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.TLS_1_2_Certificate.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.TLS_1_2_Certificate.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.TLS_1_2_Certificate.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.TLS_1_2_Certificate.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Certificate_List_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.TLS_1_2_Cert_List_Length) is
   begin
   ...
   procedure Set_Certificate_List (Ctx : in out Context; Seq_Ctx : RFLX.TLS_Handshake.TLS_1_2_Certificate_Entries.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (RFLX.TLS_Handshake.TLS_1_2_Certificate_Entries.Byte_Size (Seq_Ctx));
   ...
   begin
      ...
      RFLX.TLS_Handshake.TLS_1_2_Certificate_Entries.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.TLS_1_2_Certificate.Valid_Length (Ctx, RFLX.TLS_Handshake.TLS_1_2_Certificate.F_Certificate_List, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.TLS_1_2_Certificate.Available_Space (Ctx, RFLX.TLS_Handshake.TLS_1_2_Certificate.F_Certificate_List)) >= Length,
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Switch_To_Certificate_List (Ctx : in out Context; Seq_Ctx : out RFLX.TLS_Handshake.TLS_1_2_Certificate_Entries.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Certificate_List);
   ...
   begin
      ...
      RFLX.TLS_Handshake.TLS_1_2_Certificate_Entries.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Certificate_List;

   procedure Update_Certificate_List (Ctx : in out Context; Seq_Ctx : in out RFLX.TLS_Handshake.TLS_1_2_Certificate_Entries.Context) is
      Valid_Sequence : constant Boolean := RFLX.TLS_Handshake.TLS_1_2_Certificate.Complete_Certificate_List (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      RFLX.TLS_Handshake.TLS_1_2_Certificate_Entries.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
