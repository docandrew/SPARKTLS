...
package body RFLX.TLS_Handshake.CH_Extension_TLS
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.CH_Extension_TLS.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.CH_Extension_TLS.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.CH_Extension_TLS.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.CH_Extension_TLS.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.CH_Extension_TLS.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.CH_Extension_TLS.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.CH_Extension_TLS.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.CH_Extension_TLS.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.CH_Extension_TLS.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.CH_Extension_TLS.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.CH_Extension_TLS.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.CH_Extension_TLS.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.CH_Extension_TLS.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.CH_Extension_TLS.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.CH_Extension_TLS.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   begin
      ...
      pragma Assert (RFLX.TLS_Handshake.CH_Extension_TLS.Valid_Length (Ctx, RFLX.TLS_Handshake.CH_Extension_TLS.F_Data, Length));
