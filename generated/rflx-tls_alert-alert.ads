...
is
   ...
   type Virtual_Field is (F_Initial, F_Level, F_Description, F_Final);

   subtype Field is Virtual_Field range F_Level .. F_Description;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Alert.Alert.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Alert.Alert.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Alert.Alert.Has_Buffer (Ctx)
       and then RFLX.TLS_Alert.Alert.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Alert.Alert.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Alert.Alert.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Alert.Alert.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Alert.Alert.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Alert.Alert.Has_Buffer (Ctx)
       and then RFLX.TLS_Alert.Alert.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Alert.Alert.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Alert.Alert.Has_Buffer (Ctx)
       and then RFLX.TLS_Alert.Alert.Valid_Value (Fld, Val)
       and then RFLX.TLS_Alert.Alert.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Alert.Alert.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Alert.Alert.Valid_Next (Ctx, Fld);
   ...
   function Get_Level (Ctx : Context) return RFLX.TLS_Alert.Alert_Level
   with
     Pre =>
       RFLX.TLS_Alert.Alert.Valid (Ctx, RFLX.TLS_Alert.Alert.F_Level);

   function Get_Description (Ctx : Context) return RFLX.Tls_Parameters.TLS_Alerts
   with
     Pre =>
       RFLX.TLS_Alert.Alert.Valid (Ctx, RFLX.TLS_Alert.Alert.F_Description);
   ...
   procedure Set_Level (Ctx : in out Context; Val : RFLX.TLS_Alert.Alert_Level)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Alert.Alert.Has_Buffer (Ctx)
       and then RFLX.TLS_Alert.Alert.Valid_Next (Ctx, RFLX.TLS_Alert.Alert.F_Level)
       and then RFLX.TLS_Alert.Valid_Alert_Level (RFLX.TLS_Alert.To_Base_Integer (Val))
       and then RFLX.TLS_Alert.Alert.Available_Space (Ctx, RFLX.TLS_Alert.Alert.F_Level) >= RFLX.TLS_Alert.Alert.Field_Size (Ctx, RFLX.TLS_Alert.Alert.F_Level)
       and then RFLX.TLS_Alert.Alert.Field_Condition (Ctx, RFLX.TLS_Alert.Alert.F_Level, RFLX.TLS_Alert.To_Base_Integer (Val)),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Level)
       and Get_Level (Ctx) = Val
       and Invalid (Ctx, F_Description)
       and Valid_Next (Ctx, F_Description)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Level) = Valid_Next (Ctx, F_Level)'Old
       and Field_First (Ctx, F_Level) = Field_First (Ctx, F_Level)'Old;

   procedure Set_Description (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Alerts_Enum)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Alert.Alert.Has_Buffer (Ctx)
       and then RFLX.TLS_Alert.Alert.Valid_Next (Ctx, RFLX.TLS_Alert.Alert.F_Description)
       and then RFLX.Tls_Parameters.Valid_TLS_Alerts (RFLX.Tls_Parameters.To_Base_Integer (Val))
       and then RFLX.TLS_Alert.Alert.Available_Space (Ctx, RFLX.TLS_Alert.Alert.F_Description) >= RFLX.TLS_Alert.Alert.Field_Size (Ctx, RFLX.TLS_Alert.Alert.F_Description)
       and then RFLX.TLS_Alert.Alert.Field_Condition (Ctx, RFLX.TLS_Alert.Alert.F_Description, RFLX.Tls_Parameters.To_Base_Integer (Val)),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Description)
       and Get_Description (Ctx) = (True, Val)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Description))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Description) = Valid_Next (Ctx, F_Description)'Old
       and Get_Level (Ctx) = Get_Level (Ctx)'Old
       and Field_First (Ctx, F_Description) = Field_First (Ctx, F_Description)'Old
       and (for all F in Field range F_Level .. F_Level =>
   ...
   type Structure is
      record
         Level : RFLX.TLS_Alert.Alert_Level;
         Description : RFLX.Tls_Parameters.TLS_Alerts;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Alert.Alert.Has_Buffer (Ctx)
       and then RFLX.TLS_Alert.Alert.Valid_Structure (Struct)
       and then RFLX.TLS_Alert.Alert.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Level (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);

   function Field_Size_Description (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Level)) then True)
      and then (if Well_Formed (Cursors (F_Description)) then Valid (Cursors (F_Level))))
   with
     Pre =>
   ...
   function Field_First_Level (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Level);

   function Field_First_Description (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First + 8)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Description);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Level =>
             Field_First_Level (Cursors, First, Verified_Last, Written_Last),
          when F_Description =>
             Field_First_Description (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
      and then ((if
                    Well_Formed (Cursors (F_Level))
                 then
                    (Cursors (F_Level).Last - Cursors (F_Level).First + 1 = 8
                     and then Cursors (F_Level).First = First))
                and then (if
                             Well_Formed (Cursors (F_Description))
                          then
                             (Cursors (F_Description).Last - Cursors (F_Description).First + 1 = 8
                              and then Cursors (F_Description).First = Cursors (F_Level).Last + 1))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Level)
      and then RFLX.TLS_Alert.Alert.Field_First (Ctx, RFLX.TLS_Alert.Alert.F_Level) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Level) = Ctx.Last - Ctx.First + 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Level =>
             RFLX.TLS_Alert.Valid_Alert_Level (Val),
          when F_Description =>
             RFLX.Tls_Parameters.Valid_TLS_Alerts (Val));
   ...
   function Field_Condition (Ctx : Context; Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Level =>
             True,
          when F_Description =>
             (RFLX_Types.Base_Integer (Ctx.Cursors (F_Level).Value) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.TLS_Alert.Warning))
              and then (Val = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Parameters.Close_Notify))
                        or else Val = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Parameters.User_Canceled))))
             or else (Val /= RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Parameters.User_Canceled))
                      and then RFLX_Types.Base_Integer (Ctx.Cursors (F_Level).Value) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.TLS_Alert.Fatal))
                      and then Val /= RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Parameters.Close_Notify))));
   ...
   function Well_Formed_Message (Ctx : Context) return Boolean is
     (Valid (Ctx, F_Description)
      and then ((RFLX_Types.Base_Integer (Ctx.Cursors (F_Level).Value) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.TLS_Alert.Warning))
                 and then (RFLX_Types.Base_Integer (Ctx.Cursors (F_Description).Value) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Parameters.Close_Notify))
                           or else RFLX_Types.Base_Integer (Ctx.Cursors (F_Description).Value) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Parameters.User_Canceled))))
                or else (RFLX_Types.Base_Integer (Ctx.Cursors (F_Description).Value) /= RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Parameters.User_Canceled))
                         and then RFLX_Types.Base_Integer (Ctx.Cursors (F_Level).Value) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.TLS_Alert.Fatal))
                         and then RFLX_Types.Base_Integer (Ctx.Cursors (F_Description).Value) /= RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Parameters.Close_Notify)))));
   ...
   function Get_Level (Ctx : Context) return RFLX.TLS_Alert.Alert_Level is
     (To_Actual (Ctx.Cursors (F_Level).Value));

   function Get_Description (Ctx : Context) return RFLX.Tls_Parameters.TLS_Alerts is
     (To_Actual (Ctx.Cursors (F_Description).Value));
   ...
   function Valid_Structure (Struct : Structure) return Boolean is
     (RFLX.Tls_Parameters.Valid_TLS_Alerts (Struct.Description)
      and then ((To_Base_Integer (Struct.Level) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.TLS_Alert.Warning))
                 and then (To_Base_Integer (Struct.Description) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Parameters.Close_Notify))
                           or else To_Base_Integer (Struct.Description) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Parameters.User_Canceled))))
                or else (To_Base_Integer (Struct.Description) /= RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Parameters.User_Canceled))
                         and then To_Base_Integer (Struct.Level) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.TLS_Alert.Fatal))
                         and then To_Base_Integer (Struct.Description) /= RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Parameters.Close_Notify)))));
   ...
   function Field_Size_Level (Struct : Structure) return RFLX_Types.Bit_Length is
     (8);

   function Field_Size_Description (Struct : Structure) return RFLX_Types.Bit_Length is
     (8);

end RFLX.TLS_Alert.Alert;
