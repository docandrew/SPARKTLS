...
package body RFLX.TLS_Handshake.Signature_Algorithms
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Signature_Algorithms.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.Signature_Algorithms.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Signature_Algorithms.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Signature_Algorithms.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.Signature_Algorithms.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Signature_Algorithms.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Signature_Algorithms.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Signature_Algorithms.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Signature_Algorithms.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Signature_Algorithms.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Signature_Algorithms.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Signature_Algorithms.Valid_Length (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.Signature_Algorithms.Available_Space (Ctx, RFLX.TLS_Handshake.Signature_Algorithms.F_Algorithms)) >= Length,
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Update_Algorithms (Ctx : in out Context; Seq_Ctx : in out RFLX.TLS_Handshake.Signature_Schemes.Context) is
      Valid_Sequence : constant Boolean := RFLX.TLS_Handshake.Signature_Algorithms.Complete_Algorithms (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
