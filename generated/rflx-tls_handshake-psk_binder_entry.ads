...
package RFLX.TLS_Handshake.PSK_Binder_Entry
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Length, F_PSK_Binder_Entry, F_Final);

   subtype Field is Virtual_Field range F_Length .. F_PSK_Binder_Entry;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.PSK_Binder_Entry.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.PSK_Binder_Entry.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.PSK_Binder_Entry.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_PSK_Binder_Entry =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Next (Ctx, Fld);
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.PSK_Binder_Entry_Length
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Binder_Entry.Valid (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_Length);

   pragma Warnings (On, "precondition is always False");

   function Get_PSK_Binder_Entry (Ctx : Context) return RFLX_Types.Bytes
   with
     Ghost,
     Pre =>
       RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Well_Formed (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry),
     Post =>
       Get_PSK_Binder_Entry'Result'Length = RFLX_Types.To_Length (Field_Size (Ctx, F_PSK_Binder_Entry));

   procedure Get_PSK_Binder_Entry (Ctx : Context; Data : out RFLX_Types.Bytes)
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Well_Formed (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry)
       and then Data'Length = RFLX_Types.To_Length (RFLX.TLS_Handshake.PSK_Binder_Entry.Field_Size (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry)),
     Post =>
       Equal (Ctx, F_PSK_Binder_Entry, Data);

   generic
      with procedure Process_PSK_Binder_Entry (PSK_Binder_Entry : RFLX_Types.Bytes);
   procedure Generic_Get_PSK_Binder_Entry (Ctx : Context)
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.PSK_Binder_Entry.Present (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry);
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.PSK_Binder_Entry_Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_Length)
       and then RFLX.TLS_Handshake.Valid_PSK_Binder_Entry_Length (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_Length) >= RFLX.TLS_Handshake.PSK_Binder_Entry.Field_Size (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_Length)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Field_Condition (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_Length),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Length)
       and Get_Length (Ctx) = Val
       and Invalid (Ctx, F_PSK_Binder_Entry)
       and Valid_Next (Ctx, F_PSK_Binder_Entry)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   procedure Initialize_PSK_Binder_Entry (Ctx : in out Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry) >= RFLX.TLS_Handshake.PSK_Binder_Entry.Field_Size (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry),
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_PSK_Binder_Entry)
       and then (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_PSK_Binder_Entry))
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
       and then Valid_Next (Ctx, F_PSK_Binder_Entry) = Valid_Next (Ctx, F_PSK_Binder_Entry)'Old
       and then Get_Length (Ctx) = Get_Length (Ctx)'Old
       and then Field_First (Ctx, F_PSK_Binder_Entry) = Field_First (Ctx, F_PSK_Binder_Entry)'Old;

   procedure Set_PSK_Binder_Entry (Ctx : in out Context; Data : RFLX_Types.Bytes)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry) >= RFLX.TLS_Handshake.PSK_Binder_Entry.Field_Size (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Length (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry, Data'Length)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Field_Condition (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_PSK_Binder_Entry)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_PSK_Binder_Entry))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_PSK_Binder_Entry) = Valid_Next (Ctx, F_PSK_Binder_Entry)'Old
       and Get_Length (Ctx) = Get_Length (Ctx)'Old
       and Field_First (Ctx, F_PSK_Binder_Entry) = Field_First (Ctx, F_PSK_Binder_Entry)'Old
       and Equal (Ctx, F_PSK_Binder_Entry, Data);

   generic
      with procedure Process_PSK_Binder_Entry (PSK_Binder_Entry : out RFLX_Types.Bytes);
      with function Process_Data_Pre (Length : RFLX_Types.Length) return Boolean;
   procedure Generic_Set_PSK_Binder_Entry (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry) >= RFLX.TLS_Handshake.PSK_Binder_Entry.Field_Size (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Length (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.PSK_Binder_Entry.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry)) >= Length
   ...
   type Structure is
      record
         Length : RFLX.TLS_Handshake.PSK_Binder_Entry_Length;
         PSK_Binder_Entry : RFLX_Types.Bytes (RFLX_Types.Index'First .. RFLX_Types.Index'First + 254);
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_PSK_Binder_Entry (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Length)) then True)
      and then (if Well_Formed (Cursors (F_PSK_Binder_Entry)) then Valid (Cursors (F_Length))))
   with
     Pre =>
   ...
   function Field_First_PSK_Binder_Entry (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First + 8)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_PSK_Binder_Entry);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          ...
          when F_PSK_Binder_Entry =>
             Field_First_PSK_Binder_Entry (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then ((if
                ...
                and then (if
                             Well_Formed (Cursors (F_PSK_Binder_Entry))
                          then
                             (Cursors (F_PSK_Binder_Entry).Last - Cursors (F_PSK_Binder_Entry).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Length).Value) * 8
                              and then Cursors (F_PSK_Binder_Entry).First = Cursors (F_Length).Last + 1))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.TLS_Handshake.PSK_Binder_Entry.Field_First (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_Length) rem RFLX_Types.Byte'Size = 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Length =>
             RFLX.TLS_Handshake.Valid_PSK_Binder_Entry_Length (Val),
          when F_PSK_Binder_Entry =>
             True);

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Length | F_PSK_Binder_Entry =>
             True);
   ...
   function Well_Formed_Message (Ctx : Context) return Boolean is
     (Well_Formed (Ctx, F_PSK_Binder_Entry));

   function Valid_Message (Ctx : Context) return Boolean is
     (Valid (Ctx, F_PSK_Binder_Entry));

   function Incomplete_Message (Ctx : Context) return Boolean is
     ((for some F in Field =>
          Incomplete (Ctx, F)));

   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.PSK_Binder_Entry_Length is
   ...
   function Field_Size_PSK_Binder_Entry (Struct : Structure) return RFLX_Types.Bit_Length is
     (RFLX_Types.Bit_Length (Struct.Length) * 8);

end RFLX.TLS_Handshake.PSK_Binder_Entry;
