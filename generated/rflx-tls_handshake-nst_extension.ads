...
package RFLX.TLS_Handshake.NST_Extension
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.NST_Extension.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.NST_Extension.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.NST_Extension.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.NST_Extension.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.NST_Extension.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.NST_Extension.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.NST_Extension.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.NST_Extension.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.NST_Extension.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.NST_Extension.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.NST_Extension.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.NST_Extension.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.NST_Extension.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Data =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.NST_Extension.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.NST_Extension.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.NST_Extension.Valid (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Tag);

   function Get_Data_Length (Ctx : Context) return RFLX.TLS_Handshake.Data_Length
   with
     Pre =>
       RFLX.TLS_Handshake.NST_Extension.Valid (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data_Length);
   ...
   with
     ...
     Pre =>
       RFLX.TLS_Handshake.NST_Extension.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.NST_Extension.Well_Formed (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data)
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Next (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data),
     Post =>
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.NST_Extension.Well_Formed (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data)
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Next (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data)
       and then Data'Length = RFLX_Types.To_Length (RFLX.TLS_Handshake.NST_Extension.Field_Size (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data)),
     Post =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.NST_Extension.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.NST_Extension.Present (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data);
   ...
   with
     ...
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.NST_Extension.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Next (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Tag)
       and then RFLX.Tls_Extensiontype_Values.Valid_TLS_ExtensionType_Values (RFLX.Tls_Extensiontype_Values.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.NST_Extension.Available_Space (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Tag) >= RFLX.TLS_Handshake.NST_Extension.Field_Size (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Tag)
       and then RFLX.TLS_Handshake.NST_Extension.Field_Condition (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Tag, RFLX.Tls_Extensiontype_Values.To_Base_Integer (Val)),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Tag)
       ...
       and (if
               RFLX_Types.Base_Integer (To_Base_Integer (Get_Tag (Ctx))) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Extensiontype_Values.Early_Data))
            then
   ...
   with
     ...
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.NST_Extension.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Next (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data_Length)
       and then RFLX.TLS_Handshake.Valid_Data_Length (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.NST_Extension.Available_Space (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data_Length) >= RFLX.TLS_Handshake.NST_Extension.Field_Size (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data_Length)
       and then RFLX.TLS_Handshake.NST_Extension.Field_Condition (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data_Length, RFLX.TLS_Handshake.To_Base_Integer (Val)),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.NST_Extension.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Next (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data)
       and then RFLX.TLS_Handshake.NST_Extension.Available_Space (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data) >= RFLX.TLS_Handshake.NST_Extension.Field_Size (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data)
       and then RFLX.TLS_Handshake.NST_Extension.Field_Condition (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data, 0)
       and then RFLX.TLS_Handshake.NST_Extension.Field_Size (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data) = 0,
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Next (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data)
       and then RFLX.TLS_Handshake.NST_Extension.Available_Space (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data) >= RFLX.TLS_Handshake.NST_Extension.Field_Size (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data)
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Length (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data, Data'Length)
       and then RFLX.TLS_Handshake.NST_Extension.Available_Space (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.TLS_Handshake.NST_Extension.Field_Condition (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data, 0),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Data)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Next (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data)
       and then RFLX.TLS_Handshake.NST_Extension.Available_Space (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data) >= RFLX.TLS_Handshake.NST_Extension.Field_Size (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data)
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Length (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.NST_Extension.Available_Space (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Data)) >= Length
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.NST_Extension.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.NST_Extension.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.NST_Extension.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Tag)) then True)
      and then (if
                ...
                then
                   (Valid (Cursors (F_Tag))
                    and then RFLX_Types.Base_Integer (Cursors (F_Tag).Value) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Extensiontype_Values.Early_Data))))
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.TLS_Handshake.NST_Extension.Field_First (Ctx, RFLX.TLS_Handshake.NST_Extension.F_Tag) rem RFLX_Types.Byte'Size = 1
   ...
   function Field_Condition (Ctx : Context; Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Tag =>
             Val = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Extensiontype_Values.Early_Data)),
   ...
   function Valid_Structure (Struct : Structure) return Boolean is
     (RFLX.Tls_Extensiontype_Values.Valid_TLS_ExtensionType_Values (Struct.Tag)
      and then To_Base_Integer (Struct.Tag) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Tls_Extensiontype_Values.Early_Data)));
...
end RFLX.TLS_Handshake.NST_Extension;
