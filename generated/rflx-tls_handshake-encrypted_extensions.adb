...
package body RFLX.TLS_Handshake.Encrypted_Extensions
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.Encrypted_Extensions.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Encrypted_Extensions.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.Encrypted_Extensions.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Encrypted_Extensions.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Encrypted_Extensions.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Encrypted_Extensions.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Encrypted_Extensions_Length) is
   begin
   ...
   procedure Set_Extensions (Ctx : in out Context; Seq_Ctx : RFLX.TLS_Handshake.EE_Extensions.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (RFLX.TLS_Handshake.EE_Extensions.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Extensions, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      RFLX.TLS_Handshake.EE_Extensions.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Extensions;
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Length (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.Encrypted_Extensions.Available_Space (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions)) >= Length,
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Switch_To_Extensions (Ctx : in out Context; Seq_Ctx : out RFLX.TLS_Handshake.EE_Extensions.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Extensions);
   ...
   begin
      ...
      RFLX.TLS_Handshake.EE_Extensions.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Extensions;

   procedure Update_Extensions (Ctx : in out Context; Seq_Ctx : in out RFLX.TLS_Handshake.EE_Extensions.Context) is
      Valid_Sequence : constant Boolean := RFLX.TLS_Handshake.Encrypted_Extensions.Complete_Extensions (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      RFLX.TLS_Handshake.EE_Extensions.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
