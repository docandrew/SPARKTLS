...
package body RFLX.TLS_Handshake.Cipher_Suite_DTLS
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.TLS_Handshake.Cipher_Suite_DTLS.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Cipher_Suite_DTLS.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Cipher_Suite_DTLS.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Cipher_Suite_DTLS.Available_Space (Ctx, Fld)
     ...
     Post =>
       ...
       and then (Ctx.Cursors (Fld).Value = Val
                 and then (if Fld in F_Suite and then Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, Fld)))
   is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Cipher_Suite_DTLS.Valid_Next (Ctx, Fld)
       and then Fld in F_Suite
       and then RFLX.TLS_Handshake.Cipher_Suite_DTLS.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Cipher_Suite_DTLS.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Cipher_Suite_DTLS.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Cipher_Suite_DTLS.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Cipher_Suite_DTLS.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Cipher_Suite_DTLS.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Cipher_Suite_DTLS.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Suite (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Cipher_Suites_Enum) is
   begin
      Set_Scalar (Ctx, F_Suite, RFLX.Tls_Parameters.To_Base_Integer (Val));
   end Set_Suite;

   procedure Set_Suite (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Cipher_Suites)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Cipher_Suite_DTLS.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Cipher_Suite_DTLS.Valid_Next (Ctx, RFLX.TLS_Handshake.Cipher_Suite_DTLS.F_Suite)
       and then RFLX.Tls_Parameters.Valid_TLS_Cipher_Suites (Val)
