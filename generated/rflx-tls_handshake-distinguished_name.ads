...
package RFLX.TLS_Handshake.Distinguished_Name
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Length, F_Name, F_Final);

   subtype Field is Virtual_Field range F_Length .. F_Name;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Distinguished_Name.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Distinguished_Name.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Distinguished_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Distinguished_Name.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Distinguished_Name.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Distinguished_Name.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Distinguished_Name.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Distinguished_Name.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Distinguished_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Distinguished_Name.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Distinguished_Name.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Distinguished_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Distinguished_Name.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Distinguished_Name.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Distinguished_Name.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Name =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Distinguished_Name.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Distinguished_Name.Valid_Next (Ctx, Fld);
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Distinguished_Name_Length
   with
     Pre =>
       RFLX.TLS_Handshake.Distinguished_Name.Valid (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Length);
   ...
   with
     ...
     Pre =>
       RFLX.TLS_Handshake.Distinguished_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Distinguished_Name.Well_Formed (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name)
       and then RFLX.TLS_Handshake.Distinguished_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name),
     Post =>
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Distinguished_Name.Well_Formed (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name)
       and then RFLX.TLS_Handshake.Distinguished_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name)
       and then Data'Length = RFLX_Types.To_Length (RFLX.TLS_Handshake.Distinguished_Name.Field_Size (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name)),
     Post =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Distinguished_Name.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Distinguished_Name.Present (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name);
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Distinguished_Name_Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Distinguished_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Distinguished_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Length)
       and then RFLX.TLS_Handshake.Valid_Distinguished_Name_Length (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Distinguished_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Length) >= RFLX.TLS_Handshake.Distinguished_Name.Field_Size (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Length)
       and then RFLX.TLS_Handshake.Distinguished_Name.Field_Condition (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Length),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Distinguished_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Distinguished_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name)
       and then RFLX.TLS_Handshake.Distinguished_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name) >= RFLX.TLS_Handshake.Distinguished_Name.Field_Size (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Distinguished_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name)
       and then RFLX.TLS_Handshake.Distinguished_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name) >= RFLX.TLS_Handshake.Distinguished_Name.Field_Size (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name)
       and then RFLX.TLS_Handshake.Distinguished_Name.Valid_Length (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name, Data'Length)
       and then RFLX.TLS_Handshake.Distinguished_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.TLS_Handshake.Distinguished_Name.Field_Condition (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Distinguished_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name)
       and then RFLX.TLS_Handshake.Distinguished_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name) >= RFLX.TLS_Handshake.Distinguished_Name.Field_Size (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name)
       and then RFLX.TLS_Handshake.Distinguished_Name.Valid_Length (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.Distinguished_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Name)) >= Length
   ...
   type Structure is
      record
         Length : RFLX.TLS_Handshake.Distinguished_Name_Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Distinguished_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Distinguished_Name.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.Distinguished_Name.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.TLS_Handshake.Distinguished_Name.Field_First (Ctx, RFLX.TLS_Handshake.Distinguished_Name.F_Length) rem RFLX_Types.Byte'Size = 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Length =>
             RFLX.TLS_Handshake.Valid_Distinguished_Name_Length (Val),
          when F_Name =>
             True);

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Length | F_Name =>
             True);
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Distinguished_Name_Length is
...
end RFLX.TLS_Handshake.Distinguished_Name;
