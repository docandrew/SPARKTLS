...
package body RFLX.TLS_Handshake.PSK_Identity
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Length =>
             Invalid (Ctx.Cursors (F_Identity)),
          when F_Identity =>
             Invalid (Ctx.Cursors (F_Obfuscated_Ticket_Age)),
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.PSK_Identity.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.PSK_Identity.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Obfuscated_Ticket_Age then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   function Get_Identity (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Identity).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Identity).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Identity;

   procedure Get_Identity (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Identity).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Identity).Last);
   begin
   ...
   procedure Generic_Get_Identity (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Identity).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Identity).Last);
   begin
      Process_Identity (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Identity;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.PSK_Identity.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.PSK_Identity.Composite_Field (Fld)
                 then
     ...
     Post =>
       ...
       and then (Ctx.Cursors (Fld).Value = Val
                 and then (if
                              Fld in F_Obfuscated_Ticket_Age
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Next (Ctx, Fld)
       and then Fld in F_Length | F_Obfuscated_Ticket_Age
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.PSK_Identity.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.PSK_Identity.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.PSK_Identity.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.PSK_Identity.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.PSK_Identity.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Identity_Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Length;

   procedure Set_Obfuscated_Ticket_Age (Ctx : in out Context; Val : RFLX.TLS_Handshake.Obfuscated_Ticket_Age) is
   begin
      Set_Scalar (Ctx, F_Obfuscated_Ticket_Age, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Obfuscated_Ticket_Age;

   procedure Initialize_Identity_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_Identity) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Identity)
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Identity);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Identity) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Identity);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Identity) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Identity_Private;

   procedure Initialize_Identity (Ctx : in out Context) is
   begin
      Initialize_Identity_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Identity)));
   end Initialize_Identity;

   procedure Set_Identity (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Identity));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Identity_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Identity)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Identity)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Identity))) = Data);
   end Set_Identity;

   procedure Generic_Set_Identity (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Identity));
   begin
      if Length > 0 then
         Process_Identity (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.TLS_Handshake.PSK_Identity.Valid_Length (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity, Length));
      Initialize_Identity_Private (Ctx, Length);
   end Generic_Set_Identity;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Length := Get_Length (Ctx);
      Struct.Identity := (others => 0);
      Get_Identity (Ctx, Struct.Identity (Struct.Identity'First .. Struct.Identity'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_Identity)) + 1) - 2));
      Struct.Obfuscated_Ticket_Age := Get_Obfuscated_Ticket_Age (Ctx);
   end To_Structure;
   ...
   begin
      ...
      Set_Identity (Ctx, Struct.Identity (Struct.Identity'First .. Struct.Identity'First + RFLX_Types.Index (RFLX_Types.To_Length (8 * RFLX_Types.Bit_Length (Struct.Length)) + 1) - 2));
      Set_Obfuscated_Ticket_Age (Ctx, Struct.Obfuscated_Ticket_Age);
   end To_Context;
