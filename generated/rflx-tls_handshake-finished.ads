...
package RFLX.TLS_Handshake.Finished
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Verify_Data, F_Final);

   subtype Field is Virtual_Field range F_Verify_Data .. F_Verify_Data;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Finished.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Finished.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Finished.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Finished.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Finished.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Finished.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Finished.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Finished.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Finished.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Verify_Data =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Finished.Valid_Next (Ctx, Fld);
   ...
   function Get_Verify_Data (Ctx : Context) return RFLX_Types.Bytes
   with
     Ghost,
     Pre =>
       RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Finished.Well_Formed (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data)
       and then RFLX.TLS_Handshake.Finished.Valid_Next (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data),
     Post =>
       Get_Verify_Data'Result'Length = RFLX_Types.To_Length (Field_Size (Ctx, F_Verify_Data));

   procedure Get_Verify_Data (Ctx : Context; Data : out RFLX_Types.Bytes)
   with
     Pre =>
       RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Finished.Well_Formed (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data)
       and then RFLX.TLS_Handshake.Finished.Valid_Next (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data)
       and then Data'Length = RFLX_Types.To_Length (RFLX.TLS_Handshake.Finished.Field_Size (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data)),
     Post =>
       Equal (Ctx, F_Verify_Data, Data);

   generic
      with procedure Process_Verify_Data (Verify_Data : RFLX_Types.Bytes);
   procedure Generic_Get_Verify_Data (Ctx : Context)
   with
     Pre =>
       RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Finished.Present (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data);
   ...
   procedure Set_Verify_Data_Empty (Ctx : in out Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Finished.Valid_Next (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data)
       and then RFLX.TLS_Handshake.Finished.Available_Space (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data) >= RFLX.TLS_Handshake.Finished.Field_Size (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data)
       and then RFLX.TLS_Handshake.Finished.Field_Condition (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data)
       and then RFLX.TLS_Handshake.Finished.Field_Size (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data) = 0,
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Verify_Data)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Verify_Data))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Verify_Data) = Valid_Next (Ctx, F_Verify_Data)'Old
       and Field_First (Ctx, F_Verify_Data) = Field_First (Ctx, F_Verify_Data)'Old;

   procedure Initialize_Verify_Data (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Finished.Valid_Next (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data)
       and then RFLX.TLS_Handshake.Finished.Valid_Length (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data, Length)
       and then RFLX.TLS_Handshake.Finished.Available_Space (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data) >= RFLX_Types.To_Bit_Length (Length),
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Verify_Data)
       and then Field_Size (Ctx, F_Verify_Data) = RFLX_Types.To_Bit_Length (Length)
       and then (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Verify_Data))
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
       and then Valid_Next (Ctx, F_Verify_Data) = Valid_Next (Ctx, F_Verify_Data)'Old
       and then Field_First (Ctx, F_Verify_Data) = Field_First (Ctx, F_Verify_Data)'Old;

   procedure Set_Verify_Data (Ctx : in out Context; Data : RFLX_Types.Bytes)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Finished.Valid_Next (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data)
       and then RFLX.TLS_Handshake.Finished.Available_Space (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data) >= RFLX.TLS_Handshake.Finished.Field_Size (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data)
       and then RFLX.TLS_Handshake.Finished.Valid_Length (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data, Data'Length)
       and then RFLX.TLS_Handshake.Finished.Available_Space (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.TLS_Handshake.Finished.Field_Condition (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Verify_Data)
       ...
       and Valid_Next (Ctx, F_Verify_Data) = Valid_Next (Ctx, F_Verify_Data)'Old
       and Field_First (Ctx, F_Verify_Data) = Field_First (Ctx, F_Verify_Data)'Old
       and Equal (Ctx, F_Verify_Data, Data);

   generic
      with procedure Process_Verify_Data (Verify_Data : out RFLX_Types.Bytes);
      with function Process_Data_Pre (Length : RFLX_Types.Length) return Boolean;
   procedure Generic_Set_Verify_Data (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Finished.Has_Buffer (Ctx)
       ...
       and then RFLX.TLS_Handshake.Finished.Available_Space (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data) >= RFLX.TLS_Handshake.Finished.Field_Size (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data)
       and then RFLX.TLS_Handshake.Finished.Valid_Length (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.Finished.Available_Space (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data)) >= Length
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Verify_Data)) then True))
   with
     Pre =>
   ...
   function Field_First_Verify_Data (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Verify_Data);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Verify_Data =>
             Field_First_Verify_Data (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
      and then ((if
                    Well_Formed (Cursors (F_Verify_Data))
                 then
                    (Cursors (F_Verify_Data).Last - Cursors (F_Verify_Data).First + 1 = RFLX_Types.Bit_Length (Written_Last - First + 1)
                     and then Cursors (F_Verify_Data).First = First))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Verify_Data)
      and then RFLX.TLS_Handshake.Finished.Field_First (Ctx, RFLX.TLS_Handshake.Finished.F_Verify_Data) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Verify_Data) = Ctx.Last - Ctx.First + 1
      and then Invalid (Ctx, F_Verify_Data));
   ...
   function Well_Formed_Message (Ctx : Context) return Boolean is
     (Well_Formed (Ctx, F_Verify_Data));

   function Valid_Message (Ctx : Context) return Boolean is
     (Valid (Ctx, F_Verify_Data));
...
end RFLX.TLS_Handshake.Finished;
