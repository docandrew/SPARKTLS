...
package body RFLX.TLS_Handshake.Pre_Shared_Key_SH
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.TLS_Handshake.Pre_Shared_Key_SH.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Pre_Shared_Key_SH.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Pre_Shared_Key_SH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Pre_Shared_Key_SH.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Pre_Shared_Key_SH.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Pre_Shared_Key_SH.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Pre_Shared_Key_SH.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Pre_Shared_Key_SH.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Pre_Shared_Key_SH.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Pre_Shared_Key_SH.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Pre_Shared_Key_SH.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Pre_Shared_Key_SH.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Pre_Shared_Key_SH.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Pre_Shared_Key_SH.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Selected_Identity (Ctx : in out Context; Val : RFLX.TLS_Handshake.Identity_Index) is
   begin
      Set_Scalar (Ctx, F_Selected_Identity, RFLX.TLS_Handshake.To_Base_Integer (Val));
