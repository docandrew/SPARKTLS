...
package RFLX.TLS_Handshake.Server_Name
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Name_Type, F_Length, F_Name, F_Final);

   subtype Field is Virtual_Field range F_Name_Type .. F_Name;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Server_Name.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Server_Name.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Server_Name.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Server_Name.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Server_Name.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Name =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Server_Name.Valid_Next (Ctx, Fld);
   ...
   function Get_Name_Type (Ctx : Context) return RFLX.TLS_Handshake.Name_Type
   with
     Pre =>
       RFLX.TLS_Handshake.Server_Name.Valid (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name_Type);

   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Name_Length
   with
     Pre =>
       RFLX.TLS_Handshake.Server_Name.Valid (Ctx, RFLX.TLS_Handshake.Server_Name.F_Length);

   pragma Warnings (On, "precondition is always False");

   function Get_Name (Ctx : Context) return RFLX_Types.Bytes
   with
     Ghost,
     Pre =>
       RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Well_Formed (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name),
     Post =>
       Get_Name'Result'Length = RFLX_Types.To_Length (Field_Size (Ctx, F_Name));

   procedure Get_Name (Ctx : Context; Data : out RFLX_Types.Bytes)
   with
     Pre =>
       RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Well_Formed (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name)
       and then Data'Length = RFLX_Types.To_Length (RFLX.TLS_Handshake.Server_Name.Field_Size (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name)),
     Post =>
       Equal (Ctx, F_Name, Data);

   generic
      with procedure Process_Name (Name : RFLX_Types.Bytes);
   procedure Generic_Get_Name (Ctx : Context)
   with
     Pre =>
       RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Server_Name.Present (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name);
   ...
   procedure Set_Name_Type (Ctx : in out Context; Val : RFLX.TLS_Handshake.Name_Type)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name_Type)
       and then RFLX.TLS_Handshake.Valid_Name_Type (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Server_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name_Type) >= RFLX.TLS_Handshake.Server_Name.Field_Size (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name_Type)
       and then RFLX.TLS_Handshake.Server_Name.Field_Condition (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name_Type),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Name_Type)
       ...
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Name_Type) = Valid_Next (Ctx, F_Name_Type)'Old
       and Field_First (Ctx, F_Name_Type) = Field_First (Ctx, F_Name_Type)'Old;

   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Name_Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Server_Name.F_Length)
       and then RFLX.TLS_Handshake.Valid_Name_Length (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Server_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Server_Name.F_Length) >= RFLX.TLS_Handshake.Server_Name.Field_Size (Ctx, RFLX.TLS_Handshake.Server_Name.F_Length)
       and then RFLX.TLS_Handshake.Server_Name.Field_Condition (Ctx, RFLX.TLS_Handshake.Server_Name.F_Length),
     Post =>
       Has_Buffer (Ctx)
       ...
       and (for all F in Field range F_Name_Type .. F_Name_Type =>
   ...
   procedure Initialize_Name (Ctx : in out Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name)
       and then RFLX.TLS_Handshake.Server_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name) >= RFLX.TLS_Handshake.Server_Name.Field_Size (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name),
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Name)
       and then (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Name))
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
       and then Valid_Next (Ctx, F_Name) = Valid_Next (Ctx, F_Name)'Old
       and then Get_Length (Ctx) = Get_Length (Ctx)'Old
       and then Field_First (Ctx, F_Name) = Field_First (Ctx, F_Name)'Old;

   procedure Set_Name (Ctx : in out Context; Data : RFLX_Types.Bytes)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name)
       and then RFLX.TLS_Handshake.Server_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name) >= RFLX.TLS_Handshake.Server_Name.Field_Size (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Length (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name, Data'Length)
       and then RFLX.TLS_Handshake.Server_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.TLS_Handshake.Server_Name.Field_Condition (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Name)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Name))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       ...
       and Field_First (Ctx, F_Name) = Field_First (Ctx, F_Name)'Old
       and Equal (Ctx, F_Name, Data);

   generic
      with procedure Process_Name (Name : out RFLX_Types.Bytes);
      with function Process_Data_Pre (Length : RFLX_Types.Length) return Boolean;
   procedure Generic_Set_Name (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Next (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name)
       and then RFLX.TLS_Handshake.Server_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name) >= RFLX.TLS_Handshake.Server_Name.Field_Size (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Length (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.Server_Name.Available_Space (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name)) >= Length
   ...
   type Structure is
      record
         Name_Type : RFLX.TLS_Handshake.Name_Type;
         Length : RFLX.TLS_Handshake.Name_Length;
         Name : RFLX_Types.Bytes (RFLX_Types.Index'First .. RFLX_Types.Index'First + 65534);
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.Server_Name.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Name_Type (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
   ...
   function Field_Size_Name (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Name_Type)) then True)
      and then (if Well_Formed (Cursors (F_Length)) then Valid (Cursors (F_Name_Type)))
      and then (if Well_Formed (Cursors (F_Name)) then Valid (Cursors (F_Length))))
   with
     Pre =>
   ...
   function Field_First_Name_Type (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Name_Type);
   ...
   function Field_First_Name (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First + 24)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Name);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Name_Type =>
             Field_First_Name_Type (Cursors, First, Verified_Last, Written_Last),
          when F_Length =>
             Field_First_Length (Cursors, First, Verified_Last, Written_Last),
          when F_Name =>
             Field_First_Name (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
      and then ((if
                    Well_Formed (Cursors (F_Name_Type))
                 then
                    (Cursors (F_Name_Type).Last - Cursors (F_Name_Type).First + 1 = 8
                     and then Cursors (F_Name_Type).First = First))
                and then (if
                             Well_Formed (Cursors (F_Length))
                          then
                             (Cursors (F_Length).Last - Cursors (F_Length).First + 1 = 16
                              and then Cursors (F_Length).First = Cursors (F_Name_Type).Last + 1))
                and then (if
                             Well_Formed (Cursors (F_Name))
                          then
                             (Cursors (F_Name).Last - Cursors (F_Name).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Length).Value) * 8
                              and then Cursors (F_Name).First = Cursors (F_Length).Last + 1))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Name_Type)
      and then RFLX.TLS_Handshake.Server_Name.Field_First (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name_Type) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Name_Type) = Ctx.Last - Ctx.First + 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Name_Type =>
             RFLX.TLS_Handshake.Valid_Name_Type (Val),
          when F_Length =>
             RFLX.TLS_Handshake.Valid_Name_Length (Val),
          when F_Name =>
             True);

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Name_Type | F_Length | F_Name =>
             True);
   ...
   function Get_Name_Type (Ctx : Context) return RFLX.TLS_Handshake.Name_Type is
     (To_Actual (Ctx.Cursors (F_Name_Type).Value));

   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Name_Length is
   ...
   function Field_Size_Name_Type (Struct : Structure) return RFLX_Types.Bit_Length is
     (8);

   function Field_Size_Length (Struct : Structure) return RFLX_Types.Bit_Length is
     (16);

   function Field_Size_Name (Struct : Structure) return RFLX_Types.Bit_Length is
     (RFLX_Types.Bit_Length (Struct.Length) * 8);

end RFLX.TLS_Handshake.Server_Name;
