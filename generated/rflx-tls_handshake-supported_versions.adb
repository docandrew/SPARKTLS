...
package body RFLX.TLS_Handshake.Supported_Versions
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Length =>
             Invalid (Ctx.Cursors (F_Versions)),
          when F_Versions =>
             True);
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Supported_Versions.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.Supported_Versions.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Versions then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Supported_Versions.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Supported_Versions.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Supported_Versions.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Supported_Versions.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Supported_Versions.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.Supported_Versions.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Supported_Versions.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Supported_Versions.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Supported_Versions.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Supported_Versions.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Supported_Versions.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Supported_Versions.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Supported_Versions.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Supported_Versions_Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Length;

   procedure Set_Versions (Ctx : in out Context; Seq_Ctx : RFLX.TLS_Handshake.Protocol_Versions.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (RFLX.TLS_Handshake.Protocol_Versions.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Versions, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      RFLX.TLS_Handshake.Protocol_Versions.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Versions;

   procedure Initialize_Versions_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Supported_Versions.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Supported_Versions.Valid_Next (Ctx, RFLX.TLS_Handshake.Supported_Versions.F_Versions)
       and then RFLX.TLS_Handshake.Supported_Versions.Valid_Length (Ctx, RFLX.TLS_Handshake.Supported_Versions.F_Versions, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.Supported_Versions.Available_Space (Ctx, RFLX.TLS_Handshake.Supported_Versions.F_Versions)) >= Length,
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Versions)
       and then Field_Size (Ctx, F_Versions) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Versions)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Versions);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Versions) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Versions);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Versions) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Versions_Private;

   procedure Initialize_Versions (Ctx : in out Context) is
   begin
      Initialize_Versions_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Versions)));
   end Initialize_Versions;

   procedure Switch_To_Versions (Ctx : in out Context; Seq_Ctx : out RFLX.TLS_Handshake.Protocol_Versions.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Versions);
      Last : constant RFLX_Types.Bit_Index := Field_Last (Ctx, F_Versions);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      if Invalid (Ctx, F_Versions) then
         Reset_Dependent_Fields (Ctx, F_Versions);
         pragma Warnings (Off, "attribute Update is an obsolescent feature");
      ...
      RFLX.TLS_Handshake.Protocol_Versions.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Versions;

   procedure Update_Versions (Ctx : in out Context; Seq_Ctx : in out RFLX.TLS_Handshake.Protocol_Versions.Context) is
      Valid_Sequence : constant Boolean := RFLX.TLS_Handshake.Supported_Versions.Complete_Versions (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      RFLX.TLS_Handshake.Protocol_Versions.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
      if Valid_Sequence then
         Ctx.Cursors (F_Versions) := (State => S_Valid, First => Ctx.Cursors (F_Versions).First, Last => Ctx.Cursors (F_Versions).Last, Value => Ctx.Cursors (F_Versions).Value);
      else
         Reset_Dependent_Fields (Ctx, F_Versions);
         Ctx.Cursors (F_Versions) := (others => <>);
      end if;
   end Update_Versions;
