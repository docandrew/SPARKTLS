...
package RFLX.TLS_Record.TLS_Record_Header
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Content_Type, F_Legacy_Record_Version, F_Length, F_Final);

   subtype Field is Virtual_Field range F_Content_Type .. F_Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Record.TLS_Record_Header.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Record.TLS_Record_Header.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Record.TLS_Record_Header.Has_Buffer (Ctx)
       and then RFLX.TLS_Record.TLS_Record_Header.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Record.TLS_Record_Header.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Record.TLS_Record_Header.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Record.TLS_Record_Header.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Record.TLS_Record_Header.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Record.TLS_Record_Header.Has_Buffer (Ctx)
       and then RFLX.TLS_Record.TLS_Record_Header.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Record.TLS_Record_Header.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Record.TLS_Record_Header.Has_Buffer (Ctx)
       and then RFLX.TLS_Record.TLS_Record_Header.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Record.TLS_Record_Header.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Record.TLS_Record_Header.Valid_Next (Ctx, Fld);
   ...
   function Get_Content_Type (Ctx : Context) return RFLX.TLS_Record.Record_Content_Type_Any
   with
     Pre =>
       RFLX.TLS_Record.TLS_Record_Header.Valid (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Content_Type);

   function Get_Legacy_Record_Version (Ctx : Context) return RFLX.TLS_Record.Record_Version_Any
   with
     Pre =>
       RFLX.TLS_Record.TLS_Record_Header.Valid (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Legacy_Record_Version);

   function Get_Length (Ctx : Context) return RFLX.TLS_Record.Record_Length_Any
   with
     Pre =>
       RFLX.TLS_Record.TLS_Record_Header.Valid (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Length);
   ...
   procedure Set_Content_Type (Ctx : in out Context; Val : RFLX.TLS_Record.Record_Content_Type_Any)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Record.TLS_Record_Header.Has_Buffer (Ctx)
       and then RFLX.TLS_Record.TLS_Record_Header.Valid_Next (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Content_Type)
       and then RFLX.TLS_Record.Valid_Record_Content_Type_Any (RFLX.TLS_Record.To_Base_Integer (Val))
       and then RFLX.TLS_Record.TLS_Record_Header.Available_Space (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Content_Type) >= RFLX.TLS_Record.TLS_Record_Header.Field_Size (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Content_Type)
       and then RFLX.TLS_Record.TLS_Record_Header.Field_Condition (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Content_Type),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Content_Type)
       and Get_Content_Type (Ctx) = Val
       ...
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Content_Type) = Valid_Next (Ctx, F_Content_Type)'Old
       and Field_First (Ctx, F_Content_Type) = Field_First (Ctx, F_Content_Type)'Old;

   procedure Set_Legacy_Record_Version (Ctx : in out Context; Val : RFLX.TLS_Record.Record_Version_Any)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Record.TLS_Record_Header.Has_Buffer (Ctx)
       and then RFLX.TLS_Record.TLS_Record_Header.Valid_Next (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Legacy_Record_Version)
       and then RFLX.TLS_Record.Valid_Record_Version_Any (RFLX.TLS_Record.To_Base_Integer (Val))
       and then RFLX.TLS_Record.TLS_Record_Header.Available_Space (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Legacy_Record_Version) >= RFLX.TLS_Record.TLS_Record_Header.Field_Size (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Legacy_Record_Version)
       and then RFLX.TLS_Record.TLS_Record_Header.Field_Condition (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Legacy_Record_Version),
     Post =>
       Has_Buffer (Ctx)
       ...
       and Get_Content_Type (Ctx) = Get_Content_Type (Ctx)'Old
       and Field_First (Ctx, F_Legacy_Record_Version) = Field_First (Ctx, F_Legacy_Record_Version)'Old
       and (for all F in Field range F_Content_Type .. F_Content_Type =>
               Context_Cursors_Index (Context_Cursors (Ctx), F) = Context_Cursors_Index (Context_Cursors (Ctx)'Old, F));

   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Record.Record_Length_Any)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Record.TLS_Record_Header.Has_Buffer (Ctx)
       and then RFLX.TLS_Record.TLS_Record_Header.Valid_Next (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Length)
       and then RFLX.TLS_Record.Valid_Record_Length_Any (RFLX.TLS_Record.To_Base_Integer (Val))
       and then RFLX.TLS_Record.TLS_Record_Header.Available_Space (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Length) >= RFLX.TLS_Record.TLS_Record_Header.Field_Size (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Length)
       and then RFLX.TLS_Record.TLS_Record_Header.Field_Condition (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Length),
     Post =>
       Has_Buffer (Ctx)
       ...
       and (for all F in Field range F_Content_Type .. F_Legacy_Record_Version =>
               Context_Cursors_Index (Context_Cursors (Ctx), F) = Context_Cursors_Index (Context_Cursors (Ctx)'Old, F));
   ...
   type Structure is
      record
         Content_Type : RFLX.TLS_Record.Record_Content_Type_Any;
         Legacy_Record_Version : RFLX.TLS_Record.Record_Version_Any;
         Length : RFLX.TLS_Record.Record_Length_Any;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Record.TLS_Record_Header.Has_Buffer (Ctx)
       and then RFLX.TLS_Record.TLS_Record_Header.Valid_Structure (Struct)
       and then RFLX.TLS_Record.TLS_Record_Header.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Content_Type (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);

   function Field_Size_Legacy_Record_Version (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Content_Type)) then True)
      and then (if Well_Formed (Cursors (F_Legacy_Record_Version)) then Valid (Cursors (F_Content_Type)))
      and then (if Well_Formed (Cursors (F_Length)) then Valid (Cursors (F_Legacy_Record_Version))))
   with
     Pre =>
   ...
   function Field_Size_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Length'Base is
     (case Fld is
          when F_Content_Type =>
             8,
          when F_Legacy_Record_Version | F_Length =>
             16)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
   ...
   function Field_First_Content_Type (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Content_Type);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Content_Type =>
             Field_First_Content_Type (Cursors, First, Verified_Last, Written_Last),
          when F_Legacy_Record_Version =>
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
      and then ((if
                    Well_Formed (Cursors (F_Content_Type))
                 then
                    (Cursors (F_Content_Type).Last - Cursors (F_Content_Type).First + 1 = 8
                     and then Cursors (F_Content_Type).First = First))
                and then (if
                          ...
                          then
                             (Cursors (F_Legacy_Record_Version).Last - Cursors (F_Legacy_Record_Version).First + 1 = 16
                              and then Cursors (F_Legacy_Record_Version).First = Cursors (F_Content_Type).Last + 1))
                and then (if
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Content_Type)
      and then RFLX.TLS_Record.TLS_Record_Header.Field_First (Ctx, RFLX.TLS_Record.TLS_Record_Header.F_Content_Type) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Content_Type) = Ctx.Last - Ctx.First + 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Content_Type =>
             RFLX.TLS_Record.Valid_Record_Content_Type_Any (Val),
          when F_Legacy_Record_Version =>
             RFLX.TLS_Record.Valid_Record_Version_Any (Val),
          when F_Length =>
             RFLX.TLS_Record.Valid_Record_Length_Any (Val));

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Content_Type | F_Legacy_Record_Version | F_Length =>
             True);
   ...
   function Get_Content_Type (Ctx : Context) return RFLX.TLS_Record.Record_Content_Type_Any is
     (To_Actual (Ctx.Cursors (F_Content_Type).Value));

   function Get_Legacy_Record_Version (Ctx : Context) return RFLX.TLS_Record.Record_Version_Any is
     (To_Actual (Ctx.Cursors (F_Legacy_Record_Version).Value));

   function Get_Length (Ctx : Context) return RFLX.TLS_Record.Record_Length_Any is
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Unused_Struct : Structure) return Boolean is
     (RFLX_Types.Base_Integer (RFLX_Types.To_Last_Bit_Index (Ctx.Buffer_Last) - RFLX_Types.To_First_Bit_Index (Ctx.Buffer_First) + 1) >= 40);

   function Field_Size_Content_Type (Struct : Structure) return RFLX_Types.Bit_Length is
     (8);

   function Field_Size_Legacy_Record_Version (Struct : Structure) return RFLX_Types.Bit_Length is
     (16);

   function Field_Size_Length (Struct : Structure) return RFLX_Types.Bit_Length is
     (16);

end RFLX.TLS_Record.TLS_Record_Header;
