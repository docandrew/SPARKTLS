...
package RFLX.TLS_Handshake.PSK_Identity
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Length, F_Identity, F_Obfuscated_Ticket_Age, F_Final);

   subtype Field is Virtual_Field range F_Length .. F_Obfuscated_Ticket_Age;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.PSK_Identity.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.PSK_Identity.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.PSK_Identity.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.PSK_Identity.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Identity.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Identity =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.PSK_Identity.Valid_Next (Ctx, Fld);
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Identity_Length
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Identity.Valid (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Length);

   function Get_Obfuscated_Ticket_Age (Ctx : Context) return RFLX.TLS_Handshake.Obfuscated_Ticket_Age
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Identity.Valid (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Obfuscated_Ticket_Age);

   pragma Warnings (On, "precondition is always False");

   function Get_Identity (Ctx : Context) return RFLX_Types.Bytes
   with
     Ghost,
     Pre =>
       RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Well_Formed (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity),
     Post =>
       Get_Identity'Result'Length = RFLX_Types.To_Length (Field_Size (Ctx, F_Identity));

   procedure Get_Identity (Ctx : Context; Data : out RFLX_Types.Bytes)
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Well_Formed (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity)
       and then Data'Length = RFLX_Types.To_Length (RFLX.TLS_Handshake.PSK_Identity.Field_Size (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity)),
     Post =>
       Equal (Ctx, F_Identity, Data);

   generic
      with procedure Process_Identity (Identity : RFLX_Types.Bytes);
   procedure Generic_Get_Identity (Ctx : Context)
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.PSK_Identity.Present (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity);
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Identity_Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Length)
       and then RFLX.TLS_Handshake.Valid_Identity_Length (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.PSK_Identity.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Length) >= RFLX.TLS_Handshake.PSK_Identity.Field_Size (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Length)
       and then RFLX.TLS_Handshake.PSK_Identity.Field_Condition (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Length),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Length)
       and Get_Length (Ctx) = Val
       and Invalid (Ctx, F_Identity)
       and Invalid (Ctx, F_Obfuscated_Ticket_Age)
       and Valid_Next (Ctx, F_Identity)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   procedure Set_Obfuscated_Ticket_Age (Ctx : in out Context; Val : RFLX.TLS_Handshake.Obfuscated_Ticket_Age)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Obfuscated_Ticket_Age)
       and then RFLX.TLS_Handshake.Valid_Obfuscated_Ticket_Age (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.PSK_Identity.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Obfuscated_Ticket_Age) >= RFLX.TLS_Handshake.PSK_Identity.Field_Size (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Obfuscated_Ticket_Age)
       and then RFLX.TLS_Handshake.PSK_Identity.Field_Condition (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Obfuscated_Ticket_Age),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Obfuscated_Ticket_Age)
       and Get_Obfuscated_Ticket_Age (Ctx) = Val
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Obfuscated_Ticket_Age))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Obfuscated_Ticket_Age) = Valid_Next (Ctx, F_Obfuscated_Ticket_Age)'Old
       and Get_Length (Ctx) = Get_Length (Ctx)'Old
       and Field_First (Ctx, F_Obfuscated_Ticket_Age) = Field_First (Ctx, F_Obfuscated_Ticket_Age)'Old
       and (for all F in Field range F_Length .. F_Identity =>
   ...
   procedure Initialize_Identity (Ctx : in out Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity)
       and then RFLX.TLS_Handshake.PSK_Identity.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity) >= RFLX.TLS_Handshake.PSK_Identity.Field_Size (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity),
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Identity)
       and then Invalid (Ctx, F_Obfuscated_Ticket_Age)
       and then Valid_Next (Ctx, F_Obfuscated_Ticket_Age)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
       and then Valid_Next (Ctx, F_Identity) = Valid_Next (Ctx, F_Identity)'Old
       and then Get_Length (Ctx) = Get_Length (Ctx)'Old
       and then Field_First (Ctx, F_Identity) = Field_First (Ctx, F_Identity)'Old;

   procedure Set_Identity (Ctx : in out Context; Data : RFLX_Types.Bytes)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity)
       and then RFLX.TLS_Handshake.PSK_Identity.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity) >= RFLX.TLS_Handshake.PSK_Identity.Field_Size (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Length (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity, Data'Length)
       and then RFLX.TLS_Handshake.PSK_Identity.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.TLS_Handshake.PSK_Identity.Field_Condition (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Identity)
       and Invalid (Ctx, F_Obfuscated_Ticket_Age)
       and Valid_Next (Ctx, F_Obfuscated_Ticket_Age)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Identity) = Valid_Next (Ctx, F_Identity)'Old
       and Get_Length (Ctx) = Get_Length (Ctx)'Old
       and Field_First (Ctx, F_Identity) = Field_First (Ctx, F_Identity)'Old
       and Equal (Ctx, F_Identity, Data);

   generic
      with procedure Process_Identity (Identity : out RFLX_Types.Bytes);
      with function Process_Data_Pre (Length : RFLX_Types.Length) return Boolean;
   procedure Generic_Set_Identity (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Next (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity)
       and then RFLX.TLS_Handshake.PSK_Identity.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity) >= RFLX.TLS_Handshake.PSK_Identity.Field_Size (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Length (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.PSK_Identity.Available_Space (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Identity)) >= Length
   ...
   type Structure is
      record
         Length : RFLX.TLS_Handshake.Identity_Length;
         Identity : RFLX_Types.Bytes (RFLX_Types.Index'First .. RFLX_Types.Index'First + 65534);
         Obfuscated_Ticket_Age : RFLX.TLS_Handshake.Obfuscated_Ticket_Age;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Identity.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Identity.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.PSK_Identity.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Identity (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);

   function Field_Size_Obfuscated_Ticket_Age (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Length)) then True)
      and then (if Well_Formed (Cursors (F_Identity)) then Valid (Cursors (F_Length)))
      and then (if Well_Formed (Cursors (F_Obfuscated_Ticket_Age)) then Well_Formed (Cursors (F_Identity))))
   with
     Pre =>
     ...
     (case Fld is
          ...
          when F_Identity =>
             (Valid (Cursors (F_Length))
              and then True),
          when F_Obfuscated_Ticket_Age =>
             (Well_Formed (Cursors (F_Identity))
              and then True))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
   ...
   function Field_First_Identity (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First + 16)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Identity);

   function Field_First_Obfuscated_Ticket_Age (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (Field_First_Identity (Cursors, First, Verified_Last, Written_Last) + Field_Size_Internal (Cursors, First, Verified_Last, Written_Last, F_Identity))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Obfuscated_Ticket_Age);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          ...
          when F_Identity =>
             Field_First_Identity (Cursors, First, Verified_Last, Written_Last),
          when F_Obfuscated_Ticket_Age =>
             Field_First_Obfuscated_Ticket_Age (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
     ...
     ((if
      ...
      and then ((if
                ...
                and then (if
                             Well_Formed (Cursors (F_Identity))
                          then
                             (Cursors (F_Identity).Last - Cursors (F_Identity).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Length).Value) * 8
                              and then Cursors (F_Identity).First = Cursors (F_Length).Last + 1))
                and then (if
                             Well_Formed (Cursors (F_Obfuscated_Ticket_Age))
                          then
                             (Cursors (F_Obfuscated_Ticket_Age).Last - Cursors (F_Obfuscated_Ticket_Age).First + 1 = 32
                              and then Cursors (F_Obfuscated_Ticket_Age).First = Cursors (F_Identity).Last + 1))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.TLS_Handshake.PSK_Identity.Field_First (Ctx, RFLX.TLS_Handshake.PSK_Identity.F_Length) rem RFLX_Types.Byte'Size = 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Length =>
             RFLX.TLS_Handshake.Valid_Identity_Length (Val),
          when F_Identity =>
             True,
          when F_Obfuscated_Ticket_Age =>
             RFLX.TLS_Handshake.Valid_Obfuscated_Ticket_Age (Val));

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Length | F_Identity | F_Obfuscated_Ticket_Age =>
             True);
   ...
   function Well_Formed_Message (Ctx : Context) return Boolean is
     (Valid (Ctx, F_Obfuscated_Ticket_Age));
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Identity_Length is
     (To_Actual (Ctx.Cursors (F_Length).Value));

   function Get_Obfuscated_Ticket_Age (Ctx : Context) return RFLX.TLS_Handshake.Obfuscated_Ticket_Age is
     (To_Actual (Ctx.Cursors (F_Obfuscated_Ticket_Age).Value));
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Struct : Structure) return Boolean is
     (RFLX_Types.Base_Integer (RFLX_Types.To_Last_Bit_Index (Ctx.Buffer_Last) - RFLX_Types.To_First_Bit_Index (Ctx.Buffer_First) + 1) >= RFLX_Types.Base_Integer (Struct.Length) * 8 + 48);

   function Field_Size_Length (Struct : Structure) return RFLX_Types.Bit_Length is
     (16);

   function Field_Size_Identity (Struct : Structure) return RFLX_Types.Bit_Length is
     (RFLX_Types.Bit_Length (Struct.Length) * 8);

   function Field_Size_Obfuscated_Ticket_Age (Struct : Structure) return RFLX_Types.Bit_Length is
     (32);

end RFLX.TLS_Handshake.PSK_Identity;
