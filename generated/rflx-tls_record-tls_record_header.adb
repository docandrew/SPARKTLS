...
package body RFLX.TLS_Record.TLS_Record_Header
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.TLS_Record.TLS_Record_Header.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Record.TLS_Record_Header.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Record.TLS_Record_Header.Has_Buffer (Ctx)
       and then RFLX.TLS_Record.TLS_Record_Header.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Record.TLS_Record_Header.Valid_Value (Fld, Val)
       and then RFLX.TLS_Record.TLS_Record_Header.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Record.TLS_Record_Header.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Record.TLS_Record_Header.Valid_Next (Ctx, Fld)
       and then Fld in F_Content_Type | F_Legacy_Record_Version | F_Length
       and then RFLX.TLS_Record.TLS_Record_Header.Valid_Value (Fld, Val)
       and then RFLX.TLS_Record.TLS_Record_Header.Valid_Size (Ctx, Fld, RFLX.TLS_Record.TLS_Record_Header.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Record.TLS_Record_Header.Available_Space (Ctx, Fld) >= RFLX.TLS_Record.TLS_Record_Header.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Record.TLS_Record_Header.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Record.TLS_Record_Header.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Content_Type (Ctx : in out Context; Val : RFLX.TLS_Record.Record_Content_Type_Any) is
   begin
      Set_Scalar (Ctx, F_Content_Type, RFLX.TLS_Record.To_Base_Integer (Val));
   end Set_Content_Type;

   procedure Set_Legacy_Record_Version (Ctx : in out Context; Val : RFLX.TLS_Record.Record_Version_Any) is
   begin
      Set_Scalar (Ctx, F_Legacy_Record_Version, RFLX.TLS_Record.To_Base_Integer (Val));
   end Set_Legacy_Record_Version;

   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Record.Record_Length_Any) is
   begin
   ...
   begin
      Struct.Content_Type := Get_Content_Type (Ctx);
      Struct.Legacy_Record_Version := Get_Legacy_Record_Version (Ctx);
   ...
   begin
      Reset (Ctx);
      Set_Content_Type (Ctx, Struct.Content_Type);
      Set_Legacy_Record_Version (Ctx, Struct.Legacy_Record_Version);
