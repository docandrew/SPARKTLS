...
package RFLX.TLS_Handshake.Encrypted_Extensions
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Length, F_Extensions, F_Final);

   subtype Field is Virtual_Field range F_Length .. F_Extensions;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Encrypted_Extensions.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Encrypted_Extensions.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Encrypted_Extensions.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Extensions =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Next (Ctx, Fld);
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Encrypted_Extensions_Length
   with
     Pre =>
       RFLX.TLS_Handshake.Encrypted_Extensions.Valid (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Length);
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Encrypted_Extensions_Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Next (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Length)
       and then RFLX.TLS_Handshake.Valid_Encrypted_Extensions_Length (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Available_Space (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Length) >= RFLX.TLS_Handshake.Encrypted_Extensions.Field_Size (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Length)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Field_Condition (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Length),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Next (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Available_Space (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions) >= RFLX.TLS_Handshake.Encrypted_Extensions.Field_Size (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Field_Condition (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Field_Size (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions) = 0,
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Set_Extensions (Ctx : in out Context; Seq_Ctx : RFLX.TLS_Handshake.EE_Extensions.Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx)
       ...
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Available_Space (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions) >= RFLX.TLS_Handshake.Encrypted_Extensions.Field_Size (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Field_Condition (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Length (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions, RFLX.TLS_Handshake.EE_Extensions.Byte_Size (Seq_Ctx))
       and then RFLX.TLS_Handshake.EE_Extensions.Has_Buffer (Seq_Ctx)
       and then RFLX.TLS_Handshake.EE_Extensions.Valid (Seq_Ctx),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Extensions)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Extensions))
   ...
   procedure Switch_To_Extensions (Ctx : in out Context; Seq_Ctx : out RFLX.TLS_Handshake.EE_Extensions.Context)
   with
     Pre =>
       not Ctx'Constrained
       ...
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Next (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Field_Size (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions) > 0
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Field_First (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions) rem RFLX_Types.Byte'Size = 1
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Available_Space (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions) >= RFLX.TLS_Handshake.Encrypted_Extensions.Field_Size (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions)
       and then RFLX.TLS_Handshake.Encrypted_Extensions.Field_Condition (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions),
     Post =>
       not RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.EE_Extensions.Has_Buffer (Seq_Ctx)
       ...
       and RFLX.TLS_Handshake.EE_Extensions.Valid (Seq_Ctx)
       and RFLX.TLS_Handshake.EE_Extensions.Sequence_Last (Seq_Ctx) = Seq_Ctx.First - 1
   ...
   function Complete_Extensions (Ctx : Context; Seq_Ctx : RFLX.TLS_Handshake.EE_Extensions.Context) return Boolean
   with
     Pre =>
       RFLX.TLS_Handshake.Encrypted_Extensions.Valid_Next (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions);

   procedure Update_Extensions (Ctx : in out Context; Seq_Ctx : in out RFLX.TLS_Handshake.EE_Extensions.Context)
   with
     Pre =>
       RFLX.TLS_Handshake.Encrypted_Extensions.Present (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Extensions)
       and then not RFLX.TLS_Handshake.Encrypted_Extensions.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.EE_Extensions.Has_Buffer (Seq_Ctx)
     ...
     Post =>
       (if
           RFLX.TLS_Handshake.Encrypted_Extensions.Complete_Extensions (Ctx, Seq_Ctx)
        then
           Present (Ctx, F_Extensions)
        else
           Invalid (Ctx, F_Extensions))
       and Has_Buffer (Ctx)
       and not RFLX.TLS_Handshake.EE_Extensions.Has_Buffer (Seq_Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       ...
       and RFLX.TLS_Handshake.EE_Extensions.Valid (Seq_Ctx) = RFLX.TLS_Handshake.EE_Extensions.Valid (Seq_Ctx)'Old
       and RFLX.TLS_Handshake.EE_Extensions.Size (Seq_Ctx) = RFLX.TLS_Handshake.EE_Extensions.Size (Seq_Ctx)'Old
       and Field_First (Ctx, F_Extensions) = Field_First (Ctx, F_Extensions)'Old
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Length)) then True)
      and then (if Well_Formed (Cursors (F_Extensions)) then Valid (Cursors (F_Length))))
   with
     Pre =>
     ...
     ((if
      ...
      and then ((if
                ...
                and then (if
                          ...
                          then
                             (Cursors (F_Extensions).Last - Cursors (F_Extensions).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Length).Value) * 8
                              and then Cursors (F_Extensions).First = Cursors (F_Length).Last + 1))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.TLS_Handshake.Encrypted_Extensions.Field_First (Ctx, RFLX.TLS_Handshake.Encrypted_Extensions.F_Length) rem RFLX_Types.Byte'Size = 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Length =>
             RFLX.TLS_Handshake.Valid_Encrypted_Extensions_Length (Val),
          when F_Extensions =>
             True);

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Length | F_Extensions =>
             True);
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Encrypted_Extensions_Length is
   ...
   function Complete_Extensions (Ctx : Context; Seq_Ctx : RFLX.TLS_Handshake.EE_Extensions.Context) return Boolean is
     (RFLX.TLS_Handshake.EE_Extensions.Valid (Seq_Ctx)
      and RFLX.TLS_Handshake.EE_Extensions.Size (Seq_Ctx) = Field_Size (Ctx, F_Extensions));
...
end RFLX.TLS_Handshake.Encrypted_Extensions;
