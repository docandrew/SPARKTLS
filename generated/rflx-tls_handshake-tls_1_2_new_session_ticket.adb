...
package body RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Ticket then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Valid_Next (Ctx, Fld)
       and then Fld in F_Ticket_Lifetime_Hint | F_Ticket_Length
       and then RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Ticket_Lifetime_Hint (Ctx : in out Context; Val : RFLX.TLS_Handshake.Ticket_Lifetime) is
   begin
      Set_Scalar (Ctx, F_Ticket_Lifetime_Hint, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Ticket_Lifetime_Hint;

   procedure Set_Ticket_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.TLS_1_2_NST_Ticket_Length) is
   begin
      Set_Scalar (Ctx, F_Ticket_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Ticket_Length;

   procedure Set_Ticket_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Ticket, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Ticket_Empty;
   ...
   begin
      ...
      pragma Assert (RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.Valid_Length (Ctx, RFLX.TLS_Handshake.TLS_1_2_New_Session_Ticket.F_Ticket, Length));
      Initialize_Ticket_Private (Ctx, Length);
   end Generic_Set_Ticket;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Ticket_Lifetime_Hint := Get_Ticket_Lifetime_Hint (Ctx);
      Struct.Ticket_Length := Get_Ticket_Length (Ctx);
      Struct.Ticket := (others => 0);
      Get_Ticket (Ctx, Struct.Ticket (Struct.Ticket'First .. Struct.Ticket'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_Ticket)) + 1) - 2));
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Ticket_Lifetime_Hint (Ctx, Struct.Ticket_Lifetime_Hint);
      Set_Ticket_Length (Ctx, Struct.Ticket_Length);
      Set_Ticket (Ctx, Struct.Ticket (Struct.Ticket'First .. Struct.Ticket'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Ticket_Length) * 8) + 1) - 2));
   end To_Context;
