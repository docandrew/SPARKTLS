...
package RFLX.TLS_Handshake.Psk_Key_Exchange_Modes
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Length, F_Modes, F_Final);

   subtype Field is Virtual_Field range F_Length .. F_Modes;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Modes =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Valid_Next (Ctx, Fld);
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.PSK_Key_Exchange_Modes_Length
   with
     Pre =>
       RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Valid (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Length);
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.PSK_Key_Exchange_Modes_Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Valid_Next (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Length)
       and then RFLX.TLS_Handshake.Valid_PSK_Key_Exchange_Modes_Length (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Available_Space (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Length) >= RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Field_Size (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Length)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Field_Condition (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Length),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Length)
       and Get_Length (Ctx) = Val
       and Invalid (Ctx, F_Modes)
       and Valid_Next (Ctx, F_Modes)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   procedure Set_Modes (Ctx : in out Context; Seq_Ctx : RFLX.TLS_Handshake.Key_Exchange_Modes.Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Valid_Next (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Available_Space (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes) >= RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Field_Size (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Field_Condition (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Valid_Length (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes, RFLX.TLS_Handshake.Key_Exchange_Modes.Byte_Size (Seq_Ctx))
       and then RFLX.TLS_Handshake.Key_Exchange_Modes.Has_Buffer (Seq_Ctx)
       and then RFLX.TLS_Handshake.Key_Exchange_Modes.Valid (Seq_Ctx),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Modes)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Modes))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Modes) = Valid_Next (Ctx, F_Modes)'Old
       and Get_Length (Ctx) = Get_Length (Ctx)'Old
       and Field_First (Ctx, F_Modes) = Field_First (Ctx, F_Modes)'Old
       and (if Field_Size (Ctx, F_Modes) > 0 then Present (Ctx, F_Modes));

   procedure Initialize_Modes (Ctx : in out Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx)
     ...
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Modes)
       and then (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Modes))
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
       and then Valid_Next (Ctx, F_Modes) = Valid_Next (Ctx, F_Modes)'Old
       and then Get_Length (Ctx) = Get_Length (Ctx)'Old
       and then Field_First (Ctx, F_Modes) = Field_First (Ctx, F_Modes)'Old;

   procedure Switch_To_Modes (Ctx : in out Context; Seq_Ctx : out RFLX.TLS_Handshake.Key_Exchange_Modes.Context)
   with
     Pre =>
       not Ctx'Constrained
       ...
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Valid_Next (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Field_Size (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes) > 0
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Field_First (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes) rem RFLX_Types.Byte'Size = 1
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Available_Space (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes) >= RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Field_Size (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes)
       and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Field_Condition (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes),
     Post =>
       not RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Key_Exchange_Modes.Has_Buffer (Seq_Ctx)
       and Ctx.Buffer_First = Seq_Ctx.Buffer_First
       and Ctx.Buffer_Last = Seq_Ctx.Buffer_Last
       and Seq_Ctx.First = Field_First (Ctx, F_Modes)
       and Seq_Ctx.Last = Field_Last (Ctx, F_Modes)
       and RFLX.TLS_Handshake.Key_Exchange_Modes.Valid (Seq_Ctx)
       and RFLX.TLS_Handshake.Key_Exchange_Modes.Sequence_Last (Seq_Ctx) = Seq_Ctx.First - 1
       and Present (Ctx, F_Modes)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Field_Last (Ctx, F_Modes) = Field_Last (Ctx, F_Modes)'Old
     ...
     Contract_Cases =>
       (Well_Formed (Ctx, F_Modes) =>
           True,
        others =>
           True);

   function Complete_Modes (Ctx : Context; Seq_Ctx : RFLX.TLS_Handshake.Key_Exchange_Modes.Context) return Boolean
   with
     Pre =>
       RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Valid_Next (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes);

   procedure Update_Modes (Ctx : in out Context; Seq_Ctx : in out RFLX.TLS_Handshake.Key_Exchange_Modes.Context)
   with
     Pre =>
       RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Present (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Modes)
       and then not RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Exchange_Modes.Has_Buffer (Seq_Ctx)
       and then Ctx.Buffer_First = Seq_Ctx.Buffer_First
       and then Ctx.Buffer_Last = Seq_Ctx.Buffer_Last
       and then Seq_Ctx.First = Field_First (Ctx, F_Modes)
       and then Seq_Ctx.Last = Field_Last (Ctx, F_Modes),
     Post =>
       (if
           RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Complete_Modes (Ctx, Seq_Ctx)
        then
           Present (Ctx, F_Modes)
        else
           Invalid (Ctx, F_Modes))
       and Has_Buffer (Ctx)
       and not RFLX.TLS_Handshake.Key_Exchange_Modes.Has_Buffer (Seq_Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       ...
       and RFLX.TLS_Handshake.Key_Exchange_Modes.Valid (Seq_Ctx) = RFLX.TLS_Handshake.Key_Exchange_Modes.Valid (Seq_Ctx)'Old
       and RFLX.TLS_Handshake.Key_Exchange_Modes.Size (Seq_Ctx) = RFLX.TLS_Handshake.Key_Exchange_Modes.Size (Seq_Ctx)'Old
       and Field_First (Ctx, F_Modes) = Field_First (Ctx, F_Modes)'Old
       and Field_Size (Ctx, F_Modes) = Field_Size (Ctx, F_Modes)'Old
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Length)) then True)
      and then (if Well_Formed (Cursors (F_Modes)) then Valid (Cursors (F_Length))))
   with
     Pre =>
   ...
   function Field_First_Modes (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First + 8)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Modes);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          ...
          when F_Modes =>
             Field_First_Modes (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then ((if
                ...
                and then (if
                             Well_Formed (Cursors (F_Modes))
                          then
                             (Cursors (F_Modes).Last - Cursors (F_Modes).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Length).Value) * 8
                              and then Cursors (F_Modes).First = Cursors (F_Length).Last + 1))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.Field_First (Ctx, RFLX.TLS_Handshake.Psk_Key_Exchange_Modes.F_Length) rem RFLX_Types.Byte'Size = 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Length =>
             RFLX.TLS_Handshake.Valid_PSK_Key_Exchange_Modes_Length (Val),
          when F_Modes =>
             True);

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Length | F_Modes =>
             True);
   ...
   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.PSK_Key_Exchange_Modes_Length is
   ...
   function Complete_Modes (Ctx : Context; Seq_Ctx : RFLX.TLS_Handshake.Key_Exchange_Modes.Context) return Boolean is
     (RFLX.TLS_Handshake.Key_Exchange_Modes.Valid (Seq_Ctx)
      and RFLX.TLS_Handshake.Key_Exchange_Modes.Size (Seq_Ctx) = Field_Size (Ctx, F_Modes));
...
end RFLX.TLS_Handshake.Psk_Key_Exchange_Modes;
