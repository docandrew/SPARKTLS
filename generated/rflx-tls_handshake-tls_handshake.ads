...
package RFLX.TLS_Handshake.TLS_Handshake
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Tag, F_Length, F_Payload, F_Final);
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.TLS_Handshake.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.TLS_Handshake.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.TLS_Handshake.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.TLS_Handshake.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.TLS_Handshake.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.TLS_Handshake.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.TLS_Handshake.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.TLS_Handshake.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.TLS_Handshake.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.TLS_Handshake.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.TLS_Handshake.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.TLS_Handshake.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.TLS_Handshake.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.TLS_Handshake.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Payload =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.TLS_Handshake.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.TLS_Handshake.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.TLS_Handshake.Valid (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Tag);

   function Get_Length (Ctx : Context) return RFLX.TLS_Handshake.Length
   with
     Pre =>
       RFLX.TLS_Handshake.TLS_Handshake.Valid (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Length);
   ...
   with
     ...
     Pre =>
       ...
       and then RFLX.TLS_Handshake.TLS_Handshake.Valid_Next (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload),
     Post =>
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.TLS_Handshake.Well_Formed (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload)
       and then RFLX.TLS_Handshake.TLS_Handshake.Valid_Next (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload)
       and then Data'Length = RFLX_Types.To_Length (RFLX.TLS_Handshake.TLS_Handshake.Field_Size (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload)),
     Post =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.TLS_Handshake.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.TLS_Handshake.Present (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload);
   ...
   with
     ...
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.TLS_Handshake.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.TLS_Handshake.Valid_Next (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Tag)
       and then RFLX.Tls_Parameters.Valid_TLS_HandshakeType (RFLX.Tls_Parameters.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.TLS_Handshake.Available_Space (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Tag) >= RFLX.TLS_Handshake.TLS_Handshake.Field_Size (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Tag)
       and then RFLX.TLS_Handshake.TLS_Handshake.Field_Condition (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Tag),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Tag)
   ...
   with
     ...
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.TLS_Handshake.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.TLS_Handshake.Valid_Next (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Length)
       and then RFLX.TLS_Handshake.Valid_Length (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.TLS_Handshake.Available_Space (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Length) >= RFLX.TLS_Handshake.TLS_Handshake.Field_Size (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Length)
       and then RFLX.TLS_Handshake.TLS_Handshake.Field_Condition (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Length),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.TLS_Handshake.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.TLS_Handshake.Valid_Next (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload)
       and then RFLX.TLS_Handshake.TLS_Handshake.Available_Space (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload) >= RFLX.TLS_Handshake.TLS_Handshake.Field_Size (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload)
       and then RFLX.TLS_Handshake.TLS_Handshake.Field_Condition (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload)
       and then RFLX.TLS_Handshake.TLS_Handshake.Field_Size (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload) = 0,
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.TLS_Handshake.Valid_Next (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload)
       and then RFLX.TLS_Handshake.TLS_Handshake.Available_Space (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload) >= RFLX.TLS_Handshake.TLS_Handshake.Field_Size (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload)
       and then RFLX.TLS_Handshake.TLS_Handshake.Valid_Length (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload, Data'Length)
       and then RFLX.TLS_Handshake.TLS_Handshake.Available_Space (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.TLS_Handshake.TLS_Handshake.Field_Condition (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Payload)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.TLS_Handshake.Valid_Next (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload)
       and then RFLX.TLS_Handshake.TLS_Handshake.Available_Space (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload) >= RFLX.TLS_Handshake.TLS_Handshake.Field_Size (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload)
       and then RFLX.TLS_Handshake.TLS_Handshake.Valid_Length (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.TLS_Handshake.Available_Space (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Payload)) >= Length
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.TLS_Handshake.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.TLS_Handshake.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.TLS_Handshake.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.TLS_Handshake.TLS_Handshake.Field_First (Ctx, RFLX.TLS_Handshake.TLS_Handshake.F_Tag) rem RFLX_Types.Byte'Size = 1
...
end RFLX.TLS_Handshake.TLS_Handshake;
