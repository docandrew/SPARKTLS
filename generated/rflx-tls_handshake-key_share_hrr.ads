...
package RFLX.TLS_Handshake.Key_Share_HRR
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Selected_Group, F_Final);

   subtype Field is Virtual_Field range F_Selected_Group .. F_Selected_Group;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Key_Share_HRR.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_HRR.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_HRR.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_HRR.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Key_Share_HRR.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Key_Share_HRR.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_HRR.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Key_Share_HRR.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_HRR.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_HRR.Valid_Next (Ctx, Fld);
   ...
   function Get_Selected_Group (Ctx : Context) return RFLX.Tls_Parameters.TLS_Supported_Groups
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_HRR.Valid (Ctx, RFLX.TLS_Handshake.Key_Share_HRR.F_Selected_Group);
   ...
   procedure Set_Selected_Group (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Supported_Groups_Enum)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_HRR.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Valid_Next (Ctx, RFLX.TLS_Handshake.Key_Share_HRR.F_Selected_Group)
       and then RFLX.Tls_Parameters.Valid_TLS_Supported_Groups (RFLX.Tls_Parameters.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Key_Share_HRR.Available_Space (Ctx, RFLX.TLS_Handshake.Key_Share_HRR.F_Selected_Group) >= RFLX.TLS_Handshake.Key_Share_HRR.Field_Size (Ctx, RFLX.TLS_Handshake.Key_Share_HRR.F_Selected_Group)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Field_Condition (Ctx, RFLX.TLS_Handshake.Key_Share_HRR.F_Selected_Group),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Selected_Group)
       and Get_Selected_Group (Ctx) = (True, Val)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Selected_Group))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Selected_Group) = Valid_Next (Ctx, F_Selected_Group)'Old
       and Field_First (Ctx, F_Selected_Group) = Field_First (Ctx, F_Selected_Group)'Old;
   ...
   type Structure is
      record
         Selected_Group : RFLX.Tls_Parameters.TLS_Supported_Groups;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_HRR.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.Key_Share_HRR.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Selected_Group (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Selected_Group)) then True))
   with
     Pre =>
   ...
   function Field_First_Selected_Group (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Selected_Group);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Selected_Group =>
             Field_First_Selected_Group (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
      and then ((if
                    Well_Formed (Cursors (F_Selected_Group))
                 then
                    (Cursors (F_Selected_Group).Last - Cursors (F_Selected_Group).First + 1 = 16
                     and then Cursors (F_Selected_Group).First = First))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Selected_Group)
      and then RFLX.TLS_Handshake.Key_Share_HRR.Field_First (Ctx, RFLX.TLS_Handshake.Key_Share_HRR.F_Selected_Group) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Selected_Group) = Ctx.Last - Ctx.First + 1
      and then Invalid (Ctx, F_Selected_Group));
   ...
   function Get_Selected_Group (Ctx : Context) return RFLX.Tls_Parameters.TLS_Supported_Groups is
     (To_Actual (Ctx.Cursors (F_Selected_Group).Value));
   ...
   function Valid_Structure (Struct : Structure) return Boolean is
     (RFLX.Tls_Parameters.Valid_TLS_Supported_Groups (Struct.Selected_Group));
   ...
   function Field_Size_Selected_Group (Struct : Structure) return RFLX_Types.Bit_Length is
     (16);

end RFLX.TLS_Handshake.Key_Share_HRR;
