...
package body RFLX.TLS_Handshake.Key_Share_Entry
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          ...
          when F_Length =>
             Invalid (Ctx.Cursors (F_Key_Exchange)),
          when F_Key_Exchange =>
             True);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_Entry.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Key_Share_Entry.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_Entry.Valid_Next (Ctx, Fld),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   function Composite_Field (Fld : Field) return Boolean is
     (Fld in F_Key_Exchange);

   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.Key_Share_Entry.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Key_Exchange then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   function Get_Key_Exchange (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Key_Exchange).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Key_Exchange).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Key_Exchange;

   procedure Get_Key_Exchange (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Key_Exchange).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Key_Exchange).Last);
   begin
   ...
   procedure Generic_Get_Key_Exchange (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Key_Exchange).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Key_Exchange).Last);
   begin
      Process_Key_Exchange (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Key_Exchange;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Key_Share_Entry.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.Key_Share_Entry.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Valid_Next (Ctx, Fld)
       and then Fld in F_Group | F_Length
       and then RFLX.TLS_Handshake.Key_Share_Entry.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Key_Share_Entry.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Key_Share_Entry.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Key_Share_Entry.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Key_Share_Entry.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Group (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Supported_Groups_Enum) is
   begin
      Set_Scalar (Ctx, F_Group, RFLX.Tls_Parameters.To_Base_Integer (Val));
   end Set_Group;

   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Key_Exchange_Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Length;

   procedure Set_Group (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_Supported_Groups)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Valid_Next (Ctx, RFLX.TLS_Handshake.Key_Share_Entry.F_Group)
       and then RFLX.Tls_Parameters.Valid_TLS_Supported_Groups (Val)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Available_Space (Ctx, RFLX.TLS_Handshake.Key_Share_Entry.F_Group) >= RFLX.TLS_Handshake.Key_Share_Entry.Field_Size (Ctx, RFLX.TLS_Handshake.Key_Share_Entry.F_Group)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Field_Condition (Ctx, RFLX.TLS_Handshake.Key_Share_Entry.F_Group),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Initialize_Key_Exchange_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Valid_Next (Ctx, RFLX.TLS_Handshake.Key_Share_Entry.F_Key_Exchange)
       and then RFLX.TLS_Handshake.Key_Share_Entry.Valid_Length (Ctx, RFLX.TLS_Handshake.Key_Share_Entry.F_Key_Exchange, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.Key_Share_Entry.Available_Space (Ctx, RFLX.TLS_Handshake.Key_Share_Entry.F_Key_Exchange)) >= Length,
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Key_Exchange)
       and then Field_Size (Ctx, F_Key_Exchange) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Key_Exchange)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Key_Exchange);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Key_Exchange) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Key_Exchange);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Key_Exchange) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Key_Exchange_Private;

   procedure Initialize_Key_Exchange (Ctx : in out Context) is
   begin
      Initialize_Key_Exchange_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Key_Exchange)));
   end Initialize_Key_Exchange;

   procedure Set_Key_Exchange (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Key_Exchange));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Key_Exchange_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Key_Exchange)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Key_Exchange)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Key_Exchange))) = Data);
   end Set_Key_Exchange;

   procedure Generic_Set_Key_Exchange (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Key_Exchange));
   begin
      if Length > 0 then
         Process_Key_Exchange (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.TLS_Handshake.Key_Share_Entry.Valid_Length (Ctx, RFLX.TLS_Handshake.Key_Share_Entry.F_Key_Exchange, Length));
      Initialize_Key_Exchange_Private (Ctx, Length);
   end Generic_Set_Key_Exchange;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Group := Get_Group (Ctx);
      Struct.Length := Get_Length (Ctx);
      Struct.Key_Exchange := (others => 0);
      Get_Key_Exchange (Ctx, Struct.Key_Exchange (Struct.Key_Exchange'First .. Struct.Key_Exchange'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_Key_Exchange)) + 1) - 2));
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Group (Ctx, Struct.Group);
      Set_Length (Ctx, Struct.Length);
      Set_Key_Exchange (Ctx, Struct.Key_Exchange (Struct.Key_Exchange'First .. Struct.Key_Exchange'First + RFLX_Types.Index (RFLX_Types.To_Length (8 * RFLX_Types.Bit_Length (Struct.Length)) + 1) - 2));
   end To_Context;

end RFLX.TLS_Handshake.Key_Share_Entry;
