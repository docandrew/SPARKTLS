...
package RFLX.TLS_Handshake.Certificate_Verify
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Algorithm, F_Signature_Length, F_Signature, F_Final);

   subtype Field is Virtual_Field range F_Algorithm .. F_Signature;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Certificate_Verify.Well_Formed_Message (Ctx)
       and then RFLX.TLS_Handshake.Certificate_Verify.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx)
       and then Offset < RFLX.TLS_Handshake.Certificate_Verify.Buffer_Length (Ctx)
       and then Pre (RFLX.TLS_Handshake.Certificate_Verify.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Certificate_Verify.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.TLS_Handshake.Certificate_Verify.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Certificate_Verify.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Certificate_Verify.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Signature =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Certificate_Verify.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Certificate_Verify.Valid (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Algorithm);

   function Get_Signature_Length (Ctx : Context) return RFLX.TLS_Handshake.Signature_Length
   with
     Pre =>
       RFLX.TLS_Handshake.Certificate_Verify.Valid (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature_Length);
   ...
   with
     ...
     Pre =>
       RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Certificate_Verify.Well_Formed (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Next (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature),
     Post =>
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Certificate_Verify.Well_Formed (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Next (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature)
       and then Data'Length = RFLX_Types.To_Length (RFLX.TLS_Handshake.Certificate_Verify.Field_Size (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature)),
     Post =>
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx)
       and RFLX.TLS_Handshake.Certificate_Verify.Present (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature);
   ...
   with
     ...
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Next (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Algorithm)
       and then RFLX.Tls_Parameters.Valid_TLS_SignatureScheme (RFLX.Tls_Parameters.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Certificate_Verify.Available_Space (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Algorithm) >= RFLX.TLS_Handshake.Certificate_Verify.Field_Size (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Algorithm)
       and then RFLX.TLS_Handshake.Certificate_Verify.Field_Condition (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Algorithm),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Set_Signature_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Signature_Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Next (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature_Length)
       and then RFLX.TLS_Handshake.Valid_Signature_Length (RFLX.TLS_Handshake.To_Base_Integer (Val))
       and then RFLX.TLS_Handshake.Certificate_Verify.Available_Space (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature_Length) >= RFLX.TLS_Handshake.Certificate_Verify.Field_Size (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature_Length)
       and then RFLX.TLS_Handshake.Certificate_Verify.Field_Condition (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature_Length),
     Post =>
       Has_Buffer (Ctx)
       ...
       and (for all F in Field range F_Algorithm .. F_Algorithm =>
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Next (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature)
       and then RFLX.TLS_Handshake.Certificate_Verify.Available_Space (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature) >= RFLX.TLS_Handshake.Certificate_Verify.Field_Size (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature)
       and then RFLX.TLS_Handshake.Certificate_Verify.Field_Condition (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature)
       and then RFLX.TLS_Handshake.Certificate_Verify.Field_Size (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature) = 0,
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Next (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature)
       and then RFLX.TLS_Handshake.Certificate_Verify.Available_Space (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature) >= RFLX.TLS_Handshake.Certificate_Verify.Field_Size (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Length (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature, Data'Length)
       and then RFLX.TLS_Handshake.Certificate_Verify.Available_Space (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.TLS_Handshake.Certificate_Verify.Field_Condition (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Signature)
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Next (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature)
       and then RFLX.TLS_Handshake.Certificate_Verify.Available_Space (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature) >= RFLX.TLS_Handshake.Certificate_Verify.Field_Size (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Length (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.Certificate_Verify.Available_Space (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature)) >= Length
   ...
   type Structure is
      record
         ...
         Signature_Length : RFLX.TLS_Handshake.Signature_Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Structure (Struct)
       and then RFLX.TLS_Handshake.Certificate_Verify.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Algorithm)) then True)
      ...
      and then ((if
                 ...
                 then
                    (Cursors (F_Algorithm).Last - Cursors (F_Algorithm).First + 1 = 16
                     and then Cursors (F_Algorithm).First = First))
                and then (if
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.TLS_Handshake.Certificate_Verify.Field_First (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Algorithm) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Algorithm) = Ctx.Last - Ctx.First + 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          ...
          when F_Signature_Length =>
             RFLX.TLS_Handshake.Valid_Signature_Length (Val),
          when F_Signature =>
             True);

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Algorithm | F_Signature_Length | F_Signature =>
             True);
   ...
   function Get_Signature_Length (Ctx : Context) return RFLX.TLS_Handshake.Signature_Length is
   ...
   function Valid_Structure (Struct : Structure) return Boolean is
     (RFLX.Tls_Parameters.Valid_TLS_SignatureScheme (Struct.Algorithm));

   function Sufficient_Buffer_Length (Ctx : Context; Struct : Structure) return Boolean is
     (RFLX_Types.Base_Integer (RFLX_Types.To_Last_Bit_Index (Ctx.Buffer_Last) - RFLX_Types.To_First_Bit_Index (Ctx.Buffer_First) + 1) >= RFLX_Types.Base_Integer (Struct.Signature_Length) * 8 + 32);
...
end RFLX.TLS_Handshake.Certificate_Verify;
