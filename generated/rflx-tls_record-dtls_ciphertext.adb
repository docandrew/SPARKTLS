...
package body RFLX.TLS_Record.DTLS_Ciphertext
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Prefix =>
             Invalid (Ctx.Cursors (F_CID_Present)),
          when F_CID_Present =>
             Invalid (Ctx.Cursors (F_Seq_Size_16)),
          when F_Seq_Size_16 =>
             Invalid (Ctx.Cursors (F_Length_Present)),
          when F_Length_Present =>
             Invalid (Ctx.Cursors (F_Epoch_Low)),
          when F_Epoch_Low =>
             Invalid (Ctx.Cursors (F_CID))
             and Invalid (Ctx.Cursors (F_Sequence_Number_16))
             and Invalid (Ctx.Cursors (F_Sequence_Number_8)),
          when F_CID =>
             Invalid (Ctx.Cursors (F_Sequence_Number_16))
             and Invalid (Ctx.Cursors (F_Sequence_Number_8)),
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Record.DTLS_Ciphertext.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Record.DTLS_Ciphertext.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Encrypted_Record then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       RFLX.TLS_Record.DTLS_Ciphertext.Has_Buffer (Ctx)
       and then RFLX.TLS_Record.DTLS_Ciphertext.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Record.DTLS_Ciphertext.Valid_Value (Fld, Val)
       and then RFLX.TLS_Record.DTLS_Ciphertext.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Record.DTLS_Ciphertext.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Record.DTLS_Ciphertext.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Record.DTLS_Ciphertext.Valid_Next (Ctx, Fld)
       and then Fld in F_Prefix | F_CID_Present | F_Seq_Size_16 | F_Length_Present | F_Epoch_Low | F_CID | F_Sequence_Number_16 | F_Sequence_Number_8 | F_Length
       and then RFLX.TLS_Record.DTLS_Ciphertext.Valid_Value (Fld, Val)
       and then RFLX.TLS_Record.DTLS_Ciphertext.Valid_Size (Ctx, Fld, RFLX.TLS_Record.DTLS_Ciphertext.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Record.DTLS_Ciphertext.Available_Space (Ctx, Fld) >= RFLX.TLS_Record.DTLS_Ciphertext.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Record.DTLS_Ciphertext.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Record.DTLS_Ciphertext.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Prefix (Ctx : in out Context; Val : RFLX.TLS_Record.Ciphertext_Prefix) is
   begin
      Set_Scalar (Ctx, F_Prefix, RFLX.TLS_Record.To_Base_Integer (Val));
   end Set_Prefix;

   procedure Set_CID_Present (Ctx : in out Context; Val : Boolean) is
   begin
      Set_Scalar (Ctx, F_CID_Present, To_Base_Integer (Val));
   end Set_CID_Present;

   procedure Set_Seq_Size_16 (Ctx : in out Context; Val : Boolean) is
   begin
      Set_Scalar (Ctx, F_Seq_Size_16, To_Base_Integer (Val));
   end Set_Seq_Size_16;

   procedure Set_Length_Present (Ctx : in out Context; Val : Boolean) is
   begin
      Set_Scalar (Ctx, F_Length_Present, To_Base_Integer (Val));
   end Set_Length_Present;

   procedure Set_Epoch_Low (Ctx : in out Context; Val : RFLX.TLS_Record.Epoch_Low) is
   begin
      Set_Scalar (Ctx, F_Epoch_Low, RFLX.TLS_Record.To_Base_Integer (Val));
   end Set_Epoch_Low;

   procedure Set_CID (Ctx : in out Context; Val : RFLX.TLS_Record.CID) is
   begin
      Set_Scalar (Ctx, F_CID, RFLX.TLS_Record.To_Base_Integer (Val));
   end Set_CID;

   procedure Set_Sequence_Number_16 (Ctx : in out Context; Val : RFLX.TLS_Record.Ciphertext_Sequence_Number_16) is
   begin
      Set_Scalar (Ctx, F_Sequence_Number_16, RFLX.TLS_Record.To_Base_Integer (Val));
   end Set_Sequence_Number_16;

   procedure Set_Sequence_Number_8 (Ctx : in out Context; Val : RFLX.TLS_Record.Ciphertext_Sequence_Number_8) is
   begin
      Set_Scalar (Ctx, F_Sequence_Number_8, RFLX.TLS_Record.To_Base_Integer (Val));
   end Set_Sequence_Number_8;

   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Record.Ciphertext_Length) is
   begin
   ...
   procedure Initialize_Encrypted_Record (Ctx : in out Context; Length : RFLX_Types.Length) is
   begin
   ...
   begin
      ...
      pragma Assert (RFLX.TLS_Record.DTLS_Ciphertext.Valid_Length (Ctx, RFLX.TLS_Record.DTLS_Ciphertext.F_Encrypted_Record, Length));
      Initialize_Encrypted_Record_Private (Ctx, Length);
