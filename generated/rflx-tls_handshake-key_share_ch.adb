...
package body RFLX.TLS_Handshake.Key_Share_CH
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Key_Share_CH.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.Key_Share_CH.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Shares then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_CH.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Share_CH.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Key_Share_CH.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Key_Share_CH.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.Key_Share_CH.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Key_Share_CH.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Key_Share_CH.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Key_Share_CH.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Key_Share_CH.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Key_Share_CH.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Key_Share_CH.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Key_Share_CH.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Key_Shares_Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Length;

   procedure Set_Shares_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Shares, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Shares_Empty;

   procedure Set_Shares (Ctx : in out Context; Seq_Ctx : RFLX.TLS_Handshake.Key_Share_Entries.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (RFLX.TLS_Handshake.Key_Share_Entries.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Shares, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      RFLX.TLS_Handshake.Key_Share_Entries.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Shares;

   procedure Initialize_Shares_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Key_Share_CH.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Key_Share_CH.Valid_Next (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares)
       and then RFLX.TLS_Handshake.Key_Share_CH.Valid_Length (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares, Length)
       and then RFLX_Types.To_Length (RFLX.TLS_Handshake.Key_Share_CH.Available_Space (Ctx, RFLX.TLS_Handshake.Key_Share_CH.F_Shares)) >= Length,
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Shares)
       and then Field_Size (Ctx, F_Shares) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Shares)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Shares);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Shares) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Shares);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Shares) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Shares_Private;

   procedure Initialize_Shares (Ctx : in out Context) is
   begin
      Initialize_Shares_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Shares)));
   end Initialize_Shares;

   procedure Switch_To_Shares (Ctx : in out Context; Seq_Ctx : out RFLX.TLS_Handshake.Key_Share_Entries.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Shares);
      Last : constant RFLX_Types.Bit_Index := Field_Last (Ctx, F_Shares);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      if Invalid (Ctx, F_Shares) then
         Reset_Dependent_Fields (Ctx, F_Shares);
         pragma Warnings (Off, "attribute Update is an obsolescent feature");
      ...
      RFLX.TLS_Handshake.Key_Share_Entries.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Shares;

   procedure Update_Shares (Ctx : in out Context; Seq_Ctx : in out RFLX.TLS_Handshake.Key_Share_Entries.Context) is
      Valid_Sequence : constant Boolean := RFLX.TLS_Handshake.Key_Share_CH.Complete_Shares (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      RFLX.TLS_Handshake.Key_Share_Entries.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
      if Valid_Sequence then
         Ctx.Cursors (F_Shares) := (State => S_Valid, First => Ctx.Cursors (F_Shares).First, Last => Ctx.Cursors (F_Shares).Last, Value => Ctx.Cursors (F_Shares).Value);
      else
         Reset_Dependent_Fields (Ctx, F_Shares);
         Ctx.Cursors (F_Shares) := (others => <>);
      end if;
   end Update_Shares;
