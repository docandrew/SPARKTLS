...
package body RFLX.TLS_Handshake.Certificate_Verify
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Certificate_Verify.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.Certificate_Verify.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Certificate_Verify.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Certificate_Verify.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.Certificate_Verify.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Next (Ctx, Fld)
       and then Fld in F_Algorithm | F_Signature_Length
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Certificate_Verify.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Certificate_Verify.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Certificate_Verify.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Certificate_Verify.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Certificate_Verify.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Certificate_Verify.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Signature_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Signature_Length) is
   begin
   ...
   begin
      ...
      pragma Assert (RFLX.TLS_Handshake.Certificate_Verify.Valid_Length (Ctx, RFLX.TLS_Handshake.Certificate_Verify.F_Signature, Length));
