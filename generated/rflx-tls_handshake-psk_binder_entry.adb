...
package body RFLX.TLS_Handshake.PSK_Binder_Entry
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Length =>
             Invalid (Ctx.Cursors (F_PSK_Binder_Entry)),
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.PSK_Binder_Entry.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_PSK_Binder_Entry then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   function Get_PSK_Binder_Entry (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_PSK_Binder_Entry).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_PSK_Binder_Entry).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_PSK_Binder_Entry;

   procedure Get_PSK_Binder_Entry (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_PSK_Binder_Entry).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_PSK_Binder_Entry).Last);
   begin
   ...
   procedure Generic_Get_PSK_Binder_Entry (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_PSK_Binder_Entry).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_PSK_Binder_Entry).Last);
   begin
      Process_PSK_Binder_Entry (Ctx.Buffer.all (First .. Last));
   end Generic_Get_PSK_Binder_Entry;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.PSK_Binder_Entry.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.PSK_Binder_Entry.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.PSK_Binder_Entry.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.PSK_Binder_Entry.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.PSK_Binder_Entry.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.PSK_Binder_Entry_Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Length;

   procedure Initialize_PSK_Binder_Entry_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.PSK_Binder_Entry.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_PSK_Binder_Entry) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_PSK_Binder_Entry)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_PSK_Binder_Entry);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_PSK_Binder_Entry) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_PSK_Binder_Entry);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_PSK_Binder_Entry) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_PSK_Binder_Entry_Private;

   procedure Initialize_PSK_Binder_Entry (Ctx : in out Context) is
   begin
      Initialize_PSK_Binder_Entry_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_PSK_Binder_Entry)));
   end Initialize_PSK_Binder_Entry;

   procedure Set_PSK_Binder_Entry (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_PSK_Binder_Entry));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_PSK_Binder_Entry_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_PSK_Binder_Entry)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_PSK_Binder_Entry)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_PSK_Binder_Entry))) = Data);
   end Set_PSK_Binder_Entry;

   procedure Generic_Set_PSK_Binder_Entry (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_PSK_Binder_Entry));
   begin
      if Length > 0 then
         Process_PSK_Binder_Entry (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.TLS_Handshake.PSK_Binder_Entry.Valid_Length (Ctx, RFLX.TLS_Handshake.PSK_Binder_Entry.F_PSK_Binder_Entry, Length));
      Initialize_PSK_Binder_Entry_Private (Ctx, Length);
   end Generic_Set_PSK_Binder_Entry;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Length := Get_Length (Ctx);
      Struct.PSK_Binder_Entry := (others => 0);
      Get_PSK_Binder_Entry (Ctx, Struct.PSK_Binder_Entry (Struct.PSK_Binder_Entry'First .. Struct.PSK_Binder_Entry'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_PSK_Binder_Entry)) + 1) - 2));
   end To_Structure;
   ...
   begin
      ...
      Set_PSK_Binder_Entry (Ctx, Struct.PSK_Binder_Entry (Struct.PSK_Binder_Entry'First .. Struct.PSK_Binder_Entry'First + RFLX_Types.Index (RFLX_Types.To_Length (8 * RFLX_Types.Bit_Length (Struct.Length)) + 1) - 2));
   end To_Context;
