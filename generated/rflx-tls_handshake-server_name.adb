...
package body RFLX.TLS_Handshake.Server_Name
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Server_Name.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.Server_Name.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Name then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   function Get_Name (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Name).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Name).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Name;

   procedure Get_Name (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Name).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Name).Last);
   begin
   ...
   procedure Generic_Get_Name (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Name).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Name).Last);
   begin
      Process_Name (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Name;
   ...
   with
     Pre =>
       RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Next (Ctx, Fld)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.Server_Name.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.Server_Name.Composite_Field (Fld)
                 then
     ...
     Post =>
       ...
       and then (Ctx.Cursors (Fld).Value = Val
                 and then (if Fld in F_Name and then Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, Fld)))
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.Server_Name.Valid_Next (Ctx, Fld)
       and then Fld in F_Name_Type | F_Length
       and then RFLX.TLS_Handshake.Server_Name.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.Server_Name.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.Server_Name.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.Server_Name.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.Server_Name.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.Server_Name.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.Server_Name.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Name_Type (Ctx : in out Context; Val : RFLX.TLS_Handshake.Name_Type) is
   begin
      Set_Scalar (Ctx, F_Name_Type, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Name_Type;

   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Name_Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Length;

   procedure Initialize_Name_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.Server_Name.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_Name) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Name)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Name);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Name) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      ...
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Name) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Name_Private;

   procedure Initialize_Name (Ctx : in out Context) is
   begin
      Initialize_Name_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Name)));
   end Initialize_Name;

   procedure Set_Name (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Name));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Name_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Name)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Name)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Name))) = Data);
   end Set_Name;

   procedure Generic_Set_Name (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Name));
   begin
      if Length > 0 then
         Process_Name (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.TLS_Handshake.Server_Name.Valid_Length (Ctx, RFLX.TLS_Handshake.Server_Name.F_Name, Length));
      Initialize_Name_Private (Ctx, Length);
   end Generic_Set_Name;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Name_Type := Get_Name_Type (Ctx);
      Struct.Length := Get_Length (Ctx);
      Struct.Name := (others => 0);
      Get_Name (Ctx, Struct.Name (Struct.Name'First .. Struct.Name'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_Name)) + 1) - 2));
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Name_Type (Ctx, Struct.Name_Type);
      Set_Length (Ctx, Struct.Length);
      Set_Name (Ctx, Struct.Name (Struct.Name'First .. Struct.Name'First + RFLX_Types.Index (RFLX_Types.To_Length (8 * RFLX_Types.Bit_Length (Struct.Length)) + 1) - 2));
   end To_Context;
