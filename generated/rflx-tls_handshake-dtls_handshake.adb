...
package body RFLX.TLS_Handshake.DTLS_Handshake
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          ...
          when F_Length =>
             Invalid (Ctx.Cursors (F_Message_Seq)),
          when F_Message_Seq =>
             Invalid (Ctx.Cursors (F_Fragment_Offset)),
          when F_Fragment_Offset =>
             Invalid (Ctx.Cursors (F_Fragment_Length)),
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.DTLS_Handshake.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLS_Handshake.DTLS_Handshake.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.DTLS_Handshake.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLS_Handshake.DTLS_Handshake.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.TLS_Handshake.DTLS_Handshake.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.TLS_Handshake.DTLS_Handshake.Valid_Next (Ctx, Fld)
       and then Fld in F_Tag | F_Length | F_Message_Seq | F_Fragment_Offset | F_Fragment_Length
       and then RFLX.TLS_Handshake.DTLS_Handshake.Valid_Value (Fld, Val)
       and then RFLX.TLS_Handshake.DTLS_Handshake.Valid_Size (Ctx, Fld, RFLX.TLS_Handshake.DTLS_Handshake.Field_Size (Ctx, Fld))
       and then RFLX.TLS_Handshake.DTLS_Handshake.Available_Space (Ctx, Fld) >= RFLX.TLS_Handshake.DTLS_Handshake.Field_Size (Ctx, Fld)
       and then RFLX.TLS_Handshake.DTLS_Handshake.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLS_Handshake.DTLS_Handshake.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Tag (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_HandshakeType_Enum) is
   begin
      Set_Scalar (Ctx, F_Tag, RFLX.Tls_Parameters.To_Base_Integer (Val));
   end Set_Tag;

   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Length;

   procedure Set_Message_Seq (Ctx : in out Context; Val : RFLX.TLS_Handshake.Message_Seq) is
   begin
      Set_Scalar (Ctx, F_Message_Seq, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Message_Seq;

   procedure Set_Fragment_Offset (Ctx : in out Context; Val : RFLX.TLS_Handshake.Length) is
   begin
      Set_Scalar (Ctx, F_Fragment_Offset, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Fragment_Offset;

   procedure Set_Fragment_Length (Ctx : in out Context; Val : RFLX.TLS_Handshake.Length) is
   begin
      Set_Scalar (Ctx, F_Fragment_Length, RFLX.TLS_Handshake.To_Base_Integer (Val));
   end Set_Fragment_Length;

   procedure Set_Tag (Ctx : in out Context; Val : RFLX.Tls_Parameters.TLS_HandshakeType)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLS_Handshake.DTLS_Handshake.Has_Buffer (Ctx)
       and then RFLX.TLS_Handshake.DTLS_Handshake.Valid_Next (Ctx, RFLX.TLS_Handshake.DTLS_Handshake.F_Tag)
       and then RFLX.Tls_Parameters.Valid_TLS_HandshakeType (Val)
   ...
   begin
      ...
      pragma Assert (RFLX.TLS_Handshake.DTLS_Handshake.Valid_Length (Ctx, RFLX.TLS_Handshake.DTLS_Handshake.F_Payload, Length));
   ...
   begin
      ...
      Struct.Message_Seq := Get_Message_Seq (Ctx);
      Struct.Fragment_Offset := Get_Fragment_Offset (Ctx);
      Struct.Fragment_Length := Get_Fragment_Length (Ctx);
   ...
   begin
      ...
      Set_Message_Seq (Ctx, Struct.Message_Seq);
      Set_Fragment_Offset (Ctx, Struct.Fragment_Offset);
      Set_Fragment_Length (Ctx, Struct.Fragment_Length);
      Set_Payload (Ctx, Struct.Payload (Struct.Payload'First .. Struct.Payload'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Fragment_Length) * 8) + 1) - 2));
   end To_Context;
