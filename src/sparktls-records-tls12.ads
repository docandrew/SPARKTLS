...
is
   ...
   is (Implicit_IV'Length = Implicit_IV_Len
   ...
   with
     ...
     Post =>
       (if Keys'Old.Counter < Record_Counter'Last
        then Keys = (Keys'Old with delta Counter => Keys'Old.Counter + 1)
        else Keys = Keys'Old and not Valid)
       and (if Valid
            then
              --  Plaintext length = encrypted - nonce - tag
              Plain_Len <= Max_Record_Plaintext
              and (Plain_Len = 0 or else Plain_Len - 1 <= Plaintext'Last));
