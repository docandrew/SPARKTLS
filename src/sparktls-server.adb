...
is
   ...
   --  Ready_Config, whose membership test it anchors.
     ...
     (S  : Session;
      ...
      D  : SPARKTLS.HS_Pool.HS_Data) return Boolean is
     (
                (if S.State = Wait_Client_Hello_Retry
                 then S.HC.Version = TLS_1_3
      ...
      and then
        (if S.State in Wait_Client_Certificate | Wait_Client_Cert_Verify
         then Hash_Len (S.HC.Neg) in 32 | 48
              and then
                True
                      and then
              ...
              and then Free_Space (S.Output) >=
                Records.Record_Header_Size + 3 + Records.Tag_Size)
              and then
                (if S.State = Wait_Client_Finished and then S.HC.Version = TLS_1_2
...
;
   ...
   with Pre => S.Negotiated_Suite in Suite_AES_128_GCM_SHA256
                                               | Suite_AES_256_GCM_SHA384
                                               | Suite_CHACHA20_POLY1305_SHA256,
                        Post => S.HC.TS = S.HC.TS'Old
                                and S.HC.Server_HS.Counter = 0
                                and True;
   ...
   begin
      ...
      if not Handled then
         ...
         if S.State in Connected | Error_State | Closed then
            S.Peer_Cert_Valid :=
              SPARKTLS.HS_Pool.Slots (S.Slot).Peer_Leaf.Present;
   ...
   begin
      ...
      if S.HC.Cfg.Select_Identity /= null
         ...
         declare
            Picked : constant Selected_Identity_Access :=
              S.HC.Cfg.Select_Identity
                (S.HC.Peer_SNI.Data
                   (S.HC.Peer_SNI.Data'First ..
                    S.HC.Peer_SNI.Data'First + S.HC.Peer_SNI.Len - 1));
         begin
            if Picked /= null then
      ...
      declare
         ...
         Want_13 : constant Boolean :=
           S.HC.Version = TLS_1_3 and Policy /= TLS_1_2_Only;
         Want_12 : constant Boolean :=
           (S.HC.Version = TLS_1_2
            or (S.HC.Version = TLS_1_3 and Policy = TLS_1_2_Only))
      ...
      begin
         if Want_13 then
            if S.Negotiated_Suite in
               ...
               | Suite_CHACHA20_POLY1305_SHA256
              and then
                (not S.HC.Client_Saw_Key_Share
                 or else not S.HC.Client_Saw_Supported_Groups)
            then
               ...
               return;
                    ...
                    if S.Negotiated_Suite not in
                       ...
                       | Suite_CHACHA20_POLY1305_SHA256
                      or else
                      not (S.HC.Client_Has_X25519 or
                           S.HC.Client_Has_P256 or
                   S.HC.Client_Has_P384 or
                   S.HC.Client_Supports_X25519 or
                   S.HC.Client_Supports_P256 or
                   S.HC.Client_Supports_P384)
            then
   ...
   procedure Handle_Wait_Client_Hello
                                                                                  ...
                                                                                  (S.Role = Role_Server
                                                                                   ...
                                                                                   | Error_State);
                                                                                        pragma Assert_And_Cut
                                                                                           ...
                                                                                           and then
                                                                                           (if S.State = Wait_Client_Hello
                                                                                           then S.HC.Legacy_Session_ID_Len
                                                                                          in 0 .. 32)
                                                                                           and then S.State in
                                                                                             Wait_Client_Hello | Error_State);
                                                                                pragma Assert (S.Role = Role_Server);
      ...
      with Post => Used (D.Reasm) = 0
                   and then S.HC.Version = S.HC.Version'Old
                           and then S.HC.Legacy_Session_ID_Len =
                             S.HC.Legacy_Session_ID_Len'Old
           is
              begin
                 Reset (D.Reasm);
              end Free_CH2_Reasm;
           begin
                     ...
                     then
                        ...
                        return;
                             ...
                             then
                                declare
                                   T : constant N32 :=
                                     N32 (Msg'Length) - (3 + S.HC.PSK.Binder_Len);
                                begin
                                   SPARKTLS_Transcript.Suffix_256
                                     (S.HC.TS, Msg (Msg'First .. Msg'First + T - 1),
                                      SPARKTLSCrypto.Hashing.SHA256.Digest (S.HC.PSK.Binder_Hash_256));
                                   SPARKTLS_Transcript.Suffix_384
                                     (S.HC.TS, Msg (Msg'First .. Msg'First + T - 1),
                                      SPARKTLSCrypto.Hashing.SHA384.Digest (S.HC.PSK.Binder_Hash_384));
                     ...
                     if S.HC.Cfg.Local = null
                       or else not S.HC.Cfg.Local.Has_Identity
                       or else S.HC.Cfg.Random = null
                       or else S.HC.Cfg.Local.NaCl_Cert_Len >
                         N32 (Max_Cert_DER)
                       or else S.HC.Cfg.Local.Int_Count > Max_Pool_Size
                       or else
                         (for some I in 0 .. Max_Pool_Size - 1 =>
                            S.HC.Cfg.Local.Ints (I).DER_Len >
                              X509.N32 (Max_Cert_DER))
                       or else
                         (S.HC.Cfg.Local.Sign_Algo = Sign_RSA_PSS
                          and then S.HC.Cfg.Local.RSA_Mod_Len
                            not in 64 .. 512)
                     then
                        Send_Alert_And_Error
                        ...
                        return;
                             ...
                             declare
                                Full_Msg : constant Byte_Seq :=
                                   Byte_Seq (Message (D.Reasm));
                                Ready_To_Build : Boolean;
                             begin
                                Complete_Client_Hello_Retry
                                  (S, D, Full_Msg, False, 0,
      ...
      begin
         ...
         if S.HC.PSK.Binder_Hash_Taken and then Binders_Size > 0 then
            ...
            if PSK_Len = 48 then
               ...
               begin
                  Trunc_Hash :=
                    Key_Schedule.Digest_384 (S.HC.PSK.Binder_Hash_384);
                  ...
                  Binder_OK := Equal
                    (Expected, Bytes_48 (S.HC.PSK.Binder));
               end;
            else
               declare
                  Trunc_Hash   : Digest;
              ...
              (if Algo_OK then
                 ...
                 and then Handshake.Sig_Algo_Compatible_With_Cert
                            (S.HC.Negotiated_Sig_Algo,
                             Cfg.Local.Sign_Algo));
            if Local_Sig_Compatible
                 (S.HC.Peer_Sig_Algos (I), Cfg.Local.Sign_Algo)
            then
   ...
   begin
      ...
      if S.HC.PSK.Offered and then Cfg.Store_Session /= null
         ...
         begin
            ...
            if Need_HRR then
                       Build_Hello_Retry_Request
                         (S, D, HRR_Group, SH_Buf, SH_Len, Rec_Out);
                               if SH_Len = 0 then
      ...
      if Cfg.Require_ALPN
        and then not Handshake.Server_Msgs.Has_ALPN_Match (S.HC)
      then
      ...
      pragma Assert
        (if S.HC.Neg.Suite = Suite_AES_256_GCM_SHA384
      ...
      begin
         case S.Negotiated_Suite is
            ...
            when others =>
               ...
               begin
                  Key_Schedule.Derive_Finished_Key
                    (Fin_Key, S.HC.Server_HS_Secret (0 .. 31));
                  HMAC_SHA_256
                    (Output => Verify_32,
                     M      => TS_Hash,
                     K      => Byte_Seq (Fin_Key));
   ...
   begin
      ...
      when Suite_AES_256_GCM_SHA384 =>
         ...
         begin
            ...
            Set_Traffic_Keys (S.HC.Client_HS, S.HC.Client_HS_Secret,
                              S.Negotiated_Suite);
            Set_Traffic_Keys (S.HC.Server_HS, S.HC.Server_HS_Secret,
                              S.Negotiated_Suite);
         end;
      when others =>
         declare
         ...
         begin
            Key_Schedule.Derive_Early_Secret
              (Early, Bytes_32 (S.HC.PSK.Value (0 .. 31)));
            if S.HC.KE.Curve = 16#0018# then
         ...
         when others =>
            ...
            begin
               Key_Schedule.Derive_Master_Secret
                 (Master, Digest (S.HC.Handshake_Secret (0 .. 31)));
   ...
   begin
      ...
      if D.Peer_Leaf.Present then
         ...
         begin
            ...
            pragma Assert
              (D.Peer_Leaf.DER_Len - 1 < X509.N32'Last);

            VR := Validate_Leaf_Policy
              (Leaf     => D.Peer_Leaf.Cert,
               Leaf_DER =>
                  Cert_X
                    (0 .. D.Peer_Leaf.DER_Len - 1),
               Hostname => "",
   ...
   begin
      Result := OK;
                  ...
                  begin
                           ...
                           when others =>
                              declare
                                 Pre_Hash : constant Digest :=
                                    Transcript_Hash_256 (S.HC);
                                 Fin_Key  : OKM_Seq (0 .. 31);
                                 Expected : Digest;
                              begin
                                 Key_Schedule.Derive_Finished_Key
                                   (Fin_Key, S.HC.Client_HS_Secret (0 .. 31));
                                 HMAC_SHA_256
                                   (Output => Expected,
                                    M      => Pre_Hash,
                                    K      => Byte_Seq (Fin_Key));
                    ...
                    and then S.HC.Cfg.Lookup_Session /= null then
                                    pragma Warnings
                           ...
                           when others =>
                              ...
                              begin
                                 Key_Schedule.Derive_Resumption_Master_Secret
                                   (Res_Master,
                                    Digest (S.HC.Master_Secret (0 .. 31)),
                                    Full_Hash);
                        ...
                        then
                           declare
                              ...
                              Include_Flags : constant Boolean :=
                                 S.HC.Cfg.TLS13_Resumption_Across_Names;
      ...
      pragma Assert (Rec.Record_Len <= Available (S.Input));
            ...
            begin
               ...
               if not Dec_Valid then
                  ...
                  if S.HC.Early_Data_Offered
                    and then S.HC.Skipped_Early_Data_Records < 32
                  then
                     S.HC.Skipped_Early_Data_Records :=
                             S.HC.Skipped_Early_Data_Records + 1;
                             Result := OK;
                             return;
                  end if;
               ...
               if Used (D.Reasm) > 0 then
                  ...
                  begin
                     if not Has_Message (D.Reasm) then
                        declare
                           Take : constant HS_Msg_Len :=
                             N32'Min (N32'Min (Wanted (D.Reasm), Plain_Len),
                                      Free_Space (D.Reasm));
                        begin
                           if Take > 0 then
                     ...
                     if Message_Too_Large (D.Reasm) then
                        ...
                        return;
                             ...
                             if not Has_Message (D.Reasm)
                               and then Pos < Plain_Len
                             then
                        declare
                           Take : constant HS_Msg_Len :=
                             N32'Min (N32'Min (Wanted (D.Reasm),
                                               Plain_Len - Pos),
                                      Free_Space (D.Reasm));
                        begin
                           if Take > 0 then
