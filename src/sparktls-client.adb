...
is
   ...
   --  legacy CCS record (RFC 8446 Â§D.4 / "middlebox-compat") because
     ...
     Post =>
       (if Result = OK
        then
          ...
          and then (if S.State = Wait_Server_Hello and then S.HC.Version = TLS_1_3
                    then
                      S.Negotiated_Suite in TLS13_Suite));
   ...
   with
     Pre =>
       ...
       and then S.HC.Cfg.Random /= null
       and then S.HC.HRR_Cookie_Len <= N32 (S.HC.HRR_Cookie'Length)
       and then (if S.HC.Version = TLS_1_3
                 then S.Negotiated_Suite in TLS13_Suite);
   ...
   begin
      ...
      Handshake.Build_Finished (Client_Verify, Finished_Buf, Finished_Len);
   ...
   begin
      ...
      --  free the buffer.
      if S.HC.Version /= TLS_1_3 and then Has_Message (D.Reasm) then
   ...
   begin
      ...
      begin
         ...
         Handshake.Client_Msgs.Parse_Server_Hello
           (S.Negotiated_Suite, S.Last_Error, S.Negotiated_ALPN, S.HC, Byte_Seq (Frag), Parse_OK);
         ...
         if not Parse_OK and then S.Last_Error = No_Error then
            ...
            pragma Assert (if Parse_OK then S.HC.Version = TLS_1_2);
         end if;
   ...
   begin
      ...
      if not Handled then
         ...
         if S.HC.Version = TLS_1_2 and S.State /= Client_Hello_Sent and S.State /= Wait_Server_Hello
         then
            SPARKTLS.Client.TLS12.Advance_Handshake_12 (S, SPARKTLS.HS_Pool.Slots (S.Slot), Result);
         else
