...
is
   ...
   function Build_SG_Raw (Restrict : Boolean; Group : Unsigned_16) return Byte_Seq is
   ...
   is
      ...
      --  HC.HRR_Selected_Group = 0 â same key_share as CH1.
      Retry_KS_Single         : constant Boolean := Retry_Mode and then HC.HRR_Selected_Group /= 0;
            ...
            begin
               ...
               for I in 1 .. N_Ext loop
                  ...
                  if Exts (I).Tag = Tag_U16 then
                     ...
                     --  DuplicateCurve).
                     Last_Err := (if Is_HRR_Msg then Illegal_Parameter else Decode_Error);
                     OK := False;
                     pragma
                       Assert_And_Cut
                         ((if Random_Was_Set then HC.Cfg.Random /= null)
         ...
         for I in 1 .. N_Ext loop
            ...
            begin
               ...
               then
                  ...
                  declare
                     List_Len : constant N32 := N32 (Data (Exts (I).Offset));
                  begin
                     if List_Len = 0 or else List_Len /= Exts (I).E_Len - 1 then
                        Last_Err := Decode_Error;
                        OK := False;
                        pragma
                          Assert_And_Cut
                     ...
                     pragma Assert (Exts (I).Offset + Exts (I).E_Len - 1 <= Data'Last);
                     if not EC_Point_Formats_Acceptable
                              (Data (Exts (I).Offset + 1 .. Exts (I).Offset + Exts (I).E_Len - 1))
                     then
                        Last_Err := Decode_Error;
                        OK := False;
                        pragma
               ...
               then
                  ...
                  begin
                     ...
                     if C_Len = 0 or else 2 + C_Len /= Exts (I).E_Len then
                        Last_Err := Illegal_Parameter;
                        OK := False;
                        pragma
                          Assert_And_Cut
                     ...
                     if C_Len <= N32 (HC.HRR_Cookie'Length) then
                        ...
                        for K in 0 .. C_Len - 1 loop
                           ...
                           pragma Loop_Invariant (if Saved_Got_HRR then HC.Got_HRR);
                           pragma Loop_Invariant (HC.HRR_Cookie_Len <= N32 (HC.HRR_Cookie'Length));
                           HC.HRR_Cookie (K) := Data (Exts (I).Offset + 2 + K);
                        end loop;
                     end if;
                  end;
               end if;
   ...
   begin
      ...
      if Curr_Is_HRR
        and then (if HC.Cfg.Client_Key_Share_Group in 16#001D# | 16#0017# | 16#0018# then True
      ...
      declare
         ...
         Buf      : RBT.Bytes_Ptr;
         Ctx      : Context;
         procedure Release_SH_Buffer (Ctx : in out Context; Buf : in out RBT.Bytes_Ptr)
         with
           Pre => Has_Buffer (Ctx) and then Buf = null,
           Post => not Has_Buffer (Ctx) and then Buf = null;
         pragma Inline (Release_SH_Buffer);

         procedure Release_SH_Buffer (Ctx : in out Context; Buf : in out RBT.Bytes_Ptr) is
         begin
            Take_Buffer (Ctx, Buf);
            RFLX_Free (Buf);
