...
is
   ...
   with
     Pre => S.Version = TLS_1_2 and then S.State = Wait_Client_Hello and then S.Role = Role_Server;
   ...
   --  CertificateRequest (RFC 5246 7.4.4), fixed shape: rsa_sign +
   --  ecdsa_sign certificate types, the modern signature-algorithm list,
   ...
   --  RFC 5246 7.4.1.4.1 signature-scheme negotiation for the server
   --  flight: pick the first client-offered scheme compatible with our
   --  local key (server preference list honored when configured; SHA-256
   ...
   is
      ...
      begin
         ...
         --  2b. CertificateStatus (RFC 6066 8): only when ServerHello echoed
         --  status_request, which Build_Server_Hello_12 does only for a
         --  staple that fits one handshake fragment.
         if S.HC.T12.Server_Will_Staple then
            declare
               Status_Buf : Byte_Seq (0 .. Max_Certificate_Status_12 - 1);
               Status_Len : N32;
            begin
               Build_Certificate_Status_12 (Cfg.Local.all, Status_Buf, Status_Len);
               if Status_Len = 0 or else Status_Len > Max_Fragment then
                  Send_Alert_And_Error (S, Internal_Error, Result);
                  return;
               end if;
               Append_Transcript (S.HC, Status_Buf (0 .. Status_Len - 1));
               Records.Build_Handshake_Record (Status_Buf (0 .. Status_Len - 1), S.Output, Rec_Out);
               if Rec_Out = 0 then
                  Send_Alert_And_Error (S, Insufficient_Buffer, Result);
                  return;
               end if;
   ...
   ------------------------------------------------------------------
   --  Server Finished handshake message (RFC 5246 7.4.9) over the current
   --  transcript. FB must span 0 .. Finished_12_Total_Len - 1 or more.
   procedure Build_Server_Finished_12
     (TS      : in SPARKTLS_Transcript.Transcript_State;
      Master  : in Bytes_48;
      Use_384 : in Boolean;
      FB      : out Byte_Seq;
      FL      : out N32)
   ...
   begin
      if Use_384 then
         SPARKTLS_Transcript.Current_384 (TS, TH4);
         ...
         begin
            ...
            Handshake.TLS12.Build_Finished_12
              (Master, Label_Server_Finished, TH_Bytes, True, FB, FL);
         end;
      else
         SPARKTLS_Transcript.Current_256 (TS, TH);
         Prove_Server_Finished_Label;
         pragma Assert (Valid_Finished_Label (Label_Server_Finished));
         ...
         begin
            ...
            pragma Assert (TH_Bytes'Length = 32 or else TH_Bytes'Length = 48);
            Handshake.TLS12.Build_Finished_12
              (Master, Label_Server_Finished, TH_Bytes, False, FB, FL);
         end;
      end if;
      pragma Assert (FL = Finished_12_Total_Len);
   end Build_Server_Finished_12;

   --  Resumed-flight NewSessionTicket (RFC 5077 3.3: MUST re-issue when
   --  session_ticket was advertised in the abbreviated ServerHello). Seals
   --  the restored master secret under the caller-supplied active TEK with
   --  a fresh nonce, appends to transcript and scratch flight. OK is False
   --  when the message or record does not fit.
   ...
   with
     --  Frame for the client-flight message pump: key derivation touches
     --  neither the state machine, the negotiated suite nor the
     --  configuration.
     Post =>
       S.State = S.State'Old
       and then S.Negotiated_Suite = S.Negotiated_Suite'Old
       and then S.HC.CKE_Received_12 = S.HC.CKE_Received_12'Old
   is
      use Key_Schedule_12;
      Use_384    : constant Boolean :=
        S.Negotiated_Suite
   ...
   ------------------------------------------------------------------
   --  Mid-handshake record consumers for the CKE phase (decomposition
   --  round 1): CCS and plaintext-alert records arriving while the
   --  server waits for ClientKeyExchange.

   --  Validates the header of a complete (reassembled or single-fragment)
   --  ClientKeyExchange message and hands the body to
   --  Parse_Client_Key_Exchange.  Sets HC.Ext_Parse_Err when the failure
   --  deserves a specific alert (otherwise the caller sends decode_error).
   procedure Parse_Complete_CKE_12
     (HC : in out Handshake_Context; Msg : in Byte_Seq; CKE_Good : out Boolean)
   with
     Pre  =>
       Msg'Length > 0
       and then Msg'Last < N32 (Natural'Last)
       and then SPARKTLS.Handshake.Server_Msgs.Local_Config_Valid (HC.Cfg.Local),
     Post => SPARKTLS.Handshake.Server_Msgs.Local_Config_Valid (HC.Cfg.Local)
   is
   ...
   begin
      ...
      if not POK or else Msg_Type /= HT_Client_Key_Exchange or else 4 + Msg_Len /= N32 (Msg'Length)
      then
         return;
      end if;
   ...
   end Parse_Complete_CKE_12;

   --  Consumes the current record and fails the handshake with Err.
   procedure Consume_And_Alert_12
     (S : in out Session; Rec : in Records.Parse_Result; Err : in Error_Code; Result : out Action)
   with
     Pre  =>
   ...
   end Consume_And_Alert_12;

   --  A complete ClientKeyExchange message is in hand: parse it, extend the
   --  transcript, and draw the EMS session hash (RFC 7627) at exactly this
   --  point -- before Finished, after CKE.
   procedure Finish_CKE_12
     (S : in out Session; Rec : in Records.Parse_Result; Msg : in Byte_Seq; Result : out Action)
   with
     Pre  =>
       Msg'Length > 0
   ...
   is
      ...
      with Ghost;
   begin
      Parse_Complete_CKE_12 (S.HC, Msg, CKE_OK);
      if not CKE_OK then
         if S.HC.Ext_Parse_Err /= No_Error then
            ...
            Consume_And_Alert_12 (S, Rec, Decode_Error, Result);
            pragma Assert (S.Negotiated_Suite = Saved_Negotiated_Suite);
         end if;
         return;
      end if;
   ...
   --  RFC 5246 7.4.7: only one ClientKeyExchange permitted; any further
   --  handshake-content record after CKE is a state-machine violation.
   --  Classifies the offending message for the alert code the suites
   --  expect (TLS-Anvil secondClientKeyExchange, BoGo trailing-data):
   --  malformed header of a known type -> decode_error; malformed
   --  CertificateVerify length fields -> decode_error; everything
   ...
   end Reject_Post_CKE_Handshake_12;
   ...
   begin
      ...
      if CCS_OK then
         ...
         Result := OK;
         --  RFC 5246 7.1 single-CCS invariant: future CCS records on
         --  this connection are rejected via `not S.HC.CCS_Received`.
         pragma Assert (Single_CCS_RFC_5246_7_1 (S.HC));
      else
         --  RFC 5246 7.1: CCS payload MUST be the single byte 0x01
         --  (BoGo BadChangeCipherSpec-*).
         Send_Alert_And_Error (S, Unexpected_Message, Result);
      end if;
   end Consume_CKE_Phase_CCS_12;
   ...
   is
      ...
      --  plaintext close_notify and close. Other plaintext alerts
      --  during the handshake are protocol violations -- fatal.
      FS         : constant N32 := S.Input.Read_Pos + Rec.Fragment_Pos;
      Alert_Desc : Byte := 0;
   begin
      ...
      if Alert_Desc = 0 then
         ...
         Set_State (S, Closing);
         if Output_Pending (S) > 0 then
            pragma Assert (Close_Notify_Reply_State_RFC_5246_7_2_1 (S.State, Output_Pending (S)));
            Result := Has_Output;
         else
            Result := Shutdown;
         end if;
   ...
   --  ECDHE shared-secret computation, one procedure per group.
   --  Split from Compute_Shared_Secret_12 (decomposition round 1):
   --  each operates on KE state alone.

   procedure Shared_Secret_X25519_12 (KE : in out KE_State; OK : out Boolean; Err : out Error_Code)
   is
   begin
      KE.Shared (0 .. 31) := SPARKNaCl.Scalar.Mult (KE.Local_SK, KE.Peer_PK);
      --  RFC 7748 6.1 / RFC 8422 5.10: reject all-zeros shared secret
      --  (small-subgroup defence). The helper's Post is formally proven.
      OK := Shared_Secret_Is_Acceptable_X25519 (KE.Shared (0 .. 31));
   ...
   end Shared_Secret_P384_12;

   procedure Handle_Client_Certificate_Msg_12
     (S : in out Session; D : in out SPARKTLS.HS_Pool.HS_Data; Msg : in Byte_Seq; Result : out Action)
   with
     Pre  =>
       Msg'First = 0
       and then Msg'Length >= 4
       and then Msg'Last < N32'Last - 256
       and then S.State = Wait_Client_Certificate,
     Post =>
       S.Negotiated_Suite = S.Negotiated_Suite'Old
       and then (if Result = OK
                 then S.State in Wait_Client_Cert_Verify | Wait_Client_Finished
                 else S.State = Error_State);

   procedure Handle_Client_CertVerify_Msg_12
     (S : in out Session; D : in out SPARKTLS.HS_Pool.HS_Data; Msg : in Byte_Seq; Result : out Action)
   with
     Pre  =>
       Msg'First = 0
       ...
       and then S.HC.CKE_Received_12,
     Post =>
       S.Negotiated_Suite = S.Negotiated_Suite'Old
       and then (if Result = OK then S.State = Wait_Client_Finished else S.State = Error_State);
   ...
   --  A complete ClientKeyExchange message (reassembled, header included):
   --  parse it, extend the transcript, take the EMS session hash (RFC 7627)
   --  at exactly this point, then derive the keys. Msg-level twin of the
   --  former record-level Finish_CKE_12 path: the caller has already
   --  consumed the record, so no Read_Pos here.
   procedure Handle_Client_Key_Exchange_Msg_12
     (S : in out Session; Msg : in Byte_Seq; Result : out Action)
   with
     Pre  =>
       Msg'First = 0
       and then Msg'Length >= 4
       and then Msg'Last < N32'Last - 256
       and then S.HC.Cfg in Ready_Config
       and then not S.HC.CKE_Received_12,
     Post =>
       S.Negotiated_Suite = S.Negotiated_Suite'Old
       and then (if Result = OK then S.State = S.State'Old and then S.HC.CKE_Received_12
                 else S.State = Error_State)
   is
      CKE_OK                 : Boolean;
      Saved_Negotiated_Suite : constant Supported_Suite := S.Negotiated_Suite
   ...
   end Handle_Client_Key_Exchange_Msg_12;

   --  The handshake messages of the client's flight before CCS --
   --  Certificate, ClientKeyExchange, CertificateVerify -- read as a
   --  message STREAM (RFC 5246 6.2.1: several messages per record, a
   --  message across records; BoGo PackHandshake / SplitHandshakeRecords).
   --  The record is consumed up front; D.Reasm carries an unfinished
   --  message to the next record. Each complete message is dispatched on
   --  the current state.
   procedure Process_Client_Handshake_Record_12
     (S : in out Session; D : in out SPARKTLS.HS_Pool.HS_Data; Rec : in Records.Parse_Result; Result : out Action)
   with
     Pre =>
       Rec.OK
       ...
       and then S.Input.Read_Pos + Rec.Record_Len <= IO_Buffer_Capacity
       and then S.HC.Cfg in Ready_Config
       and then S.State in Wait_Client_Certificate | Wait_Client_Cert_Verify | Wait_Client_Finished
       and then (if S.State = Wait_Client_Finished then not S.HC.CKE_Received_12)
   is
      Frag_Len : constant N32 := Rec.Fragment_Len;
      FS       : constant N32 := S.Input.Read_Pos + Rec.Fragment_Pos;
   begin
      ...
      declare
         Frag : constant Byte_Seq (0 .. Frag_Len - 1) :=
           Byte_Seq (S.Input.Storage (Ix (FS) .. Ix (FS + Frag_Len - 1)));
         Pos  : N32 := 0;
      begin
         S.Input.Read_Pos := S.Input.Read_Pos + Rec.Record_Len;

         while Pos < Frag_Len loop
            pragma Loop_Invariant (Pos <= Frag_Len);
            pragma Loop_Invariant (Result = OK);
            pragma
              Loop_Invariant
                (S.State in Wait_Client_Certificate | Wait_Client_Cert_Verify | Wait_Client_Finished);
            pragma Loop_Invariant (if S.State = Wait_Client_Finished then not S.HC.CKE_Received_12);
            ...
            declare
               Take : constant HS_Msg_Len :=
                 N32'Min (N32'Min (Wanted (D.Reasm), Frag_Len - Pos), Free_Space (D.Reasm));
            begin
               if Take = 0 then
                  --  Nothing wanted or no room: the buffer is stuck on an
                  --  oversize message.
                  Reset (D.Reasm);
                  Send_Alert_And_Error (S, Decode_Error, Result);
                  return;
               end if;
               Append (D.Reasm, Frag (Pos .. Pos + Take - 1));
               Pos := Pos + Take;
            end;

            if Header_Ready (D.Reasm) then
               declare
                  --  The one message type the state admits.
                  Expected : constant Byte :=
                    (if S.State = Wait_Client_Certificate then HS_Msg_Wire (HT_Certificate)
                     elsif not S.HC.CKE_Received_12 then HS_Msg_Wire (HT_Client_Key_Exchange)
                     else HS_Msg_Wire (HT_Certificate_Verify));
               begin
                  if Declared_Type (D.Reasm) /= Expected then
                     Reset (D.Reasm);
                     Send_Alert_And_Error (S, Unexpected_Message, Result);
                     return;
                  end if;
                  if Expected = HS_Msg_Wire (HT_Client_Key_Exchange)
                    and then Declared_Size (D.Reasm) - 4 > Max_Client_Key_Exchange
                  then
                     Reset (D.Reasm);
                     Send_Alert_And_Error (S, Decode_Error, Result);
                     return;
            ...
            --  Only the header (or part of the body) so far: the rest is
            --  later in this record or in the next one.
            if Has_Message (D.Reasm) then
               declare
                  Full : constant Byte_Seq := Byte_Seq (Message (D.Reasm));
               begin
                  Reset (D.Reasm);
                  if S.State = Wait_Client_Certificate then
                     Handle_Client_Certificate_Msg_12 (S, D, Full, Result);
                  elsif not S.HC.CKE_Received_12 then
                     if S.HC.Cfg not in Ready_Config then
                        Send_Alert_And_Error (S, Internal_Error, Result);
                        return;
                     end if;
                     Handle_Client_Key_Exchange_Msg_12 (S, Full, Result);
                  else
                     Handle_Client_CertVerify_Msg_12 (S, D, Full, Result);
                  end if;
               end;
               if Result /= OK or else S.State = Error_State then
                  return;
               end if;

               --  RFC 5246 7.4.7 / 7.3: after ClientKeyExchange (with no
               --  CertificateVerify to come) the next thing on the wire is
               --  the ChangeCipherSpec record; more handshake bytes in this
               --  record are a state-machine violation.
               if S.State = Wait_Client_Finished and then S.HC.CKE_Received_12 then
                  if Pos < Frag_Len then
                     Send_Alert_And_Error (S, Unexpected_Message, Result);
                     return;
                  end if;
                  exit;
   ...
   end Process_Client_Handshake_Record_12;
   ...
   begin
      ...
      if Rec.Content = Records.Content_Change_Cipher_Spec then
         Consume_CKE_Phase_CCS_12 (S, Rec, Result);
         return;
      end if;

      if Rec.Content = Records.Content_Alert then
         Consume_CKE_Phase_Alert_12 (S, Rec, Result);
         return;
      end if;
      ...
      if S.HC.CKE_Received_12 then
         Reject_Post_CKE_Handshake_12 (S, Rec, Result);
         return;
      end if;

      if S.HC.Cfg not in Ready_Config
        or else S.State not in Wait_Client_Certificate | Wait_Client_Cert_Verify | Wait_Client_Finished
      then
         --  Fail closed (Init's gate makes this unreachable).
         S.Input.Read_Pos := S.Input.Read_Pos + Rec.Record_Len;
         S.Last_Error := Internal_Error;
      ...
      Process_Client_Handshake_Record_12 (S, D, Rec, Result);
   end Process_Client_Key_Exchange_12;

   --  A complete client Certificate message (RFC 5246 7.4.6), header
   --  included. Parses the chain, extends the transcript and moves the
   --  state on: CertificateVerify next when a certificate was sent, else
   --  straight to the ClientKeyExchange/Finished phase.
   procedure Handle_Client_Certificate_Msg_12
     (S : in out Session; D : in out SPARKTLS.HS_Pool.HS_Data; Msg : in Byte_Seq; Result : out Action)
   is
      Msg_Type : Maybe_HS_Msg;
   ...
   begin
      Result := OK;
      Handshake.Parse_Handshake_Header (Msg, Msg_Type, Msg_Len, Parse_OK);

      if not Parse_OK or else Msg_Type /= HT_Certificate then
         Send_Alert_And_Error (S, Unexpected_Message, Result);
         return;
      end if;

      if Msg_Len < 3 or else Msg_Len + 4 /= N32 (Msg'Length) or else Msg'Length < 7 then
         Send_Alert_And_Error (S, Decode_Error, Result);
         return;
      end if;
      ...
      declare
         List_Len : constant N32 :=
           N32 (Msg (4)) * 65536 + N32 (Msg (5)) * 256 + N32 (Msg (6));
      begin
      ...
      begin
         SPARKTLS.Handshake.Certs.Parse_Certificate_Chain_12
           (HC => S.HC, D => D, HS_Msg => Msg, OK => Chain_OK, Err => Chain_Err);
      ...
      Result := OK;
   end Handle_Client_Certificate_Msg_12;

   --  Record-level entry for the client-authentication states: any
   --  handshake record goes through the message pump; anything else is a
   ...
   ------------------------------------------------------------------
   --  CertificateVerify signature check (RFC 5246 7.4.8): the client signs
   --  the handshake-message transcript; all three hashes are drawn and the
   --  scheme picks which one Verify uses.
   function CV_Signature_Valid_12
   ...
   begin
      SPARKTLS_Transcript.Current_256 (TS, TH2);
      SPARKTLS_Transcript.Current_384 (TS, TH3);
      SPARKTLS_Transcript.Current_512 (TS, TH5);
      return
   ...
   end CV_Signature_Valid_12;

   --  Client certificate policy + chain validation for TLS 1.2 mTLS.
   ...
   --  A complete client CertificateVerify message (RFC 5246 7.4.8), header
   --  included: check the signature over the transcript so far, then
   --  validate the chain, then extend the transcript.
   procedure Handle_Client_CertVerify_Msg_12
     (S : in out Session; D : in out SPARKTLS.HS_Pool.HS_Data; Msg : in Byte_Seq; Result : out Action)
   is
      Msg_Type    : Maybe_HS_Msg;
   ...
   begin
      ...
      if not Parse_OK or else Msg_Type /= HT_Certificate_Verify then
         Send_Alert_And_Error (S, Unexpected_Message, Result);
         return;
      end if;

      if Msg_Len < 4 or else Msg_Len + 4 /= N32 (Msg'Length) or else Msg'Length < 8 then
         Send_Alert_And_Error (S, Decode_Error, Result);
         return;
      end if;
      ...
      declare
         Scheme   : constant Maybe_Sig_Scheme :=
           Scheme_From_Wire (Unsigned_16 (Msg (4)) * 256 + Unsigned_16 (Msg (5)));
         Sig_Len  : constant N32 := N32 (Msg (6)) * 256 + N32 (Msg (7));
         Verified : Boolean;
      begin
         if Sig_Len = 0 or else Sig_Len /= Msg_Len - 4 or else 8 + Sig_Len - 1 > Msg'Last then
            Send_Alert_And_Error (S, Decode_Error, Result);
            return;
         end if;
         ...
         --  not a signature failure (RFC 5246 7.4.8).
         if Scheme = Sig_Ed25519 then
         ...
         if S.HC.Cfg.Verify_Sig_Algo_Count > 0
           and then
             not Sig_Scheme_In_List
                   (Scheme, S.HC.Cfg.Verify_Sig_Algos, S.HC.Cfg.Verify_Sig_Algo_Count)
         then
            Send_Alert_And_Error (S, Illegal_Parameter, Result);
            return;
         end if;
         ...
         begin
            ...
            Verified := CV_Signature_Valid_12 (S.HC.TS, Sig, D.Peer_Leaf.Cert, Scheme);
         end;
      ...
      Set_State (S, Wait_Client_Finished);
      Result := OK;
   end Handle_Client_CertVerify_Msg_12;

   ------------------------------------------------------------------
   ------------------------------------------------------------------
   --  Expected client Finished verify_data (RFC 5246 7.4.9) over the
   --  current transcript.
   procedure Expected_Client_Finished_12
     (TS      : in SPARKTLS_Transcript.Transcript_State;
      Master  : in Bytes_48;
      Use_384 : in Boolean;
      Exp     : out Key_Schedule_12.Verify_Data_12)
   is
      use Key_Schedule_12;
      TH  : Digest;
      TH4 : SPARKNaCl.Hashing.SHA384.Digest;
   begin
      if Use_384 then
         ...
         begin
            ...
            pragma Assert (TH_Bytes'Length = 48);
            pragma Assert (TH_Bytes'Length = 32 or else TH_Bytes'Length = 48);
            Compute_Finished_12 (Exp, Master, Label_Client_Finished, TH_Bytes, True);
         end;
      else
         SPARKTLS_Transcript.Current_256 (TS, TH);
         Prove_Client_Finished_Label;
         ...
         begin
            ...
            pragma Assert (TH_Bytes'Length = 32);
            pragma Assert (TH_Bytes'Length = 32 or else TH_Bytes'Length = 48);
            Compute_Finished_12 (Exp, Master, Label_Client_Finished, TH_Bytes, False);
         end;
      end if;
   end Expected_Client_Finished_12;

   ----------------------------------------------------------------------------
   --  Verify_Client_Finished_12
   --  Validates a complete decrypted Finished message (type, length header,
   --  exact record fit per RFC 5246 7.4.9) and checks verify_data in
   --  constant time. On success appends the message to the transcript and
   --  sets Verified; on failure sends the appropriate alert (encrypted in
   --  the abbreviated-resume flow, where our CCS is already out).
   ----------------------------------------------------------------------------
   procedure Verify_Client_Finished_12
     (S         : in out Session;
      ...
      Result    : out Action)
   with Pre => Plaintext'First = 0 and then PL >= 4 and then PL - 1 <= Plaintext'Last;
   ...
   begin
      ...
      declare
         Exp : Verify_Data_12;
      begin
         Expected_Client_Finished_12 (S.HC.TS, S.HC.Master_Secret_12, Use_384, Exp);
         ...
         declare
            Received : constant Key_Schedule_12.Verify_Data_12 :=
              Key_Schedule_12.Verify_Data_12 (Plaintext (4 .. 4 + Finished_Verify_Len - 1));
         begin
      ...
      Verified := True;
      Result := OK;
   end Verify_Client_Finished_12;

   --  RFC 5077 3.3 NewSessionTicket (full handshake): issued iff the client
   ...
   --  Full-handshake server flight: [NST?] + CCS + encrypted Finished,
   --  assembled in a scratch buffer and committed to S.Output atomically
   --  (all or nothing). The Finished encryption advances
   --  S.Server_App.Counter, so a commit failure after encryption is fatal:
   --  the burned nonce stays burned and the connection dies here. On any
   --  failure the error state is set and OK is False.
   --
   --  NST goes BEFORE CCS (RFC 5077 3.3): server's WRITE state is still
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            Verify_Client_Finished_12 (S, Plaintext, PL, Use_384, Fin_OK, Result);
            if not Fin_OK then
               return;
            end if;
         end;
      ...
      if not S.HC.T12.Resuming then
         Commit_Finished_Flight_12 (S, Use_384, Flight_OK);
   ...
   begin
      case S.State is
         ...
         when Wait_Client_Certificate | Wait_Client_Cert_Verify =>
            --  Certificate, ClientKeyExchange and CertificateVerify all
            --  arrive as handshake records and go through one message pump
            --  (they may share a record or span several).
