...
private package SPARKTLS.Key_Update
  with SPARK_Mode => On
is
   ...
   with Pre => Len in 32 | 48, Post => TK.Counter = 0 and then TK.Suite = Suite;

   --  Build a KeyUpdate handshake message into Out_Buf.
   --  Request is True to ask the peer to rotate in turn.
   procedure Build_Key_Update (Out_Buf : out Byte_Seq; Len : out N32; Request : in Boolean)
   with
     Pre => Out_Buf'First = 0 and then Out_Buf'Length >= Key_Update_Msg_Len,
