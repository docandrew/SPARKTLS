...
package body SPARKTLS.Key_Update
  with SPARK_Mode => On
is
   ...
   procedure Install_Keys (TK : out Traffic_Keys; Secret : in Bytes_48; Suite : in Supported_Suite)
   ...
   procedure Update_Secret
     (Secret : in out Bytes_48; Len : in N32; TK : in out Traffic_Keys; Suite : in Supported_Suite)
   is
   ...
   procedure Build_Key_Update (Out_Buf : out Byte_Seq; Len : out N32; Request : in Boolean) is
   begin
      Out_Buf := (others => 0);
      ...
      Out_Buf (2) := 0;
      Out_Buf (3) := 1;
      Out_Buf (4) := (if Request then Update_Requested else Update_Not_Requested);
   ...
   procedure Parse_Key_Update (Msg : in Byte_Seq; Request : out Boolean; Status : out Parse_Status)
   is
   begin
      Request := False;
