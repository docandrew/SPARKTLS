...
| Harvest-now-decrypt-later against the key exchange | closed | TLS 1.3 offers and prefers X25519MLKEM768 (draft-ietf-tls-ecdhe-mlkem): the handshake secret is HKDF over ML-KEM-768 secret || X25519 secret, so a future quantum computer breaking X25519 alone recovers nothing. ML-KEM-768 comes from `sparkmlkem`, SPARK-proved, sanitising every intermediate, with ctgrind and dudect lanes; the encapsulation key is checked per FIPS 203 7.2 before use and the hybrid is refused in TLS 1.2. `Offer_Post_Quantum` turns it off. | `Generate_KS_Hybrid`, `Compute_SH_Shared_Secret`; OpenSSL 3.6 interop both directions; BoGo `*X25519MLKEM768*` cases. |
